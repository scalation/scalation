      ...
      extends Forecaster
{
    ...
    def transformBack_f (xf: VectoD): VectoD =
    {
        d match {
            case 0 => xf
            case 1 => val tbx = xx.slice(xx.size - 1) ++ xf
                for (i <- 1 until tbx.dim) tbx(i) += tbx(i-1)
                tbx.slice(1)
            case 2 => val tbx = xx.slice(xx.size - 2) ++ xf
                for (i <- 2 until tbx.dim) tbx(i) += (2*tbx(i-1) - tbx(i-2))
                tbx.slice(2)
            case 3 => val tbx = xx.slice(xx.size - 3) ++ xf
    ...
    def hannanRissanen (): VectoD =
    {
        ...
        breakable {for (i <- 0 until maxIterations) {
            ...
            if (i > minIterations && (sse >= minSSE || sse.isNaN)) {    // can only break the loop if it has run at least 'minIterations' times
                φθ = bestbφθ
                e = beste
                break
            } // if
