...
import scalation.linalgebra.{Converter, MatriD, MatriI, MatrixI, VectoD, VectorI, VectorS}
import scalation.linalgebra.par.{MatrixD, VectorD}
import scalation.stat.Statistic
import scalation.util.{banner, Error, time}

import scalation.analytics.{HyperParameter, Strings}
 ...
 */
class ANCOVA (x_ : MatrixD, t: MatriI, y: VectorD, fname_ : Strings = null, technique: RegTechnique = QR)
      extends Regression (x_ ++^ ANCOVA.dummyVars (t), y, fname_, null, technique)
{
    ...
    {
        crossValidate ((x: MatriD, y: VectoD) => new ANCOVA (x_, t, y.asInstanceOf [VectorD], fname, technique),
                                                 xx, k, rando)
    } // crossVal
...
object ANCOVA extends Error
{
     ...
     */
    def dummyVars (t: MatriI): MatrixD =
    {
