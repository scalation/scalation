      ...
      extends MatriR with Error with Serializable
{
    ...
    def set (u: MatriR)
    {
        if (u.isInstanceOf [SymTriMatrixR]) {
            val uu = u.asInstanceOf [SymTriMatrixR]
            for (i <- range_d) _dg(i) = uu._dg(i)
            for (j <- range_s) _sd(j) = uu._sd(j)
        } else {
            flaw ("set", "must convert u to a SymTriMatrixR first")
        } // if
    } // set
     ...
     */
    def sliceEx (row: Int, col: Int): SymTriMatrixR =
    {
        throw new UnsupportedOperationException ("SymTriMatrixR does not support sliceEx")
    } // sliceEx
     ...
     */
    def dot (b: SymTriMatrixR): VectorR = 
    {
        if (dim1 != b.dim1) flaw ("dot", "matrix dot matrix - incompatible first dimensions")
        if (dim1 == 1) return VectorR (_dg(0) * b._dg(0))
        
        var c = new VectorR (dim2)
    ...
    {
        if (dim1 != b.dim1) flaw ("dot", "matrix dot matrix - incompatible first dimensions")
        if (dim1 == 1) return VectorR (_dg(0) * b(0, 0))
     ...
     *  @param b  the second matrix of the dot product
     */
    def mdot (b: SymTriMatrixR): MatrixR = 
    {
     ...
     *  @param u  the vector to multiply by
     */
    def ** (u: VectoR): MatrixR = this * SymTriMatrixR (u)
     ...
     *  @param u  the vector to multiply by
     */
    def **: (u: VectoR): MatrixR = SymTriMatrixR (u) * this
    ...
    def solve (b: VectoR): VectoR =
    {
        ...
        val d = _dg                           // diagonal
        val e = new VectorR (_sd ++ _0)      // augmented super-diagonal
     ...
     */
    def sumAbs: Real = _dg.norm1 + _sd.norm1 + _sd.norm1
...
object SymTriMatrixR extends Error
{
     ...
     */ 
    def apply (u: VectoR): SymTriMatrixR =
    {   
        val x = new SymTriMatrixR (u.dim)
 ...
 *  > runMain scalation.linalgebra.SymTriMatrixRTest
 */
