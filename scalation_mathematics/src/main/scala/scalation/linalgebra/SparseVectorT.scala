...
import scalation.math.{TimeO, noTimeNum}
import scalation.util.{Error, SortedLinkedHashMap}
import scalation.util.SortingT
import scalation.util.SortingT.{iqsort, qsort2}
 ...
 */
class SparseVectorT (val dim_ : Int,
           protected var v:     SparseVectorT.RowMap = null)
      extends VectoT
//    extends Traversable [TimeNum] with PartiallyOrdered [SparseVectorT] with Vec with Error with Serializable
{
    /** Dimension
     */
    lazy val dim = dim_
    
    if (v == null) v = new SparseVectorT.RowMap ()
    
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Construct a vector and assign values from vector 'u'.
     *  @param u  the other vector
     */
    def this (u: VectoT) { this (u.dim); for (i <- range) this(i) = u(i) } 
     ...
     */
    def this (iv: (Int, TimeNum), dm: Int) { this (dm); this(iv._1) = iv._2 }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create an exact copy of 'this' vector.
     */
    def copy: SparseVectorT = new SparseVectorT (dim, v.clone.asInstanceOf [SparseVectorT.RowMap])

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a zero vector (all elements are zero) of length 'size'.
     *  @param size  the number of elements in the vector
     */
    def zero (size: Int = dim): SparseVectorT = new SparseVectorT (size)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a one vector (all elements are one) of length 'size'.
     *  @param size  the number of elements in the vector
     */
    def one (size: Int = dim): SparseVectorT = { val sv = new SparseVectorT (size); sv.set (_1); sv }
     ...
     */
    def oneAt (j: Int, size: Int = dim): SparseVectorT = new SparseVectorT ((j, _1), size)
     ...
     *  @param size  the size of the vector (upper bound = size - 1)
     */
    def _oneAt (j: Int, size: Int = dim): SparseVectorT = new SparseVectorT ((j, -_1), size) 
      
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' vector's element at the 'i'-th index position. 
     *  @param i  the given index
     */
    def apply (i: Int): TimeNum = if (i >= dim) throw new IndexOutOfBoundsException else v.getOrElse (i, _0)
                                   
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' vector's elements within the given range (vector slicing).
     *  @param r  the given range
     */
    def apply (r: Range): SparseVectorT = slice (r.start, r.end)
     ...
     *  @param x  the value to assign
     */
    def update (r: Range, x: TimeNum)  { for (i <- r) this(i) = x } 
     ...
     */
    def update (r: Range, u: VectoT) { for (i <- r) this(i) = u(i - r.start) }
    
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Iterate over 'this' vector element by element.
     *  @param f  the function to apply
     */
    def foreach [U] (f: TimeNum => U) { var i = 0; while (i < dim) { f (this(i)); i += 1 } } 
    
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Set each value in 'this' vector to 'x'.
     *  @param x  the value to be assigned
     */
    def set (x: TimeNum) { for (i <- range) this(i) = x }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Set the values in 'this' vector to the values in array 'u'.
     *  @param u  the array of values to be assigned
     */
    def set (u: Seq [TimeNum]) { for (i <- range) this(i) = u(i) }
    
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `SparseVectorT` into a `VectorI`.
     */
    def toInt: VectorI = 
    {
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `SparseVectorT` into a `VectorL`.
      */
    def toLong: VectorL =
    { 
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `SparseVectorT` into a `VectorD`.
     */
     ...
     */
    def expand (more: Int = dim): SparseVectorT =
    {
        if (more < 1) this       // no change
        else {
            val sv = new SparseVectorT (dim + more)
            sv.v ++= v
            sv
        } // if
    } // expand
     ...
     */
    private def nonZero: Array [TimeNum] = v.values.toArray
    
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Return 'this' vector's positive elements as an array.
     */
    private def positive: Array [TimeNum] = v.values.filter (_ > _0).toArray
    
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Return 'this' vector's negatives elements as an array.
     */
    private def negative: Array [TimeNum] = v.values.filter (_ < _0).toArray
     ...
     */
    override def filter (p: TimeNum => Boolean): SparseVectorT =
    { 
        SparseVectorT (for (i <- range if p (this(i)) ) yield v(i))
    } // filter
     ...
     *  that 'e != noTimeNum'.
     */
    def filterMissing: SparseVectorT = filter (_ != noTimeNum)
     ...
     *  @param f  the function to apply
     */
    def map (f: TimeNum => TimeNum): SparseVectorT = SparseVectorT (this ().map (f))
     ...
     */
    override def slice (from: Int, till: Int): SparseVectorT =
    {
        null    // FIX: new SparseVectorT (till - from, v.slice (from, till))
    } // slice
     ...
     */
    def select (basis: Array [Int]): SparseVectorT =
    {
        SparseVectorT (for (i <- 0 until basis.length) yield this(basis(i)))
    } // select
     ...
     */
    def ++ (b: VectoT): SparseVectorT =
    {
        val c = new SparseVectorT (dim + b.dim)
     ...
     */
    def ++ (s: TimeNum): SparseVectorT =
    {
        val c = new SparseVectorT (dim + 1)
     ...
     */
    def + (b: VectoT): SparseVectorT = { SparseVectorT (for ( i <- range) yield this(i) + b(i)) }
    ...
    {
        if (s =~ _0) this 
        else SparseVectorT (for (i <- range) yield this(i) + s)       
    } // +
     ...
     */
    def + (s: (Int, TimeNum)): SparseVectorT =
    {
        if (s._1 =~ _0) this
        else { val c = new SparseVectorT (this); c(s._1) += s._2; c }
    } // +
     ...
     *  @param b  the vector to add
     */
    def += (b: VectoT): SparseVectorT = { for (i <- range) this(i) += b(i); this }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Add in-place 'this' vector and scalar 's'.
     *  @param s  the scalar to add
     */
    def += (s: TimeNum): SparseVectorT = { for (i <- range) this(i) += s; this } 
 
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Return the negative of 'this' vector (unary minus).
     */
    def unary_- (): SparseVectorT =
    {
        val c = new SparseVectorT (dim)
        for (x <- v) c(x._1) = -x._2 
        c
    } // unary_-
     ...
     *  @param b  the vector to multiply by
     */
    def *= (b: VectoT): SparseVectorT = { for (x <- v) this(x._1) = x._2 * b(x._1); this }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Multiply in-place 'this' vector and scalar 's'.
     *  @param s  the scalar to multiply by
     */
    def *= (s: TimeNum): SparseVectorT = { for (x <- v) this(x._1) = x._2 * s; this }
     ...
     */
    def sumNE (i: Int): TimeNum = sum - this(i)
     ...
     */
    def normalize: SparseVectorT = this * (_1 / sum)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Normalize 'this' vector so its length is one (unit vector).
     */
    def normalizeU: SparseVectorT = this * (_1 / norm)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Normalize 'this' vector to have a maximum of one.
     */
    def normalize1: SparseVectorT = this * (_1 / max ())
     ...
     *  @param b  the other vector
     */
    def dot (b: SparseVectorT): TimeNum = 
    {
        var sum = _0
     ...
     *  @param b  the other vector
     */
    def max (b: VectoT): SparseVectorT =
    { 
        val c = new SparseVectorT (dim)
        for (i <- range) { 
            val x = this(i)
     ...
     *  @param b  the other vector
     */
    def min (b: VectoT): SparseVectorT =
    {
        val c = new SparseVectorT (dim)
        for (i <- range) { 
            val x = this(i) 
     ...
     */
    def distinct: SparseVectorT = 
    {
        val us = new SparseVectorT (this); us.sort ()
        var c1 = ArrayBuffer [TimeNum]()     
        c1 += us(0)  
    ...
    {
        var count = 1
        val us = new SparseVectorT (this); us.sort ()                // sorted vector
     ...
     */
//  def exists (pred: (TimeNum) => Boolean): Boolean = v.exists (pred)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Determine whether 'x' is contained in 'this' vector.
     *  @param x  the element to be checked
     */
    def contains (x: TimeNum): Boolean = v.values.exists (_ == x)
    
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Reverse the order of the elements in 'this' vector.
     */
    def reverse (): SparseVectorT = 
    {        
         SparseVectorT (for (i <- (0 until dim).reverse) yield this(i))
    } // reverse

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Determine whether 'this' vector is in sorted (ascending) order.
     */
    def isSorted: Boolean = (new SortingT (this().toArray)).isSorted
    ...
    {                  
        ...
        v =  SparseVectorT (neg ++ Array.fill(dim - (neg.length + pos.length))(_0) ++ pos).v          
    } // sort
    ...
    {                  
        ...
        v =  SparseVectorT (pos ++ Array.fill (dim - (neg.length + pos.length))(_0) ++ neg).v          
    } // sort2
     ...
     *  @param b  the other vector
     */
    def sameDimensions (b: SparseVectorT): Boolean = dim <= b.dim
     ...
     *  @param b  the other vector
     */
    def tryCompareTo [B >: SparseVectorT] (b: B)
        (implicit view_1: (B) => PartiallyOrdered [B]): Option [Int] =
    {
        ...
        for (i <- range) {
            val b_i = b.asInstanceOf [SparseVectorT] (i)
    ...
    {
        b.isInstanceOf [SparseVectorT] && (v equals b.asInstanceOf [SparseVectorT].v)
    } // equals
     ...
     */
    def toDense: VectorT = VectorT (for (i <- range) yield this(i))
    ...
    {
        var sb = new StringBuilder ("SparseVectorT(")
 ...
 */
object SparseVectorT
{
    /** Shorthand type definition for sparse vector
     */
    type RowMap = SortedLinkedHashMap [Int, TimeNum]

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a `SparseVectorT` from one or more values (repeated values `TimeNum`*).
     *  @param x   the first `TimeNum` number
     *  @param xs  the rest of the `TimeNum` numbers
     */
    def apply (x: TimeNum, xs: TimeNum*): SparseVectorT =
    {
        val c = new SparseVectorT (1 + xs.length)
        c(0) = x
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a `SparseVectorT` from a sequence of `TimeNum`s.
     *  @param xs  the sequence of the TimeNum numbers
     */
    def apply (xs: Seq [TimeNum]): SparseVectorT =
    {
        val c = new SparseVectorT (xs.length)
        for (i <- 0 until c.dim) c(i) = xs(i)
        c
    } // apply

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a `SparseVectorT` from one or more values (repeated values `String`*).
     *  @param x   the first `String`
     *  @param xs  the rest of the `String`s
     */
    def apply (x: String, xs: String*): SparseVectorT =
    {
        val c = new SparseVectorT (1 + xs.length)
        c(0)  = TimeNum (x)
        for (i <- 0 until c.dim-1) c(i+1) = TimeNum (xs(i))
        c
    } // apply

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a `SparseVectorT` from an array of `String`s.
     *  @param xs  the array of the `String`s
     */
    def apply (xs: Array [String]): SparseVectorT =
    {
        val c = new SparseVectorT (xs.length)
        for (i <- c.range) c(i) = TimeNum (xs(i))
        c
    } // apply

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a `SparseVectorT` from an array of `String`s, skipping the first 'skip'
     ...
     */
    def apply (xs: Array [String], skip: Int): SparseVectorT =
    {
        val c = new SparseVectorT (xs.length - skip)
        for (i <- skip until xs.length) {
            c.v(i - skip) = if (xs(i) matches "[\\-\\+]?\\d*(\\.\\d+)?") TimeNum (xs(i)) else xs(i).hashCode ()
        } // for
        c
    } // apply
     ...
     */
    def ++ (b: TimeNum, u: SparseVectorT): SparseVectorT =
    {
        val c = new SparseVectorT (u.dim + 1)
        for (i <- c.range) c(i) = if (i == 0) b else u.v(i - 1)
        c
    } // ++

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Return a `SparseVectorT` containing a sequence of increasing integers in a range.
     ...
     */
    def range (start: Int, end: Int): SparseVectorT =
    {
        val c = new SparseVectorT (end - start)
        for (i <- c.range) c.v(i) = (start + i).toTimeNum
        c
    } // range

} // SparseVectorT object
...
{   
    var x: SparseVectorT = null
    var y: SparseVectorT = null

    for (l <- 1 to 4) {
        println ("\n\tTest SparseVectorT on vectors of dim " + l)
        x = new SparseVectorT (l)
        y = new SparseVectorT (l)
        x.set (2)
        y.set (3)
    ...
    val z = SparseVectorT ("1", "2", "3", "4")
    println ("z = " + z)
    println ("z.map (_ * 2)    = " + z.map ((e: TimeNum) => e * 2))
...
object SparseVectorTTest2 extends App
{
    ...
    val v1 = new VectorT (1000000)  
    val v2 = new VectorT (1000000)

    for (x <- 0 until 10) {    
        var cnt = 0   
        while (cnt < 1000000) {
            val x  = r.nextInt (50).toTimeNum + 1
            val x1 = r.nextInt (90)
            if(x1 == 1) v1(cnt) = x
            else v1(cnt) = _0  
            val z  = r.nextInt (50).toTimeNum + 1
            val z1 = r.nextInt (90)
            if(z1 == 1) v2(cnt) = z
            else v2(cnt) = _0
            cnt += 1
    ...
    var z1 = SparseVectorT (v1())
    var z2 = SparseVectorT (v2())
