...
import scalation.math.{StrO, noStrNum}
...
//    extends Traversable [StrNum] with PartiallyOrdered [VectorS] with Vec with Error with Serializable
{
     ...
     */
    def this (iv: (Int, StrNum), dm: Int) { this (dm); v(iv._1) = iv._2 }
     ...
     *  that 'e != noStrNum'.
     */
    def filterMissing: VectorS = filter (_ != noStrNum) 
     ...
     */
    override def split (k: Int): Array [VectoS] =
    {
        if (k <= 0) flaw ("split", "k must be a positive integer")
        val pieces = Array.ofDim [VectoS] (k)
        val size = dim / k
     ...
     */
    def sum: StrNum = v.sum

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Sum the elements of 'this' vector skipping the 'i'-th element (Not Equal 'i').
     *  @param i  the index of the element to skip
     */
    def sumNE (i: Int): StrNum = v.sum - v(i)
     ...
     */
    def normalize: VectorS = this * (_1 / v.sum)
...
object VectorS
{
    ...
    def apply (x: String, xs: String*): VectorS =
    {
        val c = new VectorS (1 + xs.length)
        try c(0)  = StrNum (x)
        catch { case _ : NumberFormatException | _ : ClassCastException => c.v(0) = noStrNum }
        for (i <- 1 until c.dim) {
            try c.v(i) = StrNum (xs(i-1))
            catch { case _ : NumberFormatException | _ : ClassCastException => c.v(i) = noStrNum }
        } // for
        c
    } // apply
