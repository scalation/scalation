      ...
      extends MatriI with Error with Serializable
{
     ...
     */
    override def apply (iv: VectoI): MatrixI =
    {
        val a = Array.ofDim [Array [Int]] (iv.dim)
        for (i <- a.indices) a(i) = v(iv(i))
        new MatrixI (a.length, dim2, a)
    } // apply
    ...
    def set (b: MatriI)
    {
        if (b.isInstanceOf [MatrixI]) {
            ...
            flaw ("set", "must convert b to a MatrixI first")
        } // if
    } // set
     ...
     */
    override def map (f: VectoI => VectoI): MatrixI =
    {
        MatrixI (for (i <- range1) yield f(new VectorI (v(i).length, v(i))), false)
    } // map
    ...
    {
        if (from >= end) return new MatrixI (0, dim2)
    ...
    {
        if (from >= end) return new MatrixI (dim1, 0)
...
//  } // t
     ...
     */
    override def ** (b: MatriI): MatrixI = MatrixI (for (i <- range1) yield this(i) * b(i), false)
    ...
    def lud_npp: (MatrixI, MatrixI) =
    {
        ...
        val l = new MatrixI (dim1)          // lower triangular matrix
        val u = new MatrixI (this)          // upper triangular matrix (a copy of this)

        for (i <- u.range1) {
            val pivot = u.v(i)(i)
            if (pivot =~ 0) flaw ("lud_npp", "use Fac_LU since there is a zero pivot at row " + i)
            l.v(i)(i) = 1
    ...
    def lud_ip (): (MatrixI, MatrixI) =
    {
        ...
        for (i <- u.range1) {
            var pivot = u.v(i)(i)
            if (pivot =~ 0) flaw ("lud_ip", "use Fac_LU since there is a zero pivot at row" + i)
            l.v(i)(i) = 1
            for (j <- i + 1 until u.dim2) l.v(i)(j) = 0
            for (k <- i + 1 until u.dim1) {
                val mul = u.v(k)(i) / pivot
...
object MatrixI extends Error
{
     ...
     */
    def apply (u: Array [Array [Int]]): MatrixI =
    {
        new MatrixI (u.length, u(0).length, u)
    } // apply
