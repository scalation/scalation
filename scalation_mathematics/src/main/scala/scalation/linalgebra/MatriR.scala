 ...
 *      `RleMatrixR`        - compressed matrix - Run Length Encoding (RLE)
      ...
      extends Error
{
     ...
     */
    def slice (rg: Range): MatriR = slice (rg.start, rg.end)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Slice 'this' matrix row-wise excluding the given range 'rg'.
     *  @param rg  the excluded range of the slice
     */
    def sliceEx (rg: Range): MatriR = slice (0, rg.start) ++ slice (rg.end, dim1)
     ...
     */
    def sliceEx (row: Int, col: Int): MatriR
     ...
     */
    def selectRowsEx (rowIndex: Array [Int]): MatriR = selectRows ((range1 diff rowIndex).toArray)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Select all rows from 'this' matrix excluding the rows from the given 'rowIndex'.
     *  @param rowIndex  the row indices to exclude
     */
    def selectRowsEx (rowIndex: VectoI): MatriR = selectRows ((range1 diff rowIndex()).toArray)
    ...
    def meanNZ: VectoR =
    {
        ...
        for (j <- range2) {
            ...
            cm(j) = if (nzs > 0) col (j).sum / nzs.toReal else _0
        } // for
        cm
    } // meanNZ
    ...
    def meanR: VectoR =
    {
        ...
        for (i <- range1) rm(i) = this(i).sum / dim2.toReal
        rm
    } // meanR
    ...
    def meanRNZ: VectoR =
    {
        ...
        for (i <- range1) {
            ...
            rm(i) = if (nzs > 0) this(i).sum / nzs.toReal else _0
        } // for
        rm
    } // meanRNZ
     ...
     *  @see en.wikipedia.org/wiki/Matrix_norm
     */
    def normINF: Real = (for (i <- range1) yield this(i).norm1).max
