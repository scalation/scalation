...
import scalation.math.{long_exp, noLong}
...
//    extends Traversable [Long] with PartiallyOrdered [VectorL] with Vec with Error with Serializable
{
     ...
     */
    override def apply (iv: VectoI): VectorL =
    {
        val c = new VectorL (iv.dim)
     ...
     *  that 'e != noLong'.
     */
    def filterMissing: VectorL = filter (_ != noLong) 
     ...
     */
    override def split (k: Int): Array [VectoL] =
    {
        if (k <= 0) flaw ("split", "k must be a positive integer")
        val pieces = Array.ofDim [VectoL] (k)
        val size = dim / k
...
object VectorL
{
    ...
    def apply (x: String, xs: String*): VectorL =
    {
        val c = new VectorL (1 + xs.length)
        try c(0)  = x.toLong
        catch { case _ : NumberFormatException | _ : ClassCastException => c.v(0) = noLong }
        for (i <- 1 until c.dim) {
            try c.v(i) = xs(i-1).toLong
            catch { case _ : NumberFormatException | _ : ClassCastException => c.v(i) = noLong }
        } // for
        c
    } // apply
