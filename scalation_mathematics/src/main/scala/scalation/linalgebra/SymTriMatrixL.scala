      ...
      extends MatriL with Error with Serializable
{
    ...
    def set (u: MatriL)
    {
        if (u.isInstanceOf [SymTriMatrixL]) {
            val uu = u.asInstanceOf [SymTriMatrixL]
            for (i <- range_d) _dg(i) = uu._dg(i)
            for (j <- range_s) _sd(j) = uu._sd(j)
        } else {
            flaw ("set", "must convert u to a SymTriMatrixL first")
        } // if
    } // set
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixL must be symmetric")
    } // slice
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixL does not support selectRows")
    } // selectRows
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixL does not support selectCols")
    } // selectCols
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixL does not support +:")
    } // +:
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixL does not support + for VectoL")
    } // +
    ...
    def * (b: MatriL): SymTriMatrixL = 
    {
        throw new UnsupportedOperationException ("SymTriMatrixL does not support * with general matrices")
    } // *
     ...
     *  @param u  the vector to multiply by
     */
    def ** (u: VectoL): MatrixL = this * SymTriMatrixL (u)
     ...
     *  @param u  the vector to multiply by
     */
    def **: (u: VectoL): MatrixL = SymTriMatrixL (u) * this
     ...
     */
    def sumAbs: Long = _dg.norm1 + _sd.norm1 + _sd.norm1
...
object SymTriMatrixL extends Error
{
     ...
     */ 
    def apply (u: VectoL): SymTriMatrixL =
    {   
        val x = new SymTriMatrixL (u.dim)
 ...
 *  > runMain scalation.linalgebra.SymTriMatrixLTest
 */
