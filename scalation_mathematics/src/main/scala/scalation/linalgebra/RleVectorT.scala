...
import scalation.util.{ReArray, SortingT}
import scalation.util.SortingT.{iqsort, qsort2}

//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `TripletT` class holds a run length encoded triplet, e.g., (_1, 2, 3)
 ...
 */
class TripletT (var value: TimeNum, var count: Int, var startPos: Int)
{
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Update 'this' triplet.
     *  @param x  the tuple value to which 'this' triplet should be updated
     */
    def update (x: (TimeNum, Int, Int))
    {
    ...
    {
        val yy = y.asInstanceOf [TripletT]
...
object RleVectorT 
{
     ...
     *  @param y  the `VectorT` to be compressed
     */
    def apply (y: VectoT): RleVectorT = 
    {
        if (y.size == 0) return new RleVectorT (0)
        val x     = y.asInstanceOf [VectorT]
        var c1    = new ReArray [TripletT] ()
        var idx   = 0
        var count = 1
        for (i <- 1 until x.dim) {
            if (x(i) != x(i-1)) {
                c1(idx) = new TripletT (x(i-1), count, i - count); idx += 1
                count = 1
            } else count += 1
        } // for
        c1(idx) = new TripletT (x(x.dim - 1), count, x.dim - count)
        new RleVectorT (x.dim, c1)
    } //apply 

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a `RleVectorT` from a sequence of `TimeNum`.
     *  @param xs  the sequence of the `TimeNum`
     */
    def apply (xs: Seq [TimeNum]): RleVectorT =
    { 
        if (xs.length == 0) return new RleVectorT (0)
        var c1    = new ReArray [TripletT] ()
        var idx   = 0
        var count = 1
        for (i <- 1 until xs.size) {
            if (xs(i) != xs(i-1)) {
                c1(idx) = new TripletT (xs(i-1), count, i - count); idx += 1
                count = 1
            } else count += 1
        } // for
        c1(idx) = new TripletT (xs(xs.size-1), count, xs.size - count)
        new RleVectorT (xs.size, c1)
    } // apply
     ...
     */
    def apply (x: TimeNum, xs: TimeNum*): RleVectorT =
    {
        var c1    = new ReArray [TripletT] ()
        var idx   = 0
        var count = 1
        if (xs.size == 0) {
            c1(idx) = new TripletT (x, 1, 0)
            return new RleVectorT (xs.size + 1, c1)
        } // if
        if (xs(0) != x) {
            c1(idx) = new TripletT (x, 1, 0); idx += 1
            count = 1
        } else count += 1
        for (i <- 1 until xs.size) {      
            if (xs(i) != xs(i-1)) {
     ...
     */
    def apply (xs: Array [String]): RleVectorT =
    {
        if (xs.length == 0) return new RleVectorT (0)
        var c1    = new ReArray [TripletT] ()
        var idx   = 0
        ...
        for (i <- 1 until xs.size) {
            if (xs(i) != xs(i-1)) {
                var z = noTimeNum
                try z = TimeNum (xs(i-1))
                catch { case ex: NumberFormatException => }
                c1(idx) = new TripletT (z, count, i - count); idx += 1
                count = 1
            } else count += 1
        } // for
        var z = noTimeNum
        try z = TimeNum (xs(xs.size-1))
        catch { case ex: NumberFormatException => }
        c1(idx) = new TripletT (z, count, xs.size - count)
        new RleVectorT (xs.size, c1)
    } // apply
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `RleVectorT` class stores and operates on compressed Numeric Vectors 
 *  of base type `TimeNum`. 
 *  @param dim  the dimension/size of the vector (uncompressed data)
 *  @param v    the 1D array used to store vector elements
 */   
class RleVectorT (val dim: Int, protected var v: ReArray [TripletT] = null)  
      extends VectoT
{      
    if (v == null) {
        v = new ReArray [TripletT] () 
        v(0) = new TripletT (_0, dim, 0)
    } // if
     ...
     */
    def copy: RleVectorT = new RleVectorT (this)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get the uncompressed vector's element at the 'i'-th index position. 
     *  @param i  the given index
     */
    def apply (i: Int): TimeNum = v(getTriplet(i)).value

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' vector's elements within the given range (vector slicing).
     *  @param r  the given range
     */
    def apply (r: Range): VectorT = slice (r.start, r.end)
     ...
     */
    private def mergeRight (tripPos: Int, value: TimeNum) =
    {       
        if (tripPos + 1 >= csize) { 
            v(tripPos).count -= 1
            v(tripPos + 1) = new TripletT (value, 1, v(tripPos).count + v(tripPos).startPos)      
     ...
     *  @param value   the new value  
     */
    private def mergeLeft (tripPos: Int,value: TimeNum) = 
    {
        if (tripPos == 0) {
            v.shiftRight (tripPos)
            v(tripPos) = new TripletT (value, 1, 0)
            v(tripPos + 1).count -= 1; v(tripPos + 1).startPos = 1
        } else if (value =~ v(tripPos - 1).value) {
            ...
            v.shiftRight (tripPos)           
            v(tripPos) = new TripletT (value, 1, v(tripPos + 1).startPos)
            v(tripPos + 1).count -= 1; v(tripPos + 1).startPos += 1
        } // if
    } // mergeLeft
     ...
     *  @param value   the new value  
     */
    private def split (idx: Int, tripPos: Int, value: TimeNum) = 
    { 
        v.shiftRight2 (tripPos)
        val oldCount = v(tripPos).count             
        v(tripPos).count = idx - v(tripPos).startPos
        v(tripPos + 1) = new TripletT (value, 1, v(tripPos).startPos + v(tripPos).count)
        v(tripPos + 2) = new TripletT (v(tripPos).value, oldCount - (v(tripPos).count + 1), 
     ...
     *  @param value   the new value  
     */
    private def mergeBoth (tripPos: Int, value: TimeNum) = 
    {
    ...
    def set (x: TimeNum) 
    {      
        v = new ReArray [TripletT] (1) 
        v(0) = new TripletT (x, dim, 0)
    } // set
    ...
    {     
        ...
        else new VectorT (dim + more, Array.concat (c().toArray, new Array [TimeNum] (more)))
    } // expand
     ...
     */
    def zero (size: Int): RleVectorT = new RleVectorT (size)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a one vector (all elements are one) of length 'size'.
     *  @param size  the size of the new vector
     */
    def one (size: Int): RleVectorT = 
    {
        val trip = new TripletT (_1, size, 0)
        new RleVectorT (size, ReArray.fill (1) (trip))
    } // one
     ...
     */
    def oneAt (j: Int, size: Int = dim): RleVectorT =
    {   
        val c = new RleVectorT (size)
        c(j)  = _1
        c
    } // oneAt
     ...
     *  @param size  the size of the vector (upper bound = size - 1)
     */
    def _oneAt (j: Int, size: Int = dim): RleVectorT =
    {
        val c = new RleVectorT (size)
        c(j)  = -_1
        c
     ...
     */
    override def filter (p: TimeNum => Boolean): RleVectorT = 
    {    
        val c = new ReArray [TimeNum] ()
        var idx = 0
     ...
     *  that 'e != noTimeNum'.
     */
    def filterMissing: RleVectorT = filter (_ != noTimeNum)
     ...
     *  @param f  the function to apply
     */
    def map (f: TimeNum => TimeNum): RleVectorT = 
    {    
        val ar = v()
        RleVectorT (for (i <- crange; j <- 0 until ar(i).count) yield f (ar(i).value))     
    } // map
     ...
     */
    override def slice (from: Int, till: Int): VectorT = new VectorT (till - from, 
    ...
    {
        val c = new VectorT (basis.length)
     ...
     */
    def ++ (s: TimeNum): RleVectorT =
    {    
        val c = new RleVectorT (dim + 1, new ReArray [TripletT] (csize))  
//      for (i <- crange) c.v(i) = v(i)
        for (i <- crange) c.v(i) = new TripletT (v(i).value, v(i).count, v(i).startPos)
        if (v(csize - 1).value == s) c.v(csize - 1).count += 1     
        else c.v(csize) = new TripletT (s, 1, v(csize - 1).startPos + v(csize - 1).count)      
        c
    } // ++
    ...
    {     
        b match {
        case _: RleVectorT => this + b.asInstanceOf [RleVectorT]
        case _             => val c = new VectorT (dim)
                               var k = 0
     ...
     */
    def + (x: RleVectorT): VectoT =  
    {      
        var (i, j) = (0, 0) 
        var (a, b) = (v(0).count, x.v(0).count)  
        var v1 = new ReArray [TripletT] ()
     
        var sum = v(0).value + x.v(0).value
        var currCount = MIN (a, b)
        var startPos = 0
        var idx = 0
        var currTriple = new TripletT (sum, currCount, startPos)
        ...
        while (totalCount < dim) {
            ...
            if (sum == oldSum) {
                ...
                currTriple = new TripletT (sum, currCount, startPos)
                v1(idx) = currTriple; idx += 1 
                oldSum = sum
            } // if    
        } // while
        new RleVectorT (dim, v1)    
    } // +
    ...
    def + (s: TimeNum): RleVectorT =
    {
        val c = new RleVectorT (dim, new ReArray [TripletT] (csize))
        for (i <- crange)  c.v(i) = new TripletT (v(i).value + s, v(i).count, v(i).startPos) 
        c
    } // +
    ...
    {      
        ...
        var startPos = 0
        var idx = 0
        var currTriple = new TripletT (diff, currCount, startPos)
        v1(idx) = currTriple; idx += 1 
        var oldDiff = diff
        var totalCount = currCount
        ...
        while (totalCount < dim) {
            ...
            if (diff == oldDiff) {
                ...
                currTriple = new TripletT (diff, currCount, startPos)
                v1(idx) = currTriple; idx += 1 
                oldDiff = diff
            } // if    
        } // while
     ...
     */
    def -= (b: VectoT): RleVectorT = { for (i <- range) this(i) -= b(i); this }
    ...
    {      
        ...
        var startPos = 0
        var idx = 0
        var currTriple = new TripletT (prod, currCount, startPos)
        v1(idx) = currTriple; idx += 1 
        var oldProd = prod
        var totalCount = currCount
        ...
        while (totalCount < dim) {
            ...
            if (prod == oldProd) {
                ...
                currTriple = new TripletT (prod, currCount, startPos)
                v1(idx) = currTriple; idx += 1 
                oldProd = prod
            } // if    
        } // while
    ...
    {      
        ...
        var startPos = 0
        var idx = 0
        var currTriple = new TripletT (div, currCount, startPos)
        v1(idx) = currTriple; idx += 1 
        var oldDiv = div
        var totalCount = currCount
        ...
        while (totalCount < dim) {
            ...
            if (div == oldDiv) {
                ...
                currTriple = new TripletT (div, currCount, startPos)
                v1(idx) = currTriple; idx += 1 
                oldDiv = div
            } // if    
        } // while
     ...
     */
    def recip: RleVectorT =
    {
        val c = new RleVectorT (dim, new ReArray [TripletT] (csize))
        for (i <- crange) c.v(i) = new TripletT (_1 / v(i).value, v(i).count, v(i).startPos)
        c
    } // recip
    ...
    {    
        ...
        for (i <- 1 until v.length) {          
            ...
            if (currValue == oldValue) {
                ...
                oldTriple = new TripletT (currValue, v(i).count, c.v(i-1).startPos + c.v(i-1).count)
     ...
     */
    def sum: TimeNum = v.foldLeft (_0) ((s, a) => s + (a.value * a.count)) 
     ...
     */    
    def sumPos: TimeNum = v.foldLeft (_0) ((s, a) => s + MAX ((a.value * a.count), _0))
     ...
     */
    def normalize: RleVectorT = this * (_1 / sum)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Normalize 'this' vector so its length is one (unit vector).
     */
    def normalizeU: RleVectorT = this * (_1 / norm)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Normalize 'this' vector to have a maximum of one.
     */
    def normalize1: RleVectorT = this * (_1 / max ())
    ...
    {
        b match {
        case _: RleVectorT => this dot b.asInstanceOf [RleVectorT]
        case _             => var sum = _0
     ...
     */  
    def norm1: TimeNum = v.foldLeft (_0) ((s, a) => s + (ABS (a.value) * a.count))
     ...
     */
    def distinct: RleVectorT =
    {     
        val u = new ReArray [TripletT] ()
        var idx = 0
        val s = HashSet [TimeNum] ()  
        var sPos = 0
        for (t <- v if !(s contains t.value)) {
            s += t.value  
            u(idx) = new TripletT (t.value, 1, sPos); idx += 1
            sPos += 1
        } // for
        new RleVectorT (u.length, u)
    } // distinct
     ...
     */
    def reverse (): RleVectorT = 
    {      
        val nv = new ReArray [TripletT] (csize)
        for (i <-crange.reverse) nv(i) = v(i) 
        new RleVectorT (dim, nv)       
    } // reverse
     ...
     *  @param b  the other vector
     */
    def tryCompareTo [B >: RleVectorT] (b: B)
     ...
     *  @return Vector of base type `TimeNum` containing uncompressed data
     */
    def toDense: VectorT =
    { 
        val ar = v()
        VectorT (for (i <- 0 until v.length; j <- 0 until ar(i).count) yield ar(i).value)
    } // toDense
    ...
    {
        b.isInstanceOf [RleVectorT] && (v == b.asInstanceOf [RleVectorT].v)
    } // equals
    ...
    {   
        var sb = new StringBuilder ("RleVectorT (")
