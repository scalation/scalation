...
object RleVectorI 
{
    ...
    def apply (x: Int, xs: Int*): RleVectorI =
    {
        ...
        var count = 1
        if (xs.size == 0) {
            c1(idx) = new TripletI (x, 1, 0)
            return new RleVectorI (xs.size + 1, c1)
        } // if
        if (xs(0) != x) {
     ...
     */
    def apply (xs: Array [String]): RleVectorI =
    {
        if (xs.length == 0) return new RleVectorI (0)
        var c1    = new ReArray [TripletI] ()
        var idx   = 0
        ...
        for (i <- 1 until xs.size) {
            if (xs(i) != xs(i-1)) {
                ...
                c1(idx) = new TripletI (z, count, i - count); idx += 1
                count = 1
            } else count += 1
        } // for
        var z = noInt
        try z = xs(xs.size-1).toInt
        catch { case ex: NumberFormatException => }
        c1(idx) = new TripletI (z, count, xs.size - count)
        new RleVectorI (xs.size, c1)
    } // apply
...
{      
     ...
     *  that 'e != noInt'.
     */
    def filterMissing: RleVectorI = filter (_ != noInt)
