      ...
      extends MatriQ with Error with Serializable
{
     ...
     */
    def copy (): MatrixQ = new MatrixQ (dim1, dim2, (for (i <- range1) yield v(i).clone ()).toArray)
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): MatrixQ = new MatrixQ (m, n)
     ...
     *  @param i  the row index
     */
    def apply (i: Int): VectorQ = VectorQ (v(i))
     ...
     */
    def apply (ir: Range, jr: Range): MatrixQ = slice (ir.start, ir.end, jr.start, jr.end)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get the underlying 2D array for 'this' matrix.
     */
    def apply (): Array [Array [Rational]] = v
     ...
     */
    override def apply (iv: VectoI): MatrixQ =
    {
        val a = Array.ofDim [Array [Rational]] (iv.dim)
        for (i <- a.indices) a(i) = v(iv(i))
        new MatrixQ (a.length, dim2, a)
    } // apply
     ...
     */
    def update (i: Int, u: VectoQ) { v(i) = u().toArray }
    ...
    {
        if (b.isInstanceOf [MatrixQ]) {
            val bb = b.asInstanceOf [MatrixQ]
            for (i <- ir; j <- jr) v(i)(j) = bb.v(i - ir.start)(j - jr.start)
        } else {
            flaw ("update", "must convert b to a MatrixQ first")
        } // if
    } // update
     ...
     *  @param u  the 2D array of values to assign
     */
    def set (u: Array [Array [Rational]]) { for (i <- range1; j <- range2) v(i)(j) = u(i)(j) }
    ...
    def set (b: MatriQ)
    {
        if (b.isInstanceOf [MatrixQ]) {
            ...
            flaw ("set", "must convert b to a MatrixQ first")
        } // if
    } // set
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `MatrixQ` into an integer matrix `MatrixI`.
     */
    def toInt: MatrixI =
    {
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `MatrixQ` into a double matrix `MatrixD`.
     */
     ...
     */
    def toDense: MatrixQ = this

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Map the elements of 'this' matrix by applying the mapping function 'f'.
     *  @param f  the function to apply
     */
    override def map (f: VectoQ => VectoQ): MatrixQ =
    {
        MatrixQ (for (i <- range1) yield f(new VectorQ (v(i).length, v(i))), false)
    } // map
    ...
    {
        if (from >= end) return new MatrixQ (0, dim2)
        new MatrixQ (end - from, dim2, v.slice (from, end))
    } // slice
    ...
    {
        if (from >= end) return new MatrixQ (dim1, 0)
        val c = new MatrixQ (dim1, end - from)
    ...
    {
        if (r_from >= r_end || c_from >= c_end) return new MatrixQ (0, 0)
        val c = new MatrixQ (r_end - r_from, c_end - c_from)
     ...
     */
    def sliceEx (row: Int, col: Int): MatrixQ =
    {
        val c = new MatrixQ (dim1 - oneIf (row < dim1), dim2 - oneIf (col < dim2))
    ...
    {
        val c = new MatrixQ (rowIndex.length, dim2)
     ...
     */
    def setCol (col: Int, u: VectoQ) { for (i <- range1) v(i)(col) = u(i) }
    ...
    {
        val c = new MatrixQ (dim1, colIndex.length)
...
//  } // t
    ...
    def ++^ (b: MatriQ): MatrixQ =
    {
        ...
        val c = new MatrixQ (dim1, dim2 + b.dim2)
    ...
    def *= (b: MatrixQ): MatrixQ =
    {
        ...
        for (i <- range1) {
            val row_i = new VectorQ (dim2)            // save ith row so not overwritten
    ...
    def times_ip (b: MatrixQ)
    {
        ...
        val bb = if (b == this) new MatrixQ (this) else b
        for (i <- range1) {
            val row_i = new VectorQ (dim2)            // save ith row so not overwritten
            for (j <- range2) row_i(j) = v(i)(j)      // copy values from ith row of 'this' matrix
            for (j <- range2) {
     ...
     */
    def times_ip_pre (b: MatrixQ, d: Int = 0)
    {
        val d1_d = dim1 - d
        ...
        for (j <- d until dim2) {
            val col_j = new VectorQ (d1_d)                 // save jth column so not overwritten
    ...
    def times_s (b: MatrixQ): MatrixQ = 
    {
        ...
        val c = new MatrixQ (dim1, dim1)  // allocate result matrix
     ...
     */
    override def ** (b: MatriQ): MatrixQ = MatrixQ (for (i <- range1) yield this(i) * b(i), false)
     ...
     */
    private def partialPivoting (a: MatrixQ, i: Int): Int =
    {
    ...
    {
        val b = new MatrixQ (this)              // copy 'this' matrix into b
    ...
    /** Create a normalized version of 'this' matrix.
      */
    def normalizeU: MatrixQ = MatrixQ (for (j <- range2) yield col(j).normalizeU)
    ...
    {
        if (! b.isInstanceOf [MatriQ]) return false
        val bb = b.asInstanceOf [MatriQ]
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `MatrixQ` companion object provides operations for `MatrixQ` that don't require
...
object MatrixQ extends Error
{
     ...
     */
    def apply (u: Array [Array [Rational]]): MatrixQ =
    {
        new MatrixQ (u.length, u(0).length, u)
    } // apply
     ...
     */
    def apply (u: Seq [VectoQ], columnwise: Boolean = true): MatrixQ =
    {
        var x: MatrixQ = null
        val u_dim = u(0).dim
        if (columnwise) {
            x = new MatrixQ (u_dim, u.length)
            for (j <- x.range2) x.setCol (j, u(j))       // assign column vectors
        } else {
            x = new MatrixQ (u.length, u_dim)
     ...
     */
    def apply (u: Vector [VectoQ]): MatrixQ =
    {
        val u_dim = u(0).dim
     ...
     */
    def apply (fileName: String): MatrixQ =
    {
        val lines  = fromFile (fileName).getLines.toArray         // get the lines from file
        val (m, n) = (lines.length, lines(0).split (sp).length)
        val x      = new MatrixQ (m, n)
        for (i <- x.range1) {
            x(i) = VectorQ (lines(i).split (sp))
            if ((i+1) % PROGRESS == 0) println (s"Read 5 rows ...")
        } // for
        x
    } // apply
     ...
     */
    def apply (fileName: String, skip: Int): MatrixQ =
    {
        val lines  = fromFile (fileName).getLines.toArray         // get the lines from file
        val (m, n) = (lines.length-skip, lines(0).split (sp).length)
        val x      = new MatrixQ (m, n)
        for (i <- x.range1) {
            x(i) = VectorQ (lines(i+skip).split (sp))
            if ((i+1) % PROGRESS == 0) println (s"Read 5 rows ...")
        } // for
        x
    } // apply
     ...
     */
    def ++ (u: VectoQ, w: VectoQ): MatrixQ =
    {
        if (u.dim != w.dim) flaw ("++", "vector dimensions do not match")

        val c = new MatrixQ (2, u.dim)
        c(0) = u
        c(1) = w
        c
    } // ++
     ...
     */
    def outer (x: VectoQ, y: VectoQ): MatrixQ =
    {
        val c = new MatrixQ (x.dim, y.dim)
     ...
     *  @param y  the second vector -> row 1
     */
    def form_rw (x: Rational, y: VectoQ): MatrixQ =
    {
        val cols = y.dim
        val c = new MatrixQ (2, cols)
     ...
     */
    def form_rw (x: VectoQ, y: Rational): MatrixQ =
    {
        val cols = x.dim
        val c = new MatrixQ (2, cols)
        c(0) = x
     ...
     *  @param y  the second vector -> column 1
     */
    def form_cw (x: Rational, y: VectoQ): MatrixQ =
    {
        val rows = y.dim
        val c = new MatrixQ (rows, 2)
     ...
     */
    def form_cw (x: VectoQ, y: Rational): MatrixQ =
    {
        val rows = x.dim
        val c = new MatrixQ (rows, 2)
        c.setCol(0, x)
...
object MatrixQTest extends App
{
    for (l <- 1 to 4) {
        ...
        val x = new MatrixQ (l, l)
        val y = new MatrixQ (l, l)
        x.set (2)
        y.set (3)
        println ("x + y = " + (x + y))
        println ("x - y = " + (x - y))
    ...
    println ("\n\tTest MatrixQ on additional operations")

    val z   = new MatrixQ ((2, 2), 1, 2,
                                   3, 2)
    val t   = new MatrixQ ((3, 3), 1, 2, 3,
                                   4, 3, 2,
                                   1, 3, 1)
    ...
    println ("check left  nullspace = " + MatrixQ.times (v.t.nullspace, v))

    for (row <- z) println ("row = " + row.deep)

    val aa = new MatrixQ ((3, 2), 1, 2,
                                  3, 4,
                                  5, 6)
    val bb = new MatrixQ ((2, 2), 1, 2,
                                  3, 4)
    val cc = new MatrixQ ((3, 2), 3, 4,
                                  5, 6,
                                  7, 8)
