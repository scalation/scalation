      ...
      extends MatriR with Error with Serializable
{
    ...
    def set (u: MatriR)
    {
        if (u.isInstanceOf [BidMatrixR]) {
            val uu = u.asInstanceOf [BidMatrixR]
            for (i <- range_d) _dg(i) = uu._dg(i)
            for (j <- range_s) _sd(j) = uu._sd(j)
        } else {
            flaw ("set", "must convert u to a BidMatrixR first")
        } // if
    } // set
     ...
     */
    def sliceEx (row: Int, col: Int): BidMatrixR =
    {
        throw new UnsupportedOperationException ("BidMatrixR does not support sliceEx")
    } // sliceEx
 ...
 *  > runMain scalation.linalgebra.BidMatrixRTest
 */
