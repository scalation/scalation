...
{ 
    ...
    def set (u: MatriQ)
    {
        if (u.isInstanceOf [RleMatrixQ]) {
            val uu = u.asInstanceOf [RleMatrixQ]
            for (i <- range1; j <- range2) v(i)(j) = uu.v(i)(j)
        } else {
            flaw ("set", "must convert u to a RleMatrixQ first")
        } // if
    } // set
     ...
     */
    def sliceEx (row: Int, col: Int): RleMatrixQ =
    {
 ...
 *  > runMain scalation.linalgebra.RleMatrixQTest
 */
