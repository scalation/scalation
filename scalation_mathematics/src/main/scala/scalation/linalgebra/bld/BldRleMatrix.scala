 ...
 *  > runMain scalation.linalgebra.bld.BldRleMatrix
 */
...
"""; val code2 = raw"""
    ...
    def set (u: $MATRI)
    {
        if (u.isInstanceOf [$RLEMATRIX]) {
            val uu = u.asInstanceOf [$RLEMATRIX]
            for (i <- range1; j <- range2) v(i)(j) = uu.v(i)(j)
        } else {
            flaw ("set", "must convert u to a $RLEMATRIX first")
        } // if
    } // set
...
"""; val code3 = raw"""
     ...
     */
    def sliceEx (row: Int, col: Int): $RLEMATRIX =
    {
...
"""; val code9 = raw"""
     ...
     */
    def lud_npp: ($RLEMATRIX, $RLEMATRIX) = 
    {
        if (! isSquare) throw new IllegalArgumentException ("lud_npp: requires a square matrix")

        val l = new $RLEMATRIX (dim1)
        val u = new $RLEMATRIX (dim1)
...
"""; val code10 = raw"""
    ...
    def reduce: $RLEMATRIX = 
    {
        ...
        for (i <- range1) {
            var x = b(i, i)
            if (x =~ $ZERO) flaw ("reduce", "unable to find a non-zero pivot in row " + i)
 ...
 *  > runMain scalation.linalgebra.${RLEMATRIX}Test
 */
object ${RLEMATRIX}Test extends App
{
    for (l <- 1 to 4) {
...
//      println (code); println (code2); prinrln (code3); println (code4)
//      println (code5); println (code6); println (code7); println (code9)
