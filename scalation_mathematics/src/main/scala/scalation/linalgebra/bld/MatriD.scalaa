 ...
 *      `RleMatrixD`        - compressed matrix - Run Length Encoding (RLE)
      ...
      extends Error
{
     ...
     */
    def slice (rg: Range): MatriD = slice (rg.start, rg.end)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Slice 'this' matrix row-wise excluding the given range 'rg'.
     *  @param rg  the excluded range of the slice
     */
    def sliceEx (rg: Range): MatriD = slice (0, rg.start) ++ slice (rg.end, dim1)
     ...
     */
    def sliceEx (row: Int, col: Int): MatriD
     ...
     */
    def selectRowsEx (rowIndex: Array [Int]): MatriD = selectRows ((range1 diff rowIndex).toArray)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Select all rows from 'this' matrix excluding the rows from the given 'rowIndex'.
     *  @param rowIndex  the row indices to exclude
     */
    def selectRowsEx (rowIndex: VectoI): MatriD = selectRows ((range1 diff rowIndex()).toArray)
    ...
    def meanNZ: VectoD =
    {
        ...
        for (j <- range2) {
            ...
            cm(j) = if (nzs > 0) col (j).sum / nzs.toDouble else 0.0
        } // for
        cm
    } // meanNZ
    ...
    def meanR: VectoD =
    {
        ...
        for (i <- range1) rm(i) = this(i).sum / dim2.toDouble
        rm
    } // meanR
    ...
    def meanRNZ: VectoD =
    {
        ...
        for (i <- range1) {
            ...
            rm(i) = if (nzs > 0) this(i).sum / nzs.toDouble else 0.0
        } // for
        rm
    } // meanRNZ
     ...
     *  @see en.wikipedia.org/wiki/Matrix_norm
     */
    def normINF: Double = (for (i <- range1) yield this(i).norm1).max
