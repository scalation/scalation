...
{ 
    ...
    def set (u: MatriR)
    {
        if (u.isInstanceOf [RleMatrixR]) {
            val uu = u.asInstanceOf [RleMatrixR]
            for (i <- range1; j <- range2) v(i)(j) = uu.v(i)(j)
        } else {
            flaw ("set", "must convert u to a RleMatrixR first")
        } // if
    } // set
    ...
    def lud_npp: (RleMatrixR, RleMatrixR) = 
    {
        ...
        val l = new RleMatrixR (dim1)
        val u = new RleMatrixR (dim1)
     ...
     *  @param b  the constant vector.
     */ 
    def solve (b: VectoR): VectoR = solve (lud_npp, b)
 ...
 *  > runMain scalation.linalgebra.RleMatrixRTest
 */
