...
object RleVectorD 
{
     ...
     */
    def apply (xs: Array [String]): RleVectorD =
    {
        if (xs.length == 0) return new RleVectorD (0)
        var c1    = new ReArray [TripletD] ()
        var idx   = 0
        ...
        for (i <- 1 until xs.size) {
            if (xs(i) != xs(i-1)) {
                var z = noDouble
                try z = xs(i-1).toDouble
                catch { case ex: NumberFormatException => }
                c1(idx) = new TripletD (z, count, i - count); idx += 1
                count = 1
            } else count += 1
        } // for
        var z = noDouble
        try z = xs(xs.size-1).toDouble
        catch { case ex: NumberFormatException => }
        c1(idx) = new TripletD (z, count, xs.size - count)
        new RleVectorD (xs.size, c1)
    } // apply
...
{      
     ...
     *  that 'e != noDouble'.
     */
    def filterMissing: RleVectorD = filter (_ != noDouble)
