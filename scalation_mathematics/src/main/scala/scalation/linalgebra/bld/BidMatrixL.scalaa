      ...
      extends MatriL with Error with Serializable
{
    ...
    def set (u: MatriL)
    {
        if (u.isInstanceOf [BidMatrixL]) {
            val uu = u.asInstanceOf [BidMatrixL]
            for (i <- range_d) _dg(i) = uu._dg(i)
            for (j <- range_s) _sd(j) = uu._sd(j)
        } else {
            flaw ("set", "must convert u to a BidMatrixL first")
        } // if
    } // set
    ...
    {
        throw new UnsupportedOperationException ("BidMatrixL does not support sliceEx")
    } // sliceEx
