 ...
 */
 class RleMatrixI (val d1: Int, 
                   val d2: Int, 
                   private var v: Array [RleVectorI] = null,
...
{ 
    ...
    if (v == null) {
        v = Array.ofDim [RleVectorI] (d2)
        for (i <- 0 until v.length) {
            val v1 = new ReArray [TripletI] (1)
            v1(0) = new TripletI (0, d1, 0)
     ...
     */
    def copy (): RleMatrixI = new RleMatrixI (dim1, dim2, v.clone())
     ...
     */
    def zero (mm: Int, nn: Int): RleMatrixI = RleMatrixI (new MatrixI (mm, nn))
     ...
     */
    def apply (i: Int, j: Int): Int = v(j)(i)
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' matrix's vector at the 'i'-th index position ('i'-th row).
     *  @param i  the row index
     */
    def apply (i: Int): RleVectorI = { RleVectorI (for (j <- range2) yield  v(j)(i)) }
     ...
     */
    def apply (ir: Range, jr: Range): MatriI = slice (ir.start, ir.end, jr.start, jr.end)
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get the underlying 1D array for 'this' matrix.
     */
    def apply (): Array [RleVectorI] = v
     ...
     */
    def update (i: Int, j: Int, x: Int) { v(j)(i) = x }
     ...
     */
    def update (i: Int, u: VectoI) { for (j <- range2)  v(j)(i) = u(j) }
    ...
    {
        if (b.isInstanceOf [RleMatrixI]) {
            val bb = b.asInstanceOf [RleMatrixI]
            for (i <- ir; j <- jr) this(i, j) = bb(i, j)
        } else {
            flaw ("update","must convert b to RleMatrixI first")
        } // if
    } // update
    ...
    { 
        v = (for (j <- range2) yield RleVectorI (for (i <- range1) yield u(j)(i))).toArray
    } // set
    ...
    def set (u: MatriI)
    {
        if (u.isInstanceOf [RleMatrixI]) {
            val uu = u.asInstanceOf [RleMatrixI]
            for (i <- range1; j <- range2) v(i)(j) = uu.v(i)(j)
        } else {
            flaw ("set", "must convert u to a RleMatrixI first")
        } // if
    } // set
    ...
    { 
        RleMatrixI (for (i <- range2) yield RleVectorI (for (j <- from until end) yield v(i)(j)))       
    } // slice
    ...
    {
        if (from >= end) return new RleMatrixI (0, 0)
        RleMatrixI (for (i <- from until end) yield v(i))
    } // sliceCol
    ...
    {       
        RleMatrixI (for (i <- c_from until c_end) yield RleVectorI (for (j <- r_from until r_end) yield v(i)(j)))
    } // slice
     ...
     */
    def sliceEx (row: Int, col: Int): RleMatrixI =
    {
        RleMatrixI (for (i <- range2 if i != col) yield RleVectorI (for (j <- range1 if j != row) yield v(i)(j))) 
    } // sliceEx
    ...
    { 
        val c = new RleMatrixI (rowIndex.length, dim2)
     ...
     */
    def col (col: Int): RleVectorI =  v(col)
    ...
    { 
        v (col) = if (u.isInstanceOf [RleVectorI]) u.asInstanceOf [RleVectorI] 
                  else RleVectorI (u) 
    } // setCol
    ...
    {
        val c = new RleMatrixI (dim1, colIndex.length)
     ...
     */
    def t: RleMatrixI = RleMatrixI (for (i <- range1) yield this(i))
    ...
    {  
        ...
        RleMatrixI (for (i <- range2) yield 
            RleVectorI (for (j <- 0 until (dim1 + 1)) yield if (j == 0) u(i) else v(i)(j-1)))
    } // +:
    ...
    {
        val m = u match {
            case _: RleVectorI => u.asInstanceOf [RleVectorI] +: v
            case _             => RleVectorI (for (i <- u.range) yield u(i)) +: v
        } // match     
        new RleMatrixI (dim1, dim2 + 1, m) 
    } // +^:
    ...
    {  
        if (u.dim != dim2) flaw (":+", "vector does not match row dimension")
        RleMatrixI (for (i <- range2) yield 
            RleVectorI (for (j <- 0 until (dim1 + 1)) yield if (j < dim1) v(i)(j) else u(i)))
    } // :+
    ...
    {
        val m = u match {
            case _: RleVectorI => v :+ u.asInstanceOf [RleVectorI]
            case _             => v :+ RleVectorI (for (i <- u.range) yield u(i)) 
        } // match     
        new RleMatrixI (dim1, dim2 + 1, m) 
    } // :^+
    ...
    def ++^ (b: MatriI): RleMatrixI =
    {
        ...
        val m = b match {
        case _: RleMatrixI => v ++ b.asInstanceOf [RleMatrixI]()
        case _             => v ++ RleMatrixI (b.asInstanceOf [MatrixI])()
        } // match
        new RleMatrixI (dim1, dim2 + dim2, m)
    } // ++^
    ...
    def + (b: MatriI): RleMatrixI = 
    {
         val c = new RleMatrixI (dim1, dim2) 
         b match {
         case _: RleMatrixI => for (i <- range2) c.setCol (i, col(i) + b.col(i))                                                             
    ...
    {
        b match {
        case _: RleMatrixI => for (i <- range2) col(i) += b.col(i)
    ...
    def ~^ (p: Int): RleMatrixI = 
    {
        ...
        var t    = p
        var res  = new RleMatrixI (dim1, dim2)
        var temp = new RleMatrixI (dim1, dim2)
        for (i <- range1; j <- range2) {
            temp(i, j) = this(i, j)
            res(i, j)  = if (i == j) 1 else 0
        } // for
        while (t > 0) {
            if (t % 2 == 1) res *= temp
            temp *= temp
    ...
    def * (u: VectoI): VectoI = 
    {
        ...
        u match {
        case _: RleVectorI => mul2 (u.asInstanceOf [RleVectorI])
        case _             => mul (this, u)
        } // match
    } // *
    ...
    {
        b match {
        case _: RleMatrixI => t mdot b.asInstanceOf [RleMatrixI]
        case _             => toDense * b
        } // match
    } // *
    ...
    def *= (b: MatriI): MatriI = 
    {
        ...
        val temp = new RleMatrixI (b.dim1, b.dim2)
        for (i <- 0 until b.dim1; j <- 0 until b.dim2) temp(i, j) = b(i, j)
        for (i <- range1) {
            val row_i = new VectorI (dim2)
    ...
    def diag (p: Int,q: Int = 0): MatriI = 
    {
        ...
        var c = new RleMatrixI (p + dim1 + q, p + dim2 + q)
        var Ip = eye (p)
        var Iq = eye (q)
    ...
    {
        var c = new RleMatrixI (dim1 + b.dim1, dim2 + b.dim2)
        var x1 = new RleVectorI (b.dim2)
        for (i <- range1) c(i) = this(i) ++ x1
        val x2 = new RleVectorI (dim2)
    ...
    def dot (b: MatriI): RleVectorI = 
    {
        ...
        RleVectorI (for (j <- range2) yield  v(j) dot b.col(j))
    } // dot
    ...
    {
        if (dim1 != b.dim1) flaw ("dot", "incompatible")        
        RleVectorI (for (j <- range2) yield  v(j) dot b.v(j))
    } // dot
    ...
    {   
        ...
        val vv = Array.ofDim [RleVectorI] (d2)   
        for (j <- b.range2) vv(j) = RleVectorI (for (i <- range2) yield  v(i) dot b.col(j))
        new RleMatrixI (dim2, b.dim2, vv)
    } // mdot
    ...
    {   
        if (dim1 != b.dim1) flaw ("mdot", "incompatible first dimensions")
        val vv = Array.ofDim [RleVectorI] (d2)   
        for (j <- b.range2) vv(j) = RleVectorI (for (i <- range2) yield  v(i) dot b.v(j))
        new RleMatrixI (dim2, b.dim2, vv)
    } // mdot
    ...
    def inverse: MatriI = 
    {
        var b = new RleMatrixI (dim1, dim2)
        var u = eye (dim1)
    ...
    {
        var x = new RleMatrixI (dim1, dim2)
    ...
    {
        var x = RleMatrixI (new MatrixI (dim1, dim2))
     ...
     */
    private def partialPivoting (a: RleMatrixI, i: Int): Int =
    {
    ...
    {
        b.isInstanceOf [RleMatrixI] && (v.deep equals b.asInstanceOf [RleMatrixI].v.deep)
    } // equals
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `RleMatrixI` companion object provides operations for `RleMatrixI` that don't require
...
object RleMatrixI 
{
    ...
    /**
     */
    def apply (x: RleVectorI, xs: RleVectorI*): RleMatrixI = 
    {     
        new RleMatrixI (xs.length + 1, x.dim, (x +: xs).toArray)    
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def apply (x: Seq [RleVectorI]): RleMatrixI = 
    {
        new RleMatrixI (x(0).dim, x.length, x.toArray)
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def apply (u: Vector [VectoI]): RleMatrixI = 
    {
        val u_dim = u(0).dim
        val x = new RleMatrixI (u_dim, u.length)
    ...
    /**
     */
    def apply (x: MatrixI): RleMatrixI = 
    {
        RleMatrixI (for (j <- 0 until x.dim2) yield RleVectorI (x.col(j)))
    } // apply
    ...
    /**
     */
    def mul (at: RleMatrixI, u: VectoI): RleVectorI = 
    {
        RleVectorI (for (i <- 0 until at.dim1) yield at(i) dot u) 
    } // mul

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def mul (a: RleMatrixI, u: RleVectorI): RleVectorI = 
    {
        RleVectorI (for (i <- 0 until a.dim1) yield a(i) dot u)    
    } // mul
 ...
 *  > runMain scalation.linalgebra.RleMatrixITest
 */
object RleMatrixITest extends App
{
    for (l <- 1 to 4) {
        println ("\n\tTest RleMatrixI on real matrices of dim " + l)
        val x = new RleMatrixI (l, l)
        val y = new RleMatrixI (l, l)
        x.set (2)
        y.set (3)
        println ("x + y = " + (x + y))
        println ("x - y = " + (x - y))
    ...
    println ("\n\tTest RleMatrixI on additional operations")

    val z   = RleMatrixI (new MatrixI ((2, 2), 1, 2,
                                               3, 2))
    val t   = RleMatrixI (new MatrixI ((3, 3), 1, 2, 3,
                                               4, 3, 2,
                                               1, 3, 1))
    val zz  = RleMatrixI (new MatrixI ((3, 3), 3, 1, 0,
                                               1, 4, 2,
                                               0, 2, 5))
    ...
    val w = RleMatrixI (new MatrixI ((2, 3), 2,  3, 5, 
                                            -4,  2, 3))
    val v = RleMatrixI (new MatrixI ((3, 2), 2, -4, 
                                             3,  2,
                                             5,  3))
    println ("w         = " + w)
    println ("v         = " + v)
    ...
    val aa = RleMatrixI (new MatrixI ((3, 2), 1, 2,
                                              3, 4,
                                              5, 6))
    val bb = RleMatrixI (new MatrixI ((2, 2), 1, 2,
                                              3, 4))
    val cc = RleMatrixI (new MatrixI ((3, 2), 3, 4,
                                              5, 6,
                                              7, 8))
