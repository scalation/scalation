...
//  def filter (p: Real => Boolean): VectoR
     ...
     */
    def split (k: Int): Array [VectoR] = null
     ...
     *  @param basis  the index positions to be excluded
     */
    def selectEx (basis: VectoI): VectoR = select ((range diff basis()).toArray)
     ...
     */
    def reorder (rnk: VectoI): VectoR = VectorR (for (i <- range) yield this(rnk(i)))
