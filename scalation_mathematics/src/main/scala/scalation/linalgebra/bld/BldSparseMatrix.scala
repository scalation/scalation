...
"""; val code4 = raw"""
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `Sparse$MATRIX` into a `MatrixI`.
     */
    def toInt: MatrixI =
    {
     ...
     *  FIX - new builder
     */
    def toDense: $MATRIX = new $MATRIX (dim1, dim2, (for (i <- range1) yield this(i).toDense ().toArray).toArray)
...
"""; val code5 = raw"""
     ...
     */
    def setCol (col: Int, u: $VECTO) { for (i <- range1) this(i, col) = u(i) }
...
"""; val code12 = raw"""
    ...
    def lud_ip (): (Sparse$MATRIX, Sparse$MATRIX) =
    {
        ...
        for (i <- u.range1) {
            val pivot = u(i, i)
            if (pivot =~ $ZERO) flaw ("lud_ip", s"use Fac_LU since there is a zero pivot at row $i")
            l(i, i) = $ONE
            for (j <- i + 1 until u.dim2) l(i, j) = $ZERO
            for (k <- i + 1 until u.dim1) {
                val mul = u(k, i) / pivot
