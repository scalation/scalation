...
//    extends Traversable [Complex] with PartiallyOrdered [VectorC] with Vec with Error with Serializable
{
     ...
     */
    override def apply (iv: VectoI): VectorC =
    {
        val c = new VectorC (iv.dim)
     ...
     *  that 'e != noComplex'.
     */
    def filterMissing: VectorC = filter (_ != noComplex) 
     ...
     */
    override def split (k: Int): Array [VectoC] =
    {
        if (k <= 0) flaw ("split", "k must be a positive integer")
        val pieces = Array.ofDim [VectoC] (k)
        val size = dim / k
     ...
     */
    def + (s: (Int, Complex)): VectorC = { val c = copy (); c.v(s._1) += s._2; c }
     ...
     */
    def sum: Complex = v.sum

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Sum the elements of 'this' vector skipping the 'i'-th element (Not Equal 'i').
     *  @param i  the index of the element to skip
     */
    def sumNE (i: Int): Complex = v.sum - v(i)
     ...
     */
    def normalize: VectorC = this * (_1 / v.sum)
...
object VectorC
{
    ...
    def apply (x: String, xs: String*): VectorC =
    {
        val c = new VectorC (1 + xs.length)
        try c(0)  = Complex (x)
        catch { case _ : NumberFormatException | _ : ClassCastException => c.v(0) = noComplex }
        for (i <- 1 until c.dim) {
            try c.v(i) = Complex (xs(i-1))
            catch { case _ : NumberFormatException | _ : ClassCastException => c.v(i) = noComplex }
        } // for
        c
    } // apply
