...
class VectorT (val dim: Int,
     protected var v:   Array [TimeNum] = null)
      extends VectoT
//    extends Traversable [TimeNum] with PartiallyOrdered [VectorT] with Vec with Error with Serializable
{
    if (v == null) {
     ...
     */
    def this (u: VectoT) { this (u.dim); for (i <- range) v(i) = u(i) }
     ...
     */
    def this (iv: (Int, TimeNum), dm: Int) { this (dm); v(iv._1) = iv._2 }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create an exact copy of 'this' vector.
     */
    def copy (): VectorT = new VectorT (this)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a zero vector (all elements are zero) of length 'size'.
     *  @param size  the number of elements in the vector
     */
    def zero (size: Int = dim): VectorT = new VectorT (size)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a one vector (all elements are one) of length 'size'.
     *  @param size  the number of elements in the vector
     */
    def one (size: Int = dim): VectorT = new VectorT (size, Array.fill (size)(_1))
     ...
     */
    def oneAt (j: Int, size: Int = dim): VectorT = new VectorT ((j, _1), size)
     ...
     *  @param size  the size of the vector (upper bound = size - 1)
     */
    def _oneAt (j: Int, size: Int = dim): VectorT = new VectorT ((j, -_1), size)
     ...
     */
    def apply (): WrappedArray [TimeNum] = v

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' vector's elements that are given in the index vector.
     *  @param iv  the index vector
     */
    override def apply (iv: VectoI): VectorT =
    {
        val c = new VectorT (iv.dim)
     ...
     */
    def update (i: Int, x: TimeNum) { v(i) = x }
     ...
     *  @param x  the value to assign
     */
    def update (r: Range, x: TimeNum) { for (i <- r) v(i) = x }
     ...
     */
    def update (r: Range, u: VectoT) { for (i <- r) v(i) = u(i - r.start) }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Set each value in 'this' vector to 'x'.
     *  @param x  the value to be assigned
     */
    def set (x: TimeNum) { for (i <- range) v(i) = x }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Set the values in 'this' vector to the values in sequence/array 'u'.
     *  @param u  the sequence/array of values to be assigned
     */
    def set (u: Seq [TimeNum]) { for (i <- range) v(i) = u(i) }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Iterate over 'this' vector element by element.
     *  @param f  the function to apply
     */
    def foreach [U] (f: TimeNum => U) { var i = 0; while (i < dim) { f (v(i)); i += 1 } }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `VectorT` into a `VectorI`.
     */
    def toInt: VectorI = VectorI (v.map (_.toInt))

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `VectorT` into a `VectorL`.
      */
    def toLong: VectorL = VectorL (v.map (_.toLong))

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `VectorT` into a `VectorD`.
     */
    def toDouble: VectorD = VectorD (v.map (_.toDouble))

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `VectorT` into a dense `VectorT`.
     */
    def toDense: VectorT = this
     ...
     */
    override def filter (p: TimeNum => Boolean): VectorT = VectorT (v.filter (p))

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Filter out the missing elements of 'this' vector based on the predicate 
     *  that 'e != noTimeNum'.
     */
    def filterMissing: VectorT = filter (_ != noTimeNum) 
     ...
     *  @param f  the function to apply
     */
    def map (f: TimeNum => TimeNum): VectorT = VectorT (v.map (f))
     ...
     */
    override def slice (from: Int, till: Int = dim): VectorT = new VectorT (till - from, v.slice (from, till))
     ...
     */
    override def split (k: Int): Array [VectoT] =
    {
        if (k <= 0) flaw ("split", "k must be a positive integer")
        val pieces = Array.ofDim [VectoT] (k)
        val size = dim / k
     ...
     */
    def select (basis: Array [Int]): VectorT =
    {
     ...
     */
    def select (basis: VectoI): VectorT =
    {
        val c = new VectorT (basis.dim)
        for (i <- c.range) c.v(i) = v(basis(i))
        c
    } // select
     ...
     */
    def ++ (s: TimeNum): VectorT =
    {
        val c = new VectorT (dim + 1)
     ...
     */
    def + (s: (Int, TimeNum)): VectorT = { val c = copy (); c.v(s._1) += s._2; c }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Add in-place 'this' vector and vector 'b'.
     *  @param b  the vector to add (any kind)
     */
    def += (b: VectoT): VectorT = { for (i <- range) v(i) += b(i); this }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Add in-place 'this' vector and vector 'b'.
     *  @param b  the vector to add (same kind, more efficient)
     */
    def += (b: VectorT): VectorT = { for (i <- range) v(i) += b.v(i); this }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Add in-place 'this' vector and scalar 's'.
     *  @param s  the scalar to add
     */
    def += (s: TimeNum): VectorT = { for (i <- range) v(i) += s; this }
     ...
     *  @param s  the scalar exponent
     */
    def ~^= (s: TimeNum): VectorT = { for (i <- range) v(i) = v(i) ~^ s; this }
     ...
     */
    def sum: TimeNum = v.sum

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Sum the elements of 'this' vector skipping the 'i'-th element (Not Equal 'i').
     *  @param i  the index of the element to skip
     */
    def sumNE (i: Int): TimeNum = v.sum - v(i)
     ...
     */
    def normalize: VectorT = this * (_1 / v.sum)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Normalize 'this' vector so its length is one (unit vector).
     */
    def normalizeU: VectorT = this * (_1 / norm)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Normalize 'this' vector to have a maximum of one.
     */
    def normalize1: VectorT = this * (_1 / max ())
     ...
     */
    def indexWhere (p: (TimeNum) => Boolean): Int = v.indexWhere (p)
     ...
     */
    def clean (thres: Double = TOL, relative: Boolean = true): VectorT =
    {
...
//  {
//      var count = 1
//      val us = new VectorT (this); us.sort ()                // sorted vector
...
//  } // distinct
     ...
     */
    def distinct: VectorT = VectorT (v.distinct)
     ...
     */
    def contains (x: TimeNum): Boolean = v contains x

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Determine whether any of the elements in 'x' are contained in 'this' vector.
     *  @param x  the vector of elements to be checked
     */
    def containsAny (x: VectorT): Boolean = (v intersect x.v).length != 0

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Return the intersection of vectors 'this' and 'x'.
     *  @param x  the other vector
     */
    def intersect (x: VectorT): VectorT = VectorT (v intersect x.v)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Reverse the order of the elements in 'this' vector.
     */
    def reverse (): VectorT = new VectorT (dim, v.reverse)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Determine whether 'this' vector is in sorted (ascending) order.
     */
    def isSorted: Boolean = (new SortingT (v)).isSorted

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Sort 'this' vector in-place in ascending (non-decreasing) order.
     */
    def sort () { quickSort (v)(TimeO.ord) }
     ...
     */
    def tryCompareTo [B >: VectorT] (b: B)
        (implicit view_1: (B) => PartiallyOrdered [B]): Option [Int] =
    {
        ...
        for (i <- range) {
            val b_i = b.asInstanceOf [VectorT] (i)
    ...
    {
//      b.isInstanceOf [VectorT] && (v.deep equals b.asInstanceOf [VectorT].v.deep)

        if (! b.isInstanceOf [VectoT]) return false
        val bb = b.asInstanceOf [VectoT]
...
//          if (v(i) !=~ bb(i)) return false                            // stricter
    ...
    {
        var sb = new StringBuilder ("VectorT(")
...
object VectorT
{
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a `VectorT` from a sequence/array of `TimeNum`s.
     *  @param xs  the sequence/array of the `TimeNum` numbers
     */
    def apply (xs: Seq [TimeNum]): VectorT =
    {
        val c = new VectorT (xs.length)
    ...
    def apply (x: String, xs: String*): VectorT =
    {
        val c = new VectorT (1 + xs.length)
        try c(0)  = TimeNum (x)
        catch { case _ : NumberFormatException | _ : ClassCastException => c.v(0) = noTimeNum }
        for (i <- 1 until c.dim) {
            try c.v(i) = TimeNum (xs(i-1))
            catch { case _ : NumberFormatException | _ : ClassCastException => c.v(i) = noTimeNum }
        } // for
        c
    } // apply
     ...
     */
    def apply (xs: Array [String]): VectorT =
    {
        val c = new VectorT (xs.length)
        for (i <- c.range) {
            try c.v(i) = TimeNum (xs(i))
            catch { case _ : NumberFormatException | _ : ClassCastException => c.v(i) = noTimeNum }
        } // for
        c
    } // apply
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Create a `VectorT` from an array of Strings, skipping the first 'skip'
     ...
     */
    def apply (xs: Array [String], skip: Int): VectorT =
    {
        val c = new VectorT (xs.length - skip)
        for (i <- skip until xs.length) {
            c.v(i - skip) = if (xs(i) matches "\\s*") TimeNum (xs(i)) else xs(i).hashCode ()
        } // for
        c
    } // apply
     ...
     */
    def one (size: Int): VectorT = new VectorT (size, Array.fill (size)(_1))
     ...
     */
    def ++ (b: TimeNum, u: VectoT): VectorT =
    {
        val c = new VectorT (u.dim + 1)
     ...
     */
    def ++ (b: TimeNum, u: VectorT): VectorT =
    {
        val c = new VectorT (u.dim + 1)
     ...
     */
    def range (start: Int, end: Int): VectorT =
    {
        val c = new VectorT (end - start)
...
object VectorTTest extends App
{
    ...
    for (l <- 1 to 4) {
        println ("\n\tTest VectorT on vectors of dim " + l)
        x = new VectorT (l)
        y = new VectorT (l)
        x.set (2)
        y.set (3)
