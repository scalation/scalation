...
object SparseMatrixI
{
     ...
     */
    def apply (u: Array [VectoI], columnwise: Boolean = true): SparseMatrixI =
    {
        var x: SparseMatrixI = null
        val u_dim = u(0).dim
        if (columnwise) {
            x = new SparseMatrixI (u_dim, u.length)
            for (j <- 0 until u.length) x.setCol (j, u(j))    // assign column vectors
        } else {
            x = new SparseMatrixI (u.length, u_dim)
     ...
     */
    def apply (u: Vector [VectoI]): SparseMatrixI =
    {
        val u_dim = u(0).dim
        val x = new SparseMatrixI (u_dim, u.length)
        for (j <- 0 until u.length) x.setCol (j, u(j))        // assign column vectors
        x
    } // apply
     ...
     */
    def apply (fileName: String): SparseMatrixI =
    {
     ...
     */
    def eye (m: Int, n: Int = 0): SparseMatrixI =
    {
        val nn = if (n <= 0) m else n             // square matrix, if n <= 0
        val mn = if (m <= nn) m else nn           // length of main diagonal
        val c = new SparseMatrixI (m, nn)
        for (i <- 0 until mn) c(i, i) = 1
        c
    } // eye

} // SparseMatrixI object

import SparseMatrixI.eye

//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `SparseMatrixI` class stores and operates on Matrices of `Int`s.  Rather
 ...
 */
class SparseMatrixI (val d1: Int, 
                     val d2: Int)
      extends MatriI with Error with Serializable
{
     ...
     */
    def this (dim1: Int, dim2: Int, u: Array [SparseMatrixI.RowMap])
    {
        this (dim1, dim2)
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Construct a sparse matrix and assign values from `SymTriMatrixI` matrix 'b'.
     *  @param b  the matrix of values to assign
     */
//  def this (b: SymTriMatrixI)
//  {
//      this (b.d1, b.d1)
//      v(0)(0) = b.dg(0)
//      v(0)(1) = b.sd(0)
...
//  } // constructor
     ...
     */
    def copy (): SparseMatrixI = new SparseMatrixI (dim1, dim2, v.clone ())
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): SparseMatrixI = new SparseMatrixI (m, n)
     ...
     */
    def apply (i: Int, j: Int): Int = if (v(i) contains j) v(i)(j) else 0
     ...
     */
    def apply (ir: Range, jr: Range): SparseMatrixI = slice (ir.start, ir.end, jr.start, jr.end)
     ...
     *  @param x  the scalar value to assign
     */
    def update (i: Int, j: Int, x: Int) { if (! (x =~ 0)) v(i)(j) = x else v(i) -= j }
    ...
    {
        if (b.isInstanceOf [SparseMatrixI]) {
            val bb = b.asInstanceOf [SparseMatrixI]
            for (i <- ir; j <- jr) this(i, j) = b(i-ir.start, j-jr.start)
        } else {
            flaw ("update", "must convert b to a SparseMatrixI first")
        } // if
    } // update
    ...
    def set (u: MatriI)
    {
        if (u.isInstanceOf [SparseMatrixI]) {
            val uu = u.asInstanceOf [SparseMatrixI]
            for (i <- range1; j <- range2) this(i, j) = uu(i, j)
        } else {
            flaw ("set", "must convert u to a SparseMatrixI first")
        } // if
    } // set
     ...
     *  FIX - new builder
     */
    def toDense: MatrixI = new MatrixI (dim1, dim2, (for (i <- range1) yield this(i).toDense ().toArray).toArray)
     ...
     */
    def slice (from: Int, end: Int): SparseMatrixI =
    {
        val c = new SparseMatrixI (end - from, dim2)
     ...
     */
    def sliceCol (from: Int, end: Int): SparseMatrixI =
    {
        val c = new SparseMatrixI (dim1, end - from)
     ...
     */
    def slice (r_from: Int, r_end: Int, c_from: Int, c_end: Int): SparseMatrixI =
    {
        val c = new SparseMatrixI (r_end - r_from, c_end - c_from)
     ...
     */
    def sliceEx (row: Int, col: Int): SparseMatrixI =
    {
        val c = new SparseMatrixI (dim1 - 1, dim2 - 1)
     ...
     */
    def selectRows (rowIndex: Array [Int]): SparseMatrixI =
    {
        val c = new SparseMatrixI (rowIndex.length, dim2)
     ...
     */
    def setCol (col: Int, u: VectoI) { for (i <- range1) this(i, col) = u(i) }
     ...
     */
    def selectCols (colIndex: Array [Int]): SparseMatrixI =
    {
        val c = new SparseMatrixI (dim1, colIndex.length)
    ...
    def t: SparseMatrixI =
    {
        val b = new SparseMatrixI (dim2, dim1)
     ...
     */
    def +: (u: VectoI): SparseMatrixI =
    {
        if (u.dim != dim2) flaw ("+:", "vector does not match row dimension")
        val c = new SparseMatrixI (dim1 + 1, dim2)
    ...
    def +^: (u: VectoI): SparseMatrixI =
    {
        ...
        val c = new SparseMatrixI (dim1, dim2 + 1)
     ...
     */
    def ++ (b: MatriI): SparseMatrixI =
    {
        if (b.dim2 != dim2) flaw ("++", "matrix b does not match row dimension")
        val c = new SparseMatrixI (dim1 + b.dim1, dim2)
    ...
    def ++^ (b: MatriI): SparseMatrixI =
    {
        ...
        val c = new SparseMatrixI (dim1, dim2 + b.dim2)
     ...
     */
    def + (b: SparseMatrixI): SparseMatrixI =
    {
        val c = new SparseMatrixI (this)
        for (i <- range1; e <- b.v(i)) c(i, e._1) += e._2
        c
    } // +

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Add 'this' sparse matrix and matrix 'b'. 'b' may be any subtype of `MatriI`.
     *  Note, subtypes of `MatriI` should also implement a more efficient version,
     *  e.g., `def + (b: SparseMatrixI): SparseMatrixI`.
     *  @param b  the matrix to add (requires 'sameCrossDimensions')
     */
    def + (b: MatriI): SparseMatrixI =
    {
        val c = new SparseMatrixI (dim1, dim2)
    ...
    def * (b: SparseMatrixI): SparseMatrixI =
    {
        ...
        val c  = new SparseMatrixI (dim1, b.dim2)
        val bt = b.t                            // transpose the b matrix (for row access)
        for (i <- c.range1) {
            var ea: LinkedEntry [Int, Int] = null
            var eb: LinkedEntry [Int, Int] = null
            for (j <- c.range2) {
     ...
     */
    def times_s (b: SparseMatrixI): SparseMatrixI =
    {
        if (dim2 != b.dim1) flaw ("*", "matrix * matrix - incompatible cross dimensions")

        val c = new SparseMatrixI (dim1, dim1)  // allocate result matrix
    ...
    def ** (u: VectoI): SparseMatrixI =
    {
        ...
        val c  = new SparseMatrixI (dim1, dm)
     ...
     */
    private def getMaxVal (u: RowMap): Int =
    {
     ...
     *  @param u  the `SortedLinkHashMap` to be searched
     */
    private def getMinVal (u: RowMap): Int =
    {
        var x = if (u contains 0) u(0) else 0
        for (e <- u) if (e._2 < x) x = e._2
        x
     ...
     */
    def lowerT: SparseMatrixI =
    {
        val lo = new SparseMatrixI (dim1, dim2)
     ...
     */
    def upperT: SparseMatrixI =
    {
        val up = new SparseMatrixI (dim1, dim2)
     ...
     */
    def lud_npp: (SparseMatrixI, SparseMatrixI) =
    {
        if (! isSquare) throw new IllegalArgumentException ("lud_npp: requires a square matrix")

        val l = new SparseMatrixI (dim1)         // lower triangular matrix
        val u = new SparseMatrixI (this)         // upper triangular matrix (a copy of this)
        for (i <- u.range1) {
            val pivot = u(i, i)
            if (pivot =~ 0) flaw ("lud_npp", s"use Fac_LU since there is a zero pivot at row 0")
            l(i, i) = 1
     ...
     */
    def lud_ip (): (SparseMatrixI, SparseMatrixI) =
    {
        ...
        for (i <- u.range1) {
            val pivot = u(i, i)
            if (pivot =~ 0) flaw ("lud_ip", s"use Fac_LU since there is a zero pivot at row 0")
            l(i, i) = 1
            for (j <- i + 1 until u.dim2) l(i, j) = 0
            for (k <- i + 1 until u.dim1) {
                val mul = u(k, i) / pivot
     ...
     */
    private def partialPivoting (a: SparseMatrixI, i: Int): Int =
    {
     ...
     */
    private def swap (a: SparseMatrixI, i: Int, k: Int, col: Int)
    {
     ...
     */
    def diag (b: MatriI): SparseMatrixI =
    {
        val m = dim1 + b.dim1
        val n = dim2 + b.dim2
        val c = new SparseMatrixI (m, n)
     ...
     */
    def diag (p: Int, q: Int): SparseMatrixI =
    {
        if (! isSymmetric) flaw ("diag", "this matrix must be symmetric")

        val n  = dim1 + p + q
        val c  = new SparseMatrixI (n, n)
        for (i <- 0 until n; j <- 0 until n) {
            c(i, j) = if (i < p || i > p + dim1) if (i == j) 1 else 0
                    else                         this(i-p, j-p)
        } // for
        c
    } // diag
    ...
    {
        val c  = new VectorI (dim1 - math.abs (k))
...
*/
     ...
     */
    def clean (thres: Double, relative: Boolean = true): SparseMatrixI =
    {
 ...
 *  > runMain scalation.linalgebra.SparseMatrixITest
 */
object SparseMatrixITest extends App
{
    import SparseMatrixI.RowMap

    println ("\n\tTest SparseMatrixI operations")

    val z  = new SparseMatrixI ((2, 2), 1, 2,
                                        3, 2)
    val b  = VectorI (8, 7)
    val lu = z.lud_npp
    ...
    val w = new SparseMatrixI ((2, 3), 2, 3, 5, 
                                      -4, 2, 3)
    ...
    val sp = new SparseMatrixI (3, 3, Array (new RowMap ((1,   2), (2, 100)),
