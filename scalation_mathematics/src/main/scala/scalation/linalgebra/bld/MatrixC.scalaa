      ...
      extends MatriC with Error with Serializable
{
    ...
    def set (b: MatriC)
    {
        if (b.isInstanceOf [MatrixC]) {
            ...
            flaw ("set", "must convert b to a MatrixC first")
        } // if
    } // set
    ...
    {
        if (r_from >= r_end || c_from >= c_end) return new MatrixC (0, 0)
...
//  } // t
     ...
     */
    def times_ip_pre (b: MatrixC, d: Int = 0)
    {
        val d1_d = dim1 - d
        ...
        for (j <- d until dim2) {
            val col_j = new VectorC (d1_d)                 // save jth column so not overwritten
    ...
    def lud_npp: (MatrixC, MatrixC) =
    {
        ...
        val l = new MatrixC (dim1)          // lower triangular matrix
    ...
    {
        val y = new VectorC (l.dim2)                 // forward substitution
     ...
     */
    def setDiag (x: Complex) { for (i <- 0 until math.min (dim1, dim2)) v(i)(i) = x }
    ...
    /** Create a normalized version of 'this' matrix.
      */
    def normalizeU: MatrixC = MatrixC (for (j <- range2) yield col(j).normalizeU)
...
object MatrixC extends Error
{
     ...
     *  @param u    the repeated values
     */
    def apply (dim: (Int, Int), u: Complex*): MatrixC =
    {
        val x = new MatrixC (dim._1, dim._2)
        var k = 0
