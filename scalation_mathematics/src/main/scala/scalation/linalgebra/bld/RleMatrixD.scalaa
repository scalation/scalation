...
{ 
    ...
    def set (u: MatriD)
    {
        if (u.isInstanceOf [RleMatrixD]) {
            val uu = u.asInstanceOf [RleMatrixD]
            for (i <- range1; j <- range2) v(i)(j) = uu.v(i)(j)
        } else {
            flaw ("set", "must convert u to a RleMatrixD first")
        } // if
    } // set
    ...
    def reduce: RleMatrixD = 
    {
        ...
        for (i <- range1) {
            var x = b(i, i)
            if (x =~ 0.0) flaw ("reduce", "unable to find a non-zero pivot in row " + i)
