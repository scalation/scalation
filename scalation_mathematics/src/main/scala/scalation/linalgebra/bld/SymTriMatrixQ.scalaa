      ...
      extends MatriQ with Error with Serializable
{
    ...
    def set (u: MatriQ)
    {
        if (u.isInstanceOf [SymTriMatrixQ]) {
            val uu = u.asInstanceOf [SymTriMatrixQ]
            for (i <- range_d) _dg(i) = uu._dg(i)
            for (j <- range_s) _sd(j) = uu._sd(j)
        } else {
            flaw ("set", "must convert u to a SymTriMatrixQ first")
        } // if
    } // set
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixQ must be symmetric")
    } // slice
     ...
     */
    def sliceEx (row: Int, col: Int): SymTriMatrixQ =
    {
        throw new UnsupportedOperationException ("SymTriMatrixQ does not support sliceEx")
    } // sliceEx
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixQ does not support selectRows")
    } // selectRows
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixQ does not support selectCols")
    } // selectCols
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixQ does not support +:")
    } // +:
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixQ does not support + for VectoQ")
    } // +
    ...
    def * (b: MatriQ): SymTriMatrixQ = 
    {
        throw new UnsupportedOperationException ("SymTriMatrixQ does not support * with general matrices")
    } // *
     ...
     *  @param u  the vector to multiply by
     */
    def ** (u: VectoQ): MatrixQ = this * SymTriMatrixQ (u)
     ...
     *  @param u  the vector to multiply by
     */
    def **: (u: VectoQ): MatrixQ = SymTriMatrixQ (u) * this
     ...
     */
    def sumAbs: Rational = _dg.norm1 + _sd.norm1 + _sd.norm1
...
object SymTriMatrixQ extends Error
{
     ...
     */ 
    def apply (u: VectoQ): SymTriMatrixQ =
    {   
        val x = new SymTriMatrixQ (u.dim)
 ...
 *  > runMain scalation.linalgebra.SymTriMatrixQTest
 */
