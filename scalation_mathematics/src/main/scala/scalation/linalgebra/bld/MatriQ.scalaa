 ...
 *      `RleMatrixQ`        - compressed matrix - Run Length Encoding (RLE)
      ...
      extends Error
{
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): MatriQ
     ...
     *  @param j   the column index
     */
    def apply (ir: Range, j: Int): VectoQ = col(j)(ir)
     ...
     *  @param jr  the column range
     */
    def apply (i: Int, jr: Range): VectoQ = this(i)(jr)
     ...
     */
    def update (ir: Range, j: Int, u: VectoQ) { col(j)(ir) = u }
     ...
     *  @param u   the vector to assign
     */
    def update (i: Int, jr: Range, u: VectoQ) { this(i)(jr) = u }
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `MatriQ` into a `MatriI`.
     */
    def toInt: MatriI
     ...
     *  @param f  the function to apply
     */
    def map (f: VectoQ => VectoQ): MatriQ = ???
     ...
     */
    def slice (rg: Range): MatriQ = slice (rg.start, rg.end)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Slice 'this' matrix row-wise excluding the given range 'rg'.
     *  @param rg  the excluded range of the slice
     */
    def sliceEx (rg: Range): MatriQ = slice (0, rg.start) ++ slice (rg.end, dim1)
     ...
     */
    def sliceEx (row: Int, col: Int): MatriQ
     ...
     */
    def selectRowsEx (rowIndex: Array [Int]): MatriQ = selectRows ((range1 diff rowIndex).toArray)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Select all rows from 'this' matrix excluding the rows from the given 'rowIndex'.
     *  @param rowIndex  the row indices to exclude
     */
    def selectRowsEx (rowIndex: VectoI): MatriQ = selectRows ((range1 diff rowIndex()).toArray)
     ...
     */
    def col (col: Int, from: Int = 0): VectoQ
     ...
     *  @param b   the constant vector
     */
    def solve (lu: (MatriQ, MatriQ), b: VectoQ): VectoQ = solve (lu._1, lu._2, b)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Solve for 'x' in the equation 'a*x = b' where 'a' is 'this' matrix.
     *  @param b  the constant vector.
     */
    def solve (b: VectoQ): VectoQ
    ...
    def meanNZ: VectoQ =
    {
        ...
        for (j <- range2) {
            ...
            cm(j) = if (nzs > 0) col (j).sum / nzs.toRational else _0
        } // for
        cm
    } // meanNZ
    ...
    def meanR: VectoQ =
    {
        ...
        for (i <- range1) rm(i) = this(i).sum / dim2.toRational
        rm
    } // meanR
    ...
    def meanRNZ: VectoQ =
    {
        ...
        for (i <- range1) {
            ...
            rm(i) = if (nzs > 0) this(i).sum / nzs.toRational else _0
        } // for
        rm
    } // meanRNZ
     ...
     */
    def norm1: Rational = (for (j <- range2) yield col(j).norm1).max
     ...
     *  @see en.wikipedia.org/wiki/Matrix_norm
     */
    def normINF: Rational = (for (i <- range1) yield this(i).norm1).max
