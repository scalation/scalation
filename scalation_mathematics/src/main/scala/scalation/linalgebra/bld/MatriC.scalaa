 ...
 *      `RleMatrixC`        - compressed matrix - Run Length Encoding (RLE)
      ...
      extends Error
{
     ...
     *  @param rowIndex  the row indices to exclude
     */
    def selectRowsEx (rowIndex: VectoI): MatriC = selectRows ((range1 diff rowIndex()).toArray)
     ...
     *  @param b   the constant vector
     */
    def solve (lu: (MatriC, MatriC), b: VectoC): VectoC = solve (lu._1, lu._2, b)
    ...
    def meanNZ: VectoC =
    {
        ...
        for (j <- range2) {
            ...
            cm(j) = if (nzs > 0) col (j).sum / nzs.toComplex else _0
        } // for
        cm
    } // meanNZ
    ...
    def meanR: VectoC =
    {
        ...
        for (i <- range1) rm(i) = this(i).sum / dim2.toComplex
        rm
    } // meanR
    ...
    def meanRNZ: VectoC =
    {
        ...
        for (i <- range1) {
            ...
            rm(i) = if (nzs > 0) this(i).sum / nzs.toComplex else _0
        } // for
        rm
    } // meanRNZ
     ...
     *  @see en.wikipedia.org/wiki/Matrix_norm
     */
    def normINF: Complex = (for (i <- range1) yield this(i).norm1).max
