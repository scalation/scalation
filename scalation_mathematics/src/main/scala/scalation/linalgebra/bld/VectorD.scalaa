...
//    extends Traversable [Double] with PartiallyOrdered [VectorD] with Vec with Error with Serializable
{
     ...
     */
    override def apply (iv: VectoI): VectorD =
    {
        val c = new VectorD (iv.dim)
     ...
     *  that 'e != noDouble'.
     */
    def filterMissing: VectorD = filter (_ != noDouble) 
     ...
     */
    def + (s: (Int, Double)): VectorD = { val c = copy (); c.v(s._1) += s._2; c }
...
object VectorD
{
    ...
    def apply (x: String, xs: String*): VectorD =
    {
        val c = new VectorD (1 + xs.length)
        try c(0)  = x.toDouble
        catch { case _ : NumberFormatException | _ : ClassCastException => c.v(0) = noDouble }
        for (i <- 1 until c.dim) {
            try c.v(i) = xs(i-1).toDouble
            catch { case _ : NumberFormatException | _ : ClassCastException => c.v(i) = noDouble }
        } // for
        c
    } // apply
