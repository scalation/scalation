...
object SparseMatrixR
{
     ...
     */
    def apply (u: Array [VectoR], columnwise: Boolean = true): SparseMatrixR =
    {
        var x: SparseMatrixR = null
        val u_dim = u(0).dim
        if (columnwise) {
            x = new SparseMatrixR (u_dim, u.length)
            for (j <- 0 until u.length) x.setCol (j, u(j))    // assign column vectors
        } else {
            x = new SparseMatrixR (u.length, u_dim)
     ...
     */
    def apply (u: Vector [VectoR]): SparseMatrixR =
    {
        val u_dim = u(0).dim
        val x = new SparseMatrixR (u_dim, u.length)
        for (j <- 0 until u.length) x.setCol (j, u(j))        // assign column vectors
        x
    } // apply
     ...
     */
    def apply (fileName: String): SparseMatrixR =
    {
     ...
     */
    def eye (m: Int, n: Int = 0): SparseMatrixR =
    {
        val nn = if (n <= 0) m else n             // square matrix, if n <= 0
        val mn = if (m <= nn) m else nn           // length of main diagonal
        val c = new SparseMatrixR (m, nn)
        for (i <- 0 until mn) c(i, i) = _1
        c
    } // eye

} // SparseMatrixR object

import SparseMatrixR.eye

//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `SparseMatrixR` class stores and operates on Matrices of `Real`s.  Rather
 ...
 */
class SparseMatrixR (val d1: Int, 
                     val d2: Int)
      extends MatriR with Error with Serializable
{
     ...
     */
    def this (dim1: Int, dim2: Int, u: Array [SparseMatrixR.RowMap])
    {
        this (dim1, dim2)
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Construct a sparse matrix and assign values from dense matrix `MatrixR` 'b'.
     *  @param b  the matrix of values to assign
     */
    def this (b: MatrixR)
    {
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Construct a sparse matrix and assign values from `SymTriMatrixR` matrix 'b'.
     *  @param b  the matrix of values to assign
     */
//  def this (b: SymTriMatrixR)
//  {
//      this (b.d1, b.d1)
//      v(0)(0) = b.dg(0)
//      v(0)(1) = b.sd(0)
...
//  } // constructor
     ...
     */
    def copy (): SparseMatrixR = new SparseMatrixR (dim1, dim2, v.clone ())
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): SparseMatrixR = new SparseMatrixR (m, n)
     ...
     */
    def apply (i: Int, j: Int): Real = if (v(i) contains j) v(i)(j) else _0
     ...
     */
    def apply (ir: Range, jr: Range): SparseMatrixR = slice (ir.start, ir.end, jr.start, jr.end)
     ...
     *  @param x  the scalar value to assign
     */
    def update (i: Int, j: Int, x: Real) { if (! (x =~ _0)) v(i)(j) = x else v(i) -= j }
    ...
    {
        if (b.isInstanceOf [SparseMatrixR]) {
            val bb = b.asInstanceOf [SparseMatrixR]
            for (i <- ir; j <- jr) this(i, j) = b(i-ir.start, j-jr.start)
        } else {
            flaw ("update", "must convert b to a SparseMatrixR first")
        } // if
    } // update
    ...
    def set (u: MatriR)
    {
        if (u.isInstanceOf [SparseMatrixR]) {
            val uu = u.asInstanceOf [SparseMatrixR]
            for (i <- range1; j <- range2) this(i, j) = uu(i, j)
        } else {
            flaw ("set", "must convert u to a SparseMatrixR first")
        } // if
    } // set
     ...
     *  FIX - new builder
     */
    def toDense: MatrixR = new MatrixR (dim1, dim2, (for (i <- range1) yield this(i).toDense ().toArray).toArray)
    ...
    {
        val c = new SparseMatrixR (end - from, dim2)
    ...
    {
        val c = new SparseMatrixR (dim1, end - from)
    ...
    {
        val c = new SparseMatrixR (r_end - r_from, c_end - c_from)
    ...
    {
        val c = new SparseMatrixR (rowIndex.length, dim2)
    ...
    {
        val c = new SparseMatrixR (dim1, colIndex.length)
    ...
    def t: SparseMatrixR =
    {
        val b = new SparseMatrixR (dim2, dim1)
    ...
    def +: (u: VectoR): SparseMatrixR =
    {
        ...
        val c = new SparseMatrixR (dim1 + 1, dim2)
    ...
    def +^: (u: VectoR): SparseMatrixR =
    {
        ...
        val c = new SparseMatrixR (dim1, dim2 + 1)
    ...
    def ++ (b: MatriR): SparseMatrixR =
    {
        ...
        val c = new SparseMatrixR (dim1 + b.dim1, dim2)
    ...
    def ++^ (b: MatriR): SparseMatrixR =
    {
        ...
        val c = new SparseMatrixR (dim1, dim2 + b.dim2)
    ...
    {
        val c = new SparseMatrixR (this)
        for (i <- range1; e <- b.v(i)) c(i, e._1) += e._2
        c
    } // +

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Add 'this' sparse matrix and matrix 'b'. 'b' may be any subtype of `MatriR`.
     *  Note, subtypes of `MatriR` should also implement a more efficient version,
     *  e.g., `def + (b: SparseMatrixR): SparseMatrixR`.
     *  @param b  the matrix to add (requires 'sameCrossDimensions')
     */
    def + (b: MatriR): SparseMatrixR =
    {
        val c = new SparseMatrixR (dim1, dim2)
    ...
    def * (b: SparseMatrixR): SparseMatrixR =
    {
        ...
        val c  = new SparseMatrixR (dim1, b.dim2)
        val bt = b.t                            // transpose the b matrix (for row access)
        for (i <- c.range1) {
            var ea: LinkedEntry [Int, Real] = null
            var eb: LinkedEntry [Int, Real] = null
            for (j <- c.range2) {
    ...
    def mdot (b: MatriR): SparseMatrixR =
    {
        ...
        val c = new SparseMatrixR (dim2, b.dim2)
        val at = this.t                         // transpose the 'this' matrix
        for (i <- range2) {
    ...
    def times_s (b: SparseMatrixR): SparseMatrixR =
    {
        ...
        val c = new SparseMatrixR (dim1, dim1)  // allocate result matrix
    ...
    def ** (u: VectoR): SparseMatrixR =
    {
        ...
        val c  = new SparseMatrixR (dim1, dm)
     ...
     */
    private def getMaxVal (u: RowMap): Real =
    {
     ...
     *  @param u  the `SortedLinkHashMap` to be searched
     */
    private def getMinVal (u: RowMap): Real =
    {
        var x = if (u contains 0) u(0) else _0
        for (e <- u) if (e._2 < x) x = e._2
        x
    ...
    {
        val lo = new SparseMatrixR (dim1, dim2)
    ...
    {
        val up = new SparseMatrixR (dim1, dim2)
    ...
    def lud_npp: (SparseMatrixR, SparseMatrixR) =
    {
        ...
        val l = new SparseMatrixR (dim1)         // lower triangular matrix
        val u = new SparseMatrixR (this)         // upper triangular matrix (a copy of this)
        for (i <- u.range1) {
            val pivot = u(i, i)
            if (pivot =~ _0) flaw ("lud_npp", s"use Fac_LU since there is a zero pivot at row 3")
            l(i, i) = _1
    ...
    def lud_ip (): (SparseMatrixR, SparseMatrixR) =
    {
        ...
        for (i <- u.range1) {
            val pivot = u(i, i)
            if (pivot =~ _0) flaw ("lud_ip", s"use Fac_LU since there is a zero pivot at row 3")
            l(i, i) = _1
            for (j <- i + 1 until u.dim2) l(i, j) = _0
            for (k <- i + 1 until u.dim1) {
                val mul = u(k, i) / pivot
     ...
     */
    private def partialPivoting (a: SparseMatrixR, i: Int): Int =
    {
     ...
     */
    private def swap (a: SparseMatrixR, i: Int, k: Int, col: Int)
    {
    ...
    {
        val c  = new VectorR (dim1 - math.abs (k))
     ...
     */
    def setDiag (x: Real) { for (i <- range1) this(i, i) = x }
...
/*
        val b = new SparseMatrixR (this)           // copy this matrix into b
 ...
 *  > runMain scalation.linalgebra.SparseMatrixRTest
 */
object SparseMatrixRTest extends App
{
    import SparseMatrixR.RowMap

    println ("\n\tTest SparseMatrixR operations")

    val z  = new SparseMatrixR ((2, 2), 1, 2,
                                        3, 2)
    val b  = VectorR (8, 7)
    val lu = z.lud_npp
    ...
    val w = new SparseMatrixR ((2, 3), 2, 3, 5, 
                                      -4, 2, 3)
    ...
    val sp = new SparseMatrixR (3, 3, Array (new RowMap ((1,   2), (2, 100)),
