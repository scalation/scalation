      ...
      extends MatriC with Error with Serializable
{
     ...
     */
    def copy (): SymTriMatrixC = new SymTriMatrixC (dim1)             // FIX - copy the diagonals
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): SymTriMatrixC = new SymTriMatrixC (m)
    ...
    def apply (i: Int, j: Int): Complex = 
    {
        ...
        else throw new Exception ("SymTriMatrixC.apply: element not on tridiagonal")
    } // apply
    ...
    def update (ir: Range, jr: Range, b: MatriC)
    {
        ...
        if (b.isInstanceOf [SymTriMatrixC]) {
            val bb = b.asInstanceOf [SymTriMatrixC]
            for (i <- ir) {
                _dg(i) = bb.dg(i - ir.start)
                if (i > ir.start) _sd(i-1) = bb.sd(i - ir.start - 1)
            } // for
        } else {
            flaw ("update", "must convert b to a SymTriMatrixC first")
        } // if
    } // update
    ...
    def set (u: MatriC)
    {
        if (u.isInstanceOf [SymTriMatrixC]) {
            val uu = u.asInstanceOf [SymTriMatrixC]
            for (i <- range_d) _dg(i) = uu._dg(i)
            for (j <- range_s) _sd(j) = uu._sd(j)
        } else {
            flaw ("set", "must convert u to a SymTriMatrixC first")
        } // if
    } // set
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `SymTriMatrixC` into a SymTriMatrixI`.
     */
     ...
     */
    def sliceCol (from: Int, end: Int): SymTriMatrixC =
    {
        val c = new SymTriMatrixC (end - from)
        for (j <- c.range1) {
            c._dg(j) = _dg(j + from)
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixC must be symmetric")
    } // slice
     ...
     */
    def sliceEx (row: Int, col: Int): SymTriMatrixC =
    {
        throw new UnsupportedOperationException ("SymTriMatrixC does not support sliceEx")
    } // sliceEx
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixC does not support selectRows")
    } // selectRows
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixC does not support selectCols")
    } // selectCols
     ...
     */
    def +: (u: VectoC): SymTriMatrixC =
    {
        throw new UnsupportedOperationException ("SymTriMatrixC does not support +:")
    } // +:
    ...
    {
        throw new UnsupportedOperationException ("SymTriMatrixC does not support + for VectoC")
    } // +
    ...
    def * (b: MatriC): SymTriMatrixC = 
    {
        throw new UnsupportedOperationException ("SymTriMatrixC does not support * with general matrices")
    } // *

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Multiply 'this' tridiagonal matrix by matrix 'b'.  Requires 'b' to have
     *  type `SymTriMatrixC`, but returns a more general type of matrix.
     *  @param b  the matrix to multiply by
     */
    ...
    {
        if (dim1 != b.dim1) flaw ("dot", "matrix dot matrix - incompatible first dimensions")
        if (dim1 == 1) return VectorC (_dg(0) * b(0, 0))
     ...
     *  @param b  the second matrix of the dot product
     */
    def mdot (b: SymTriMatrixC): MatrixC = 
    {
     ...
     *  @param u  the vector to multiply by
     */
    def ** (u: VectoC): MatrixC = this * SymTriMatrixC (u)
     ...
     *  @param u  the vector to multiply by
     */
    def **: (u: VectoC): MatrixC = SymTriMatrixC (u) * this
     ...
     */
    def lud_npp: (MatriC, MatriC) =
    {
        val l  = eye (d1)                  // lower triangular matrix
        val u  = new MatrixC (d1, d1)      // upper triangular matrix
        val ls = new VectorC (n)           // subdiagonal of l
        val ud = new VectorC (d1)          // diagonal of u
    ...
    {
        if (! l.isInstanceOf [MatrixC] || ! u.isInstanceOf [MatrixC]) {
    ...
    {
        val j = d1 - 2
        val x = new VectorC (b)               // solution vector, start with copy of b
        val c = _sd                           // subdiagonal
        val d = _dg                           // diagonal
        val e = new VectorC (_sd ++ _0)      // augmented super-diagonal
    ...
    def inverse: MatriC =
    {
        ...
        val a = new VectorC (d1)              // alpha
        val g = new VectorC (d1)              // gamma
        val t = new VectorC (d1)              // tau
 ...
 */
object SymTriMatrixC extends Error
{
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
     ...
     */
    def apply (u: Array [VectoC], columnwise: Boolean = true): SymTriMatrixC =
    {
        var x: SymTriMatrixC = null
        val u_dim = u(0).dim
        if (u_dim != u.length) flaw ("apply", "symmetric matrices must be square")
        if (columnwise) {
            x = new SymTriMatrixC (u_dim)
     ...
     */
    def apply (u: Vector [VectoC]): SymTriMatrixC =
    {
        val u_dim = u(0).dim
        val x = new SymTriMatrixC (u_dim)
     ...
     */ 
    def apply (u: VectoC): SymTriMatrixC =
    {   
        val x = new SymTriMatrixC (u.dim)
     ...
     */
    def apply (fileName: String): SymTriMatrixC =
    {
     ...
     */
    def eye (m: Int): SymTriMatrixC =
    {
        val c = new SymTriMatrixC (m)
        for (i <- 0 until m) c(i, i) = _1
        c
    } // eye

} // SymTriMatrixC object



//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `SymTriMatrixCTest` object is used to test the `SymTriMatrixC` class.
 *  > runMain scalation.linalgebra.SymTriMatrixCTest
 */
object SymTriMatrixCTest extends App
{
    val a = new SymTriMatrixC (VectorC (3, 4, 5),
                               VectorC (1, 2))
