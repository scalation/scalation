 ...
 *  classes implementing `Int` matrices, i.e.,
 *      `MatrixI`           - dense matrix
 *      `BidMatrixI`        - bidiagonal matrix - useful for computing Singular Values
 *      `RleMatrixI`        - compressed matrix - Run Length Encoding (RLE)
 *      `SparseMatrixI`     - sparse matrix - majority of elements should be zero
 *      `SymTriMatrixI`     - symmetric triangular matrix - useful for computing Eigenvalues
 *      `par.MatrixI`       - parallel dense matrix
 *      `par.SparseMatrixI` - parallel sparse matrix
      ...
      extends Error
{
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): MatriI
     ...
     *  @param j   the column index
     */
    def apply (ir: Range, j: Int): VectoI = col(j)(ir)
     ...
     *  @param jr  the column range
     */
    def apply (i: Int, jr: Range): VectoI = this(i)(jr)
     ...
     */
    def update (ir: Range, j: Int, u: VectoI) { col(j)(ir) = u }
     ...
     *  @param u   the vector to assign
     */
    def update (i: Int, jr: Range, u: VectoI) { this(i)(jr) = u }
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `MatriI` into a `MatriI`.
     */
    def toInt: MatriI
     ...
     *  @param f  the function to apply
     */
    def map (f: VectoI => VectoI): MatriI = ???
     ...
     */
    def slice (rg: Range): MatriI = slice (rg.start, rg.end)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Slice 'this' matrix row-wise excluding the given range 'rg'.
     *  @param rg  the excluded range of the slice
     */
    def sliceEx (rg: Range): MatriI = slice (0, rg.start) ++ slice (rg.end, dim1)
     ...
     */
    def sliceEx (row: Int, col: Int): MatriI
     ...
     */
    def sliceCol (from: Int, end: Int): MatriI
     ...
     */
    def selectRowsEx (rowIndex: Array [Int]): MatriI = selectRows ((range1 diff rowIndex).toArray)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Select all rows from 'this' matrix excluding the rows from the given 'rowIndex'.
     *  @param rowIndex  the row indices to exclude
     */
    def selectRowsEx (rowIndex: VectoI): MatriI = selectRows ((range1 diff rowIndex()).toArray)
     ...
     */
    def col (col: Int, from: Int = 0): VectoI
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** From 'this' matrix subtract in-place matrix 'b' for any type extending `MatriI`.
     *  Note, subtypes of `MatriI` should also implement a more efficient version,
     *  e.g., `def -= (b: MatrixI): MatrixI`.
     *  @param b  the matrix to subtract (requires 'leDimensions')
     */
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Multiply 'this' matrix and matrix 'b' for any type extending `MatriI`.
     *  Note, subtypes of `MatriI` should also implement a more efficient version,
     *  e.g., `def * (b: MatrixI): MatrixI`.
     *  @param b  the matrix to add (requires 'leDimensions')
     */
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Multiply in-place 'this' matrix and matrix 'b' for any type extending `MatriI`.
     *  Note, subtypes of `MatriI` should also implement a more efficient version,
     *  e.g., `def *= (b: MatrixI): MatrixI`.
     ...
     *  @param b   the constant vector
     */
    def solve (lu: (MatriI, MatriI), b: VectoI): VectoI = solve (lu._1, lu._2, b)
    ...
    def meanNZ: VectoI =
    {
        ...
        for (j <- range2) {
            ...
            cm(j) = if (nzs > 0) col (j).sum / nzs.toInt else 0
        } // for
        cm
    } // meanNZ
    ...
    def meanR: VectoI =
    {
        ...
        for (i <- range1) rm(i) = this(i).sum / dim2.toInt
        rm
    } // meanR
    ...
    def meanRNZ: VectoI =
    {
        ...
        for (i <- range1) {
            ...
            rm(i) = if (nzs > 0) this(i).sum / nzs.toInt else 0
        } // for
        rm
    } // meanRNZ
     ...
     */
    def norm1: Int = (for (j <- range2) yield col(j).norm1).max
     ...
     *  @see en.wikipedia.org/wiki/Matrix_norm
     */
    def normINF: Int = (for (i <- range1) yield this(i).norm1).max
