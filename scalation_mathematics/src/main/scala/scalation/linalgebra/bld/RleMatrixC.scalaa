...
{ 
    ...
    def set (u: MatriC)
    {
        if (u.isInstanceOf [RleMatrixC]) {
            val uu = u.asInstanceOf [RleMatrixC]
            for (i <- range1; j <- range2) v(i)(j) = uu.v(i)(j)
        } else {
            flaw ("set", "must convert u to a RleMatrixC first")
        } // if
    } // set
    ...
    def lud_npp: (RleMatrixC, RleMatrixC) = 
    {
        ...
        val l = new RleMatrixC (dim1)
        val u = new RleMatrixC (dim1)
 ...
 *  > runMain scalation.linalgebra.RleMatrixCTest
 */
