...
//    extends Traversable [Rational] with PartiallyOrdered [VectorQ] with Vec with Error with Serializable
{
     ...
     */
    def this (iv: (Int, Rational), dm: Int) { this (dm); v(iv._1) = iv._2 }
     ...
     */
    override def apply (iv: VectoI): VectorQ =
    {
        val c = new VectorQ (iv.dim)
     ...
     *  that 'e != noRational'.
     */
    def filterMissing: VectorQ = filter (_ != noRational) 
     ...
     */
    override def split (k: Int): Array [VectoQ] =
    {
        if (k <= 0) flaw ("split", "k must be a positive integer")
        val pieces = Array.ofDim [VectoQ] (k)
        val size = dim / k
     ...
     */
    def + (s: (Int, Rational)): VectorQ = { val c = copy (); c.v(s._1) += s._2; c }
     ...
     */
    def sum: Rational = v.sum

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Sum the elements of 'this' vector skipping the 'i'-th element (Not Equal 'i').
     *  @param i  the index of the element to skip
     */
    def sumNE (i: Int): Rational = v.sum - v(i)
     ...
     */
    def normalize: VectorQ = this * (_1 / v.sum)
...
object VectorQ
{
    ...
    def apply (x: String, xs: String*): VectorQ =
    {
        val c = new VectorQ (1 + xs.length)
        try c(0)  = Rational (x)
        catch { case _ : NumberFormatException | _ : ClassCastException => c.v(0) = noRational }
        for (i <- 1 until c.dim) {
            try c.v(i) = Rational (xs(i-1))
            catch { case _ : NumberFormatException | _ : ClassCastException => c.v(i) = noRational }
        } // for
        c
    } // apply
