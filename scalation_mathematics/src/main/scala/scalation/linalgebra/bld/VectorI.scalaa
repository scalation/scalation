...
import scalation.math.{int_exp, noInt}
...
//    extends Traversable [Int] with PartiallyOrdered [VectorI] with Vec with Error with Serializable
{
     ...
     */
    override def apply (iv: VectoI): VectorI =
    {
        val c = new VectorI (iv.dim)
     ...
     *  that 'e != noInt'.
     */
    def filterMissing: VectorI = filter (_ != noInt) 
     ...
     */
    override def split (k: Int): Array [VectoI] =
    {
        if (k <= 0) flaw ("split", "k must be a positive integer")
        val pieces = Array.ofDim [VectoI] (k)
        val size = dim / k
...
object VectorI
{
    ...
    def apply (x: String, xs: String*): VectorI =
    {
        val c = new VectorI (1 + xs.length)
        try c(0)  = x.toInt
        catch { case _ : NumberFormatException | _ : ClassCastException => c.v(0) = noInt }
        for (i <- 1 until c.dim) {
            try c.v(i) = xs(i-1).toInt
            catch { case _ : NumberFormatException | _ : ClassCastException => c.v(i) = noInt }
        } // for
        c
    } // apply
