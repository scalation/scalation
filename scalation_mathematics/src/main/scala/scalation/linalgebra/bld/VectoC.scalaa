...
//  def filter (p: Complex => Boolean): VectoC
     ...
     *  @param basis  the index positions to be excluded
     */
    def selectEx (basis: VectoI): VectoC = select ((range diff basis()).toArray)
     ...
     */
    def reorder (rnk: VectoI): VectoC = VectorC (for (i <- range) yield this(rnk(i)))
