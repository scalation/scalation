...
object RleVectorL 
{
    ...
    def apply (x: Long, xs: Long*): RleVectorL =
    {
        ...
        var count = 1
        if (xs.size == 0) {
            c1(idx) = new TripletL (x, 1, 0)
            return new RleVectorL (xs.size + 1, c1)
        } // if
        if (xs(0) != x) {
     ...
     */
    def apply (xs: Array [String]): RleVectorL =
    {
        if (xs.length == 0) return new RleVectorL (0)
        var c1    = new ReArray [TripletL] ()
        var idx   = 0
        ...
        for (i <- 1 until xs.size) {
            if (xs(i) != xs(i-1)) {
                ...
                c1(idx) = new TripletL (z, count, i - count); idx += 1
                count = 1
            } else count += 1
        } // for
        var z = noLong
        try z = xs(xs.size-1).toLong
        catch { case ex: NumberFormatException => }
        c1(idx) = new TripletL (z, count, xs.size - count)
        new RleVectorL (xs.size, c1)
    } // apply
...
{      
     ...
     *  that 'e != noLong'.
     */
    def filterMissing: RleVectorL = filter (_ != noLong)
