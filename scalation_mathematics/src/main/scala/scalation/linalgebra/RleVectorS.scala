...
object RleVectorS 
{
    ...
    def apply (x: StrNum, xs: StrNum*): RleVectorS =
    {
        ...
        var count = 1
        if (xs.size == 0) {
            c1(idx) = new TripletS (x, 1, 0)
            return new RleVectorS (xs.size + 1, c1)
        } // if
        if (xs(0) != x) {
     ...
     */
    def apply (xs: Array [String]): RleVectorS =
    {
        if (xs.length == 0) return new RleVectorS (0)
        var c1    = new ReArray [TripletS] ()
        var idx   = 0
        ...
        for (i <- 1 until xs.size) {
            if (xs(i) != xs(i-1)) {
                ...
                c1(idx) = new TripletS (z, count, i - count); idx += 1
                count = 1
            } else count += 1
        } // for
        var z = noStrNum
        try z = StrNum (xs(xs.size-1))
        catch { case ex: NumberFormatException => }
        c1(idx) = new TripletS (z, count, xs.size - count)
        new RleVectorS (xs.size, c1)
    } // apply
...
{      
     ...
     *  that 'e != noStrNum'.
     */
    def filterMissing: RleVectorS = filter (_ != noStrNum)
