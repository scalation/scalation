...
//  def filter (p: Double => Boolean): VectoD
     ...
     */
    def slice (rg: Range): VectoD = slice (rg.start, rg.end)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Slice 'this' vector excluding the given range 'rg'.
     *  @param rg  the excluded range of the slice
     */
    def sliceEx (rg: Range): VectoD = slice (0, rg.start) ++ slice (rg.end, dim)
     ...
     */
    def split (k: Int): Array [VectoD] = null
     ...
     */
    def selectEx (basis: Array [Int]): VectoD = select ((range diff basis).toArray)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Select all elements of 'this' vector excluding ones in the 'basis'.
     *  @param basis  the index positions to be excluded
     */
    def selectEx (basis: VectoI): VectoD = select ((range diff basis()).toArray)
     ...
     */
    def reorder (rnk: VectoI): VectoD = VectorD (for (i <- range) yield this(rnk(i)))
