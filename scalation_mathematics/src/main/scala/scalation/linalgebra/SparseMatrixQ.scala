...
//  } // constructor
    ...
    def set (u: MatriQ)
    {
        if (u.isInstanceOf [SparseMatrixQ]) {
            val uu = u.asInstanceOf [SparseMatrixQ]
            for (i <- range1; j <- range2) this(i, j) = uu(i, j)
        } else {
            flaw ("set", "must convert u to a SparseMatrixQ first")
        } // if
    } // set
     ...
     */
    def sliceEx (row: Int, col: Int): SparseMatrixQ =
    {
