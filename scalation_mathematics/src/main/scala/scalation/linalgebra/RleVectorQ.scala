...
object RleVectorQ 
{
    ...
    def apply (x: Rational, xs: Rational*): RleVectorQ =
    {
        ...
        var count = 1
        if (xs.size == 0) {
            c1(idx) = new TripletQ (x, 1, 0)
            return new RleVectorQ (xs.size + 1, c1)
        } // if
        if (xs(0) != x) {
     ...
     */
    def apply (xs: Array [String]): RleVectorQ =
    {
        if (xs.length == 0) return new RleVectorQ (0)
        var c1    = new ReArray [TripletQ] ()
        var idx   = 0
        ...
        for (i <- 1 until xs.size) {
            if (xs(i) != xs(i-1)) {
                var z = noRational
                try z = Rational (xs(i-1))
                catch { case ex: NumberFormatException => }
                c1(idx) = new TripletQ (z, count, i - count); idx += 1
                count = 1
            } else count += 1
        } // for
        var z = noRational
        try z = Rational (xs(xs.size-1))
        catch { case ex: NumberFormatException => }
        c1(idx) = new TripletQ (z, count, xs.size - count)
        new RleVectorQ (xs.size, c1)
    } // apply
 ...
 */   
class RleVectorQ (val dim: Int, protected var v: ReArray [TripletQ] = null)  
      extends VectoQ
{      
    if (v == null) {
     ...
     *  that 'e != noRational'.
     */
    def filterMissing: RleVectorQ = filter (_ != noRational)
