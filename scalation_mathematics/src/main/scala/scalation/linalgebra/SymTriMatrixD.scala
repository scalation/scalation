      ...
      extends MatriD with Error with Serializable
{
    ...
    def set (u: MatriD)
    {
        if (u.isInstanceOf [SymTriMatrixD]) {
            val uu = u.asInstanceOf [SymTriMatrixD]
            for (i <- range_d) _dg(i) = uu._dg(i)
            for (j <- range_s) _sd(j) = uu._sd(j)
        } else {
            flaw ("set", "must convert u to a SymTriMatrixD first")
        } // if
    } // set
     ...
     */
    def sliceEx (row: Int, col: Int): SymTriMatrixD =
    {
        throw new UnsupportedOperationException ("SymTriMatrixD does not support sliceEx")
    } // sliceEx
     ...
     */
    def dot (b: SymTriMatrixD): VectorD = 
    {
        if (dim1 != b.dim1) flaw ("dot", "matrix dot matrix - incompatible first dimensions")
        if (dim1 == 1) return VectorD (_dg(0) * b._dg(0))
        
        var c = new VectorD (dim2)
    ...
    {
        if (dim1 != b.dim1) flaw ("dot", "matrix dot matrix - incompatible first dimensions")
        if (dim1 == 1) return VectorD (_dg(0) * b(0, 0))
     ...
     *  @param b  the second matrix of the dot product
     */
    def mdot (b: SymTriMatrixD): MatrixD = 
    {
     ...
     *  @param u  the vector to multiply by
     */
    def ** (u: VectoD): MatrixD = this * SymTriMatrixD (u)
     ...
     *  @param u  the vector to multiply by
     */
    def **: (u: VectoD): MatrixD = SymTriMatrixD (u) * this
    ...
    def solve (b: VectoD): VectoD =
    {
        ...
        val d = _dg                           // diagonal
        val e = new VectorD (_sd ++ 0.0)      // augmented super-diagonal
...
object SymTriMatrixD extends Error
{
     ...
     */ 
    def apply (u: VectoD): SymTriMatrixD =
    {   
        val x = new SymTriMatrixD (u.dim)
 ...
 *  > runMain scalation.linalgebra.SymTriMatrixDTest
 */
