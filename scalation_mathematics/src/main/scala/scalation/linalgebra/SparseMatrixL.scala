...
//  } // constructor
    ...
    def set (u: MatriL)
    {
        if (u.isInstanceOf [SparseMatrixL]) {
            val uu = u.asInstanceOf [SparseMatrixL]
            for (i <- range1; j <- range2) this(i, j) = uu(i, j)
        } else {
            flaw ("set", "must convert u to a SparseMatrixL first")
        } // if
    } // set
     ...
     */
    def sliceEx (row: Int, col: Int): SparseMatrixL =
    {
 ...
 *  > runMain scalation.linalgebra.SparseMatrixLTest
 */
