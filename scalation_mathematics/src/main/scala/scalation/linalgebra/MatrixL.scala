 ...
 */
class MatrixL (d1: Int,
               d2: Int,
               private [linalgebra] var v: Array [Array [Long]] = null)
      extends MatriL with Error with Serializable
{
     ...
     */
    def copy (): MatrixL = new MatrixL (dim1, dim2, (for (i <- range1) yield v(i).clone ()).toArray)
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): MatrixL = new MatrixL (m, n)
     ...
     *  @param i  the row index
     */
    def apply (i: Int): VectorL = VectorL (v(i))
     ...
     */
    override def apply (iv: VectoI): MatrixL =
    {
        val a = Array.ofDim [Array [Long]] (iv.dim)
        for (i <- a.indices) a(i) = v(iv(i))
        new MatrixL (a.length, dim2, a)
    } // apply
     ...
     */
    def update (i: Int, u: VectoL) { v(i) = u().toArray }
    ...
    {
        if (b.isInstanceOf [MatrixL]) {
            val bb = b.asInstanceOf [MatrixL]
            for (i <- ir; j <- jr) v(i)(j) = bb.v(i - ir.start)(j - jr.start)
        } else {
            flaw ("update", "must convert b to a MatrixL first")
        } // if
    } // update
     ...
     *  @param u  the 2D array of values to assign
     */
    def set (u: Array [Array [Long]]) { for (i <- range1; j <- range2) v(i)(j) = u(i)(j) }
    ...
    def set (b: MatriL)
    {
        if (b.isInstanceOf [MatrixL]) {
            ...
            flaw ("set", "must convert b to a MatrixL first")
        } // if
    } // set
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `MatrixL` into an integer matrix `MatrixI`.
     */
    def toInt: MatrixI =
    {
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `MatrixL` into a double matrix `MatrixD`.
     */
     ...
     */
    def toDense: MatrixL = this

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Map the elements of 'this' matrix by applying the mapping function 'f'.
     *  @param f  the function to apply
     */
    override def map (f: VectoL => VectoL): MatrixL =
    {
        MatrixL (for (i <- range1) yield f(new VectorL (v(i).length, v(i))), false)
    } // map
    ...
    {
        if (from >= end) return new MatrixL (0, dim2)
    ...
    {
        if (from >= end) return new MatrixL (dim1, 0)
    ...
    {
        if (r_from >= r_end || c_from >= c_end) return new MatrixL (0, 0)
     ...
     */
    def sliceEx (row: Int, col: Int): MatrixL =
    {
     ...
     */
    def setCol (col: Int, u: VectoL) { for (i <- range1) v(i)(col) = u(i) }
...
//  } // t
    ...
    def ++^ (b: MatriL): MatrixL =
    {
        ...
        val c = new MatrixL (dim1, dim2 + b.dim2)
     ...
     */
    def times_ip_pre (b: MatrixL, d: Int = 0)
    {
        val d1_d = dim1 - d
        ...
        for (j <- d until dim2) {
            val col_j = new VectorL (d1_d)                 // save jth column so not overwritten
     ...
     */
    override def ** (b: MatriL): MatrixL = MatrixL (for (i <- range1) yield this(i) * b(i), false)
     ...
     */
    def lud_npp: (MatrixL, MatrixL) =
    {
        if (! isSquare) throw new IllegalArgumentException ("lud_npp: requires a square matrix")

        val l = new MatrixL (dim1)          // lower triangular matrix
        val u = new MatrixL (this)          // upper triangular matrix (a copy of this)

        for (i <- u.range1) {
            val pivot = u.v(i)(i)
            if (pivot =~ 0l) flaw ("lud_npp", "use Fac_LU since there is a zero pivot at row " + i)
            l.v(i)(i) = 1l
    ...
    def lud_ip (): (MatrixL, MatrixL) =
    {
        ...
        for (i <- u.range1) {
            var pivot = u.v(i)(i)
            if (pivot =~ 0l) flaw ("lud_ip", "use Fac_LU since there is a zero pivot at row" + i)
            l.v(i)(i) = 1l
            for (j <- i + 1 until u.dim2) l.v(i)(j) = 0l
            for (k <- i + 1 until u.dim1) {
                val mul = u.v(k)(i) / pivot
    ...
    {
        val y = new VectorL (l.dim2)                 // forward substitution
     ...
     */
    def diag (p: Int, q: Int = 0): MatrixL =
    {
     ...
     */
    def setDiag (x: Long) { for (i <- 0 until math.min (dim1, dim2)) v(i)(i) = x }
     ...
     */
    def clean (thres: Double = TOL, relative: Boolean = true): MatrixL =
    {
    ...
    /** Create a normalized version of 'this' matrix.
      */
    def normalizeU: MatrixL = MatrixL (for (j <- range2) yield col(j).normalizeU)
    ...
    {
        if (! b.isInstanceOf [MatriL]) return false
        val bb = b.asInstanceOf [MatriL]
...
object MatrixL extends Error
{
     ...
     */
    def apply (u: Array [Array [Long]]): MatrixL =
    {
        new MatrixL (u.length, u(0).length, u)
    } // apply
     ...
     */
    def apply (u: Seq [VectoL], columnwise: Boolean = true): MatrixL =
    {
        var x: MatrixL = null
        val u_dim = u(0).dim
        if (columnwise) {
            x = new MatrixL (u_dim, u.length)
            for (j <- x.range2) x.setCol (j, u(j))       // assign column vectors
        } else {
            x = new MatrixL (u.length, u_dim)
     ...
     */
    def apply (u: Vector [VectoL]): MatrixL =
    {
        val u_dim = u(0).dim
        val x = new MatrixL (u_dim, u.length)
     ...
     *  @param u    the repeated values
     */
    def apply (dim: (Int, Int), u: Long*): MatrixL =
    {
        val x = new MatrixL (dim._1, dim._2)
        var k = 0
     ...
     */
    def apply (fileName: String): MatrixL =
    {
        ...
        for (i <- x.range1) {
            x(i) = VectorL (lines(i).split (sp))
            if ((i+1) % PROGRESS == 0) println (s"Read 2 rows ...")
        } // for
        x
    } // apply
     ...
     */
    def apply (fileName: String, skip: Int): MatrixL =
    {
        val lines  = fromFile (fileName).getLines.toArray         // get the lines from file
        val (m, n) = (lines.length-skip, lines(0).split (sp).length)
        val x      = new MatrixL (m, n)
        for (i <- x.range1) {
            x(i) = VectorL (lines(i+skip).split (sp))
            if ((i+1) % PROGRESS == 0) println (s"Read 2 rows ...")
        } // for
        x
    } // apply
     ...
     */
    def ++ (u: VectoL, w: VectoL): MatrixL =
    {
        if (u.dim != w.dim) flaw ("++", "vector dimensions do not match")

        val c = new MatrixL (2, u.dim)
        c(0) = u
        c(1) = w
        c
    } // ++
     ...
     */
    def times (u: VectoL, a: MatrixL): VectorL =
    {
     ...
     */
    def outer (x: VectoL, y: VectoL): MatrixL =
    {
     ...
     */
    def form_rw (x: VectoL, y: VectoL): MatrixL =
    {
     ...
     *  @param y  the second vector -> row 1
     */
    def form_rw (x: Long, y: VectoL): MatrixL =
    {
        val cols = y.dim
        val c = new MatrixL (2, cols)
     ...
     */
    def form_rw (x: VectoL, y: Long): MatrixL =
    {
        val cols = x.dim
        val c = new MatrixL (2, cols)
        c(0) = x
     ...
     */
    def form_cw (x: VectoL, y: VectoL): MatrixL =
    {
     ...
     *  @param y  the second vector -> column 1
     */
    def form_cw (x: Long, y: VectoL): MatrixL =
    {
        val rows = y.dim
        val c = new MatrixL (rows, 2)
     ...
     */
    def form_cw (x: VectoL, y: Long): MatrixL =
    {
        val rows = x.dim
        val c = new MatrixL (rows, 2)
        c.setCol(0, x)
 ...
 *  > runMain scalation.linalgebra.MatrixLTest
 */
object MatrixLTest extends App
{
    ...
    println ("check left  nullspace = " + MatrixL.times (v.t.nullspace, v))
