 ...
 *      `RleMatrixL`        - compressed matrix - Run Length Encoding (RLE)
      ...
      extends Error
{
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): MatriL
     ...
     *  @param j   the column index
     */
    def apply (ir: Range, j: Int): VectoL = col(j)(ir)
     ...
     *  @param jr  the column range
     */
    def apply (i: Int, jr: Range): VectoL = this(i)(jr)
     ...
     */
    def update (ir: Range, j: Int, u: VectoL) { col(j)(ir) = u }
     ...
     *  @param u   the vector to assign
     */
    def update (i: Int, jr: Range, u: VectoL) { this(i)(jr) = u }
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Convert 'this' `MatriL` into a `MatriI`.
     */
    def toInt: MatriI
     ...
     */
    def slice (rg: Range): MatriL = slice (rg.start, rg.end)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Slice 'this' matrix row-wise excluding the given range 'rg'.
     *  @param rg  the excluded range of the slice
     */
    def sliceEx (rg: Range): MatriL = slice (0, rg.start) ++ slice (rg.end, dim1)
     ...
     */
    def sliceEx (row: Int, col: Int): MatriL
     ...
     */
    def selectRowsEx (rowIndex: Array [Int]): MatriL = selectRows ((range1 diff rowIndex).toArray)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Select all rows from 'this' matrix excluding the rows from the given 'rowIndex'.
     *  @param rowIndex  the row indices to exclude
     */
    def selectRowsEx (rowIndex: VectoI): MatriL = selectRows ((range1 diff rowIndex()).toArray)
     ...
     */
    def col (col: Int, from: Int = 0): VectoL
     ...
     *  @param b   the constant vector
     */
    def solve (lu: (MatriL, MatriL), b: VectoL): VectoL = solve (lu._1, lu._2, b)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Solve for 'x' in the equation 'a*x = b' where 'a' is 'this' matrix.
     *  @param b  the constant vector.
     */
    def solve (b: VectoL): VectoL
