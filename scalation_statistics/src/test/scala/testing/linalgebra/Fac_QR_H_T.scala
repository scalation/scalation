...
import scalation.linalgebra.{Fac_QR_H, MatrixD, VectorD}
 ...
 *  > test
 */
class Fac_QR_H_T { @Test def testAll () { Fac_QR_H_T } }
 ...
 *  > test:runMain testing.linalgebra.Fac_QR_H_T
 */
object Fac_QR_H_T extends Tester with App
{
    ...
    KLASS   = "Fac_QR_H"                                           // the class under test
    ...
    def randomize ()
    {
        ...
        y set rvg.gen ()                                           // randomly reset variables
    } // randomize
    ...
    val (q, r) = (new Fac_QR_H (x)).factor12 ()

    test ("factor",    { val (q, r) = (new Fac_QR_H (x)).factor12 (); q * r },
                       x)
    test ("q.t * q",   { val q = (new Fac_QR_H (x)).factor1 (); (q.t * q).asInstanceOf [MatrixD].clean (TOL) },
                       eye (dim2))
    test ("solve",     { val qr = new Fac_QR_H (x); qr.factor (); qr.solve (y) },
                       { val (q, r) = (new Fac_QR_H (x)).factor12 (); r.bsolve (q.t * y) })
