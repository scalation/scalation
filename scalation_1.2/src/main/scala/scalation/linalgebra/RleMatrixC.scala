...
import scalation.math.Complex.{abs => ABS, max => MAX, min => MIN, _}
 ...
 */
 class RleMatrixC (val d1: Int, 
                   val d2: Int, 
                   private var v: Array [RleVectorC] = null,
...
{ 
    ...
    if (v == null) {
        v = Array.ofDim [RleVectorC] (d2)
        for (i <- 0 until v.length) {
            val v1 = new ReArray [TripletC] (1)
            v1(0) = new TripletC (_0, d1, 0)
            v(i) = new RleVectorC (d1, v1)
        } // for   
    } // if
     ...
     */
    def copy (): RleMatrixC = new RleMatrixC (dim1, dim2, v.clone())
     ...
     */
    def zero (mm: Int, nn: Int): RleMatrixC = RleMatrixC (new MatrixC (mm, nn))
     ...
     */
    def apply (i: Int, j: Int): Complex = v(j)(i)
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' matrix's vector at the 'i'-th index position ('i'-th row).
     *  @param i  the row index
     */
    def apply (i: Int): RleVectorC = { RleVectorC (for (j <- range2) yield  v(j)(i)) }
     ...
     */
    def apply (ir: Range, jr: Range): MatriC = slice (ir.start, ir.end, jr.start, jr.end)
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get the underlying 1D array for 'this' matrix.
     */
    def apply (): Array [RleVectorC] = v
     ...
     */
    def update (i: Int, j: Int, x: Complex) { v(j)(i) = x }
     ...
     */
    def update (i: Int, u: VectoC) { for (j <- range2)  v(j)(i) = u(j) }
    ...
    {
        if (b.isInstanceOf [RleMatrixC]) {
            val bb = b.asInstanceOf [RleMatrixC]
            for (i <- ir; j <- jr) this(i, j) = bb(i, j)
        } else {
            flaw ("update","must convert b to RleMatrixC first")
        } // if
    } // update
    ...
    { 
        v = (for (j <- range2) yield RleVectorC (for (i <- range1) yield u(j)(i))).toArray
    } // set
    ...
    { 
        RleMatrixC (for (i <- range2) yield RleVectorC (for (j <- from until end) yield v(i)(j)))       
    } // slice
    ...
    {
        if (from >= end) return new RleMatrixC (0, 0)
        RleMatrixC (for (i <- from until end) yield v(i))
    } // sliceCol
    ...
    {       
        RleMatrixC (for (i <- c_from until c_end) yield RleVectorC (for (j <- r_from until r_end) yield v(i)(j)))
    } // slice
     ...
     */
    def sliceExclude (row: Int, col: Int): RleMatrixC =
    {
        RleMatrixC (for (i <- range2 if i != col) yield RleVectorC (for (j <- range1 if j != row) yield v(i)(j))) 
    } // sliceExclude
    ...
    { 
        val c = new RleMatrixC (rowIndex.length, dim2)
     ...
     */
    def col (col: Int): RleVectorC =  v(col)
    ...
    { 
        v (col) = if (u.isInstanceOf [RleVectorC]) u.asInstanceOf [RleVectorC] 
                  else RleVectorC (u) 
    } // setCol
    ...
    {
        val c = new RleMatrixC (dim1, colIndex.length)
     ...
     */
    def t: RleMatrixC = RleMatrixC (for (i <- range1) yield this(i))
    ...
    {  
        ...
        RleMatrixC (for (i <- range2) yield 
            RleVectorC (for (j <- 0 until (dim1 + 1)) yield if (j == 0) u(i) else v(i)(j-1)))
    } // +:
    ...
    {
        val m = u match {
            case _: RleVectorC => u.asInstanceOf [RleVectorC] +: v
            case _             => RleVectorC (for (i <- u.range) yield u(i)) +: v
        } // match     
        new RleMatrixC (dim1, dim2 + 1, m) 
    } // +^:
    ...
    {  
        if (u.dim != dim2) flaw (":+", "vector does not match row dimension")
        RleMatrixC (for (i <- range2) yield 
            RleVectorC (for (j <- 0 until (dim1 + 1)) yield if (j < dim1) v(i)(j) else u(i)))
    } // :+
    ...
    {
        val m = u match {
            case _: RleVectorC => v :+ u.asInstanceOf [RleVectorC]
            case _             => v :+ RleVectorC (for (i <- u.range) yield u(i)) 
        } // match     
        new RleMatrixC (dim1, dim2 + 1, m) 
    } // :^+
    ...
    def ++^ (b: MatriC): RleMatrixC =
    {
        ...
        val m = b match {
        case _: RleMatrixC => v ++ b.asInstanceOf [RleMatrixC]()
        case _             => v ++ RleMatrixC (b.asInstanceOf [MatrixC])()
        } // match
        new RleMatrixC (dim1, dim2 + dim2, m)
    } // ++^
    ...
    def + (b: MatriC): RleMatrixC = 
    {
         val c = new RleMatrixC (dim1, dim2) 
         b match {
         case _: RleMatrixC => for (i <- range2) c.setCol (i, col(i) + b.col(i))                                                             
    ...
    {
        b match {
        case _: RleMatrixC => for (i <- range2) col(i) += b.col(i)
    ...
    def ~^ (p: Int): RleMatrixC = 
    {
        ...
        var t    = p
        var res  = new RleMatrixC (dim1, dim2)
        var temp = new RleMatrixC (dim1, dim2)
    ...
    {
        var x = new RleMatrixC (dim1, dim2)
     ...
     */
    private def partialPivoting (a: RleMatrixC, i: Int): Int =
    {
    ...
    {
        var x = RleMatrixC (new MatrixC (dim1, dim2))
    ...
    def * (u: VectoC): VectoC = 
    {
        ...
        u match {
        case _: RleVectorC => mul2 (u.asInstanceOf [RleVectorC])
        case _             => mul (this, u)
        } // match
    } // *
    ...
    {
        b match {
        case _: RleMatrixC => t mdot b.asInstanceOf [RleMatrixC]
        case _             => toDense * b
        } // match
    } // *
    ...
    def *= (b: MatriC): MatriC = 
    {
        ...
        val temp = new RleMatrixC (b.dim1, b.dim2)
        for (i <- 0 until b.dim1; j <- 0 until b.dim2) temp(i, j) = b(i, j)
        for (i <- range1) {
            val row_i = new VectorC (dim2)
    ...
    def ** (u: VectoC): MatriC = 
    {
        var c = new RleMatrixC (dim1,dim2)
    ...
    def diag (p: Int,q: Int = 0): MatriC = 
    {
        ...
        var c = new RleMatrixC (p + dim1 + q, p + dim2 + q)
        var Ip = eye (p)
        var Iq = eye (q)
    ...
    {
        var c = new RleMatrixC (dim1 + b.dim1, dim2 + b.dim2)
        var x1 = new RleVectorC (b.dim2)
        for (i <- range1) c(i) = this(i) ++ x1
        val x2 = new RleVectorC (dim2)
    ...
    def dot (b: MatriC): RleVectorC = 
    {
        ...
        RleVectorC (for (j <- range2) yield  v(j) dot b.col(j))
    } // dot
    ...
    {
        if (dim1 != b.dim1) flaw ("dot", "incompatible")        
        RleVectorC (for (j <- range2) yield  v(j) dot b.v(j))
    } // dot
    ...
    {   
        ...
        val vv = Array.ofDim [RleVectorC] (d2)   
        for (j <- b.range2) vv(j) = RleVectorC (for (i <- range2) yield  v(i) dot b.col(j))
        new RleMatrixC (dim2, b.dim2, vv)
    } // mdot
    ...
    {   
        if (dim1 != b.dim1) flaw ("mdot", "incompatible first dimensions")
        val vv = Array.ofDim [RleVectorC] (d2)   
        for (j <- b.range2) vv(j) = RleVectorC (for (i <- range2) yield  v(i) dot b.v(j))
        new RleMatrixC (dim2, b.dim2, vv)
    } // mdot
    ...
    def inverse: MatriC = 
    {
        var b = new RleMatrixC (dim1, dim2)
        var u = eye (dim1)
     ...
     */
    def lud: Tuple2 [RleMatrixC, RleMatrixC] = 
    {
        val l = new RleMatrixC (dim1, dim2)
        val u = new RleMatrixC (dim1, dim2)
    ...
    def lud_ip: (RleMatrixC, RleMatrixC) = 
    {
        ...
        var l = new RleMatrixC (dim1, dim2)
    ...
    def reduce: RleMatrixC = 
    {
        ...
        val b = new RleMatrixC (dim1, dim2)
        for (i <- range1; j <- range2) b(i, j) = this(i, j)
        for (i <- range1) {
            var x = b(i,i)
     ...
     */
    def solve (l: MatriC,u: MatriC, b: VectoC): RleVectorC = 
    {
        var y = new RleVectorC (l.dim2)
        for (i <- 0 until y.dim) {
            var sum = _0
            for (j <- 0 until i) sum += l(i, j) * y(j)
            y(i) = b(i) - sum
        } // for
        u.bsolve (y).asInstanceOf [RleVectorC]
    } // solve
     ...
     *  @param b   the constant vector
     */
    def solve (lu: Tuple2 [MatriC, MatriC], b: VectoC): RleVectorC = solve (lu._1, lu._2, b)
     ...
     *  @param b  the constant vector
     */
    def solve (u: MatriC, b: VectoC): RleVectorC = solve (this, u, b)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Solve for 'x' in the equation 'a*x = b' where 'a' is 'this' matrix.
     *  @param b  the constant vector.
     */ 
    def solve (b: VectoC): RleVectorC = solve (lud, b)
    ...
    {
        b.isInstanceOf [RleMatrixC] && (v.deep equals b.asInstanceOf [RleMatrixC].v.deep)
    } // equals
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `RleMatrixC` companion object provides operations for `RleMatrixC` that don't require
...
object RleMatrixC 
{
    ...
    /**
     */
    def apply (x: RleVectorC, xs: RleVectorC*): RleMatrixC = 
    {     
        new RleMatrixC (xs.length + 1, x.dim, (x +: xs).toArray)    
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def apply (x: Seq [RleVectorC]): RleMatrixC = 
    {
        new RleMatrixC (x(0).dim, x.length, x.toArray)
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def apply (u: Vector [VectoC]): RleMatrixC = 
    {
        val u_dim = u(0).dim
        val x = new RleMatrixC (u_dim, u.length)
    ...
    /**
     */
    def apply (x: MatrixC): RleMatrixC = 
    {
        RleMatrixC (for ( j <- 0 until x.dim2) yield RleVectorC (x.col(j)))
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def mul (at: RleMatrixC, u: VectoC): RleVectorC = 
    {
        RleVectorC (for (i <- 0 until at.dim1) yield at(i) dot u) 
    } // mul

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def mul (a: RleMatrixC, u: RleVectorC): RleVectorC = 
    {
        RleVectorC (for (i <- 0 until a.dim1) yield a(i) dot u)    
    } // mul
 ...
 *  > run-main scalation.linalgebra.RleMatrixCTest
 */
object RleMatrixCTest extends App with PackageInfo
{
    for (l <- 1 to 4) {
        println ("\n\tTest RleMatrixC on real matrices of dim " + l)
        val x = new RleMatrixC (l, l)
        val y = new RleMatrixC (l, l)
        x.set (2)
        y.set (3)
        println ("x + y = " + (x + y))
        println ("x - y = " + (x - y))
    ...
    println ("\n\tTest RleMatrixC on additional operations")

    val z   = RleMatrixC (new MatrixC ((2, 2), 1, 2,
                                               3, 2))
    val t   = RleMatrixC (new MatrixC ((3, 3), 1, 2, 3,
                                               4, 3, 2,
                                               1, 3, 1))
    val zz  = RleMatrixC (new MatrixC ((3, 3), 3, 1, 0,
                                               1, 4, 2,
                                               0, 2, 5))
    ...
    val w = RleMatrixC (new MatrixC ((2, 3), 2,  3, 5, 
                                            -4,  2, 3))
    val v = RleMatrixC (new MatrixC ((3, 2), 2, -4, 
                                             3,  2,
                                             5,  3))
    println ("w         = " + w)
    println ("v         = " + v)
    ...
    val aa = RleMatrixC (new MatrixC ((3, 2), 1, 2,
                                              3, 4,
                                              5, 6))
    val bb = RleMatrixC (new MatrixC ((2, 2), 1, 2,
                                              3, 4))
    val cc = RleMatrixC (new MatrixC ((3, 2), 3, 4,
                                              5, 6,
                                              7, 8))
