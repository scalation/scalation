 ...
 *
 *  Multi-Graph 'MuGraph' Dual Isomorphism Using Mutable Sets
 */
      ...
      extends GraphMatcher (g, q)
{
    ...
    override def bijections (): SET [Array [Int]] =
    {
        ...
        saltzDualIso (duals.prune (phi), 0)                // recursively find all bijections
    ...
    private def saltzDualIso (phi: Array [SET [Int]], depth: Int)
    {
        if (depth == q.size) {
            ...
            for (i <- phi (depth) if (! contains (phi, depth, i))) {
                ...
                saltzDualIso (duals.prune (phiCopy), depth + 1)          // solve recursively for the next depth
            } // for
        } // if
    } // saltzDualIso
