...
//      case _: VectorD => rlg = RandomVecD (nLabs, stream)
    ...
    def genRandomConnectedGraph (size: Int, nLabels: Int, eLabels: Int, avDegree: Int,
                                 inverse: Boolean = true, name: String = "g"): MGraph [TLabel] =
    {
                     ...
                     name: String = "g"): MGraph [TLabel] =
    {
        ...
        MGraphGen.buildQGraph (maxNodes, maxChMap, g.label, g.elabel, inverse, name)
    } // genBFSQuery
...
//      case _: VectorD => // implement                                                     // for `VectorD`
    ...
    private def randDisteLabels (ch: Array [SET [Int]], eLabels: Int): Map [(Int, Int), TLabel] =
    {
        ...
        typeSelector match {
        case _: Int     => for (i <- ch.indices; c <- ch(i)) elab += (i, c) -> rlg.igen.asInstanceOf [TLabel]  
        case _: Double  => for (i <- ch.indices; c <- ch(i)) elab += (i, c) -> rlg.gen.asInstanceOf [TLabel]  
        case _: String  => for (i <- ch.indices; c <- ch(i)) elab += (i, c) -> rlg.sgen.asInstanceOf [TLabel]  
//      case _: VectorD => // implement                                                     // for `VectorD`
        case _          => { flaw ("randDistLabels", "label type not supported"); null.asInstanceOf [Array [TLabel]] }
        } // match
...
object MGraphGen
{
    ...
    def buildQGraph [TLabel: ClassTag] (nodes: SET [Int], chMap: Map [Int, SET [Int]], gLabel: Array [TLabel],
                             gELabel: Map [(Int, Int), TLabel], inverse: Boolean, name: String): MGraph [TLabel] =
    {
       ...
       println("new2OldIds = " + new2OldIds.deep)
...
{
         val g = MGraphGen.genGraph ("0.0")
         banner ("data graph")
         g.printG ()
         println (GraphMetrics.stats (g))
         banner ("query graph")
         val q = MGraphGen.extractSubgraph(5, g)
         q.printG ()
         println (GraphMetrics.stats (q))
