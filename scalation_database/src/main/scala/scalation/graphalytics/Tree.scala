...
object Tree
{
     ...
     */
    def apply (depth: Double, minOut: Int = 0, maxOut: Int = 2, stream: Int = 0): Tree =
    {
    ...
    def apply (pred: Array [Int], labl: Array [TLabel], depth: Double, name: String): Tree =
    {
        ...
        for (ni <- 1 until pred.length) {
            ...
            if (pi >= 0) {
                ...
                val p = n_map.getOrElse (pi, { val pp = new TreeNode (pi, 1, lab(pi)); n_map += pi -> pp; pp })
                val n = n_map.getOrElse (ni, { val nn = new TreeNode (ni, p.lev+1, lab(ni)); n_map += ni -> nn; nn })
                tree.add (p, n)
            } // if
        } // for
        tree
 ...
 *  @param depth  the estimated average depth of the tree (used by animation)
 */
class Tree (val root: TreeNode, depth: Double, val name: String = "tree")
{
    private val DEBUG = true                          // debug flag
