...
import scalation.linalgebra.{Fac_QR_MGS, MatrixD}
 ...
 *  > test
 */
class Fac_QR_MGS_T { @Test def testAll () { Fac_QR_MGS_T } }
 ...
 *  > test:runMain testing.linalgebra.Fac_QR_MGS_T
 */
object Fac_QR_MGS_T extends Tester // with App
{
    ...
    KLASS   = "Fac_QR_MGS"                                         // the class under test
    ...
    test ("factor",    { val (q, r) = (new Fac_QR_MGS (x)).factor12 (); q * r },
                       x)
