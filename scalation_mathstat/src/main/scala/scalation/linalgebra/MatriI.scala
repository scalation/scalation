      ...
      extends Error
{
     ...
     *  @param b   the constant vector
     */
    def solve (lu: Tuple2 [MatriI, MatriI], b: VectoI): VectoI
    ...
    def rank (upper: Boolean = false): Int =
    {
        ...
        var count = 0
        for (i <- 0 until max if u(i, i) !=~ 0) count += 1
        count
    } // rank
     ...
     */
    def norm1est (inv: Boolean = true): Int =
    {
        val a  = this
        val at = a.t
        val e  = a(0); e.set (1 / a.dim2)
        ...
        while (! done) {
            val j = x.abs.argmax ()
            for (k <- e.range) e(k) = if (k == j) 1 else 0    // one in jth position
            v     = if (inv) a.solve (e) else a.col (j) 
            val g = γ
            γ     = v.norm1
            if (v.map (signum (_)) == ξ || γ <= g) {
                for (i <- x.range) x(i) = (if (i % 2 == 0) -1 else 1) * (1 + ((i - 1) / (x.dim - 1)))
                x = if (inv) a.solve (x) else a * x
                if ((1 * 2 * x.norm1) / (1 * 3 * x.dim) > γ) { v = x; γ = (1 * 2 * x.norm1) / (1 * 3 * x.dim) }
            } // if
            ξ     = v.map (signum (_))
