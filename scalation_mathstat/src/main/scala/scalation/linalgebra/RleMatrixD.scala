 ...
 */
 class RleMatrixD (val d1: Int, 
                   val d2: Int, 
                   private var v: Array [RleVectorD] = null,
...
{ 
    ...
    if (v == null) {
        v = Array.ofDim [RleVectorD] (d2)
        for (i <- 0 until v.length) {
            val v1 = new ReArray [TripletD] (1)
            v1(0) = new TripletD (0.0, d1, 0)
     ...
     */
    def copy (): RleMatrixD = new RleMatrixD (dim1, dim2, v.clone())
     ...
     */
    def zero (mm: Int, nn: Int): RleMatrixD = RleMatrixD (new MatrixD (mm, nn))
     ...
     */
    def apply (i: Int, j: Int): Double = v(j)(i)
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' matrix's vector at the 'i'-th index position ('i'-th row).
     *  @param i  the row index
     */
    def apply (i: Int): RleVectorD = { RleVectorD (for (j <- range2) yield  v(j)(i)) }
     ...
     */
    def apply (ir: Range, jr: Range): MatriD = slice (ir.start, ir.end, jr.start, jr.end)
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get the underlying 1D array for 'this' matrix.
     */
    def apply (): Array [RleVectorD] = v
     ...
     */
    def update (i: Int, j: Int, x: Double) { v(j)(i) = x }
     ...
     */
    def update (i: Int, u: VectoD) { for (j <- range2)  v(j)(i) = u(j) }
    ...
    {
        if (b.isInstanceOf [RleMatrixD]) {
            val bb = b.asInstanceOf [RleMatrixD]
            for (i <- ir; j <- jr) this(i, j) = bb(i, j)
        } else {
            flaw ("update","must convert b to RleMatrixD first")
        } // if
    } // update
    ...
    { 
        v = (for (j <- range2) yield RleVectorD (for (i <- range1) yield u(j)(i))).toArray
    } // set
     ...
     */
    def slice (from: Int, end: Int): RleMatrixD =
    { 
        RleMatrixD (for (i <- range2) yield RleVectorD (for (j <- from until end) yield v(i)(j)))       
    } // slice
     ...
     */
    def sliceCol (from: Int, end: Int): RleMatrixD = 
    {
        if (from >= end) return new RleMatrixD (0, 0)
        RleMatrixD (for (i <- from until end) yield v(i))
    } // sliceCol
     ...
     */
    def slice (r_from: Int, r_end: Int, c_from: Int, c_end: Int): RleMatrixD = 
    {       
        RleMatrixD (for (i <- c_from until c_end) yield RleVectorD (for (j <- r_from until r_end) yield v(i)(j)))
    } // slice
     ...
     */
    def selectRows (rowIndex: Array [Int]): RleMatrixD =
    { 
        val c = new RleMatrixD (rowIndex.length, dim2)
     ...
     */
    def col (col: Int): RleVectorD =  v(col)
    ...
    { 
        v (col) = if (u.isInstanceOf [RleVectorD]) u.asInstanceOf [RleVectorD] 
                  else RleVectorD (u) 
    } // setCol
     ...
     */
    def selectCols (colIndex: Array [Int]): RleMatrixD =
    {
        val c = new RleMatrixD (dim1, colIndex.length)
     ...
     */
    def t: RleMatrixD = RleMatrixD (for (i <- range1) yield this(i))

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Concatenate (row) vector 'u' and 'this' matrix, i.e., prepend 'u' to 'this'.
     *  @param u  the vector to be prepended as the new first row in new matrix
     */
    def +: (u: VectoD): RleMatrixD =
    {  
        if (u.dim != dim2) flaw ("+:", "vector does not match row dimension")
        RleMatrixD (for (i <- range2) yield 
            RleVectorD (for (j <- 0 until (dim1 + 1)) yield if (j == 0) u(i) else v(i)(j-1)))
    } // +:
    ...
    {
        val m = u match {
            case _: RleVectorD => u.asInstanceOf [RleVectorD] +: v
            case _             => RleVectorD (for (i <- u.range) yield u(i)) +: v
        } // match     
        new RleMatrixD (dim1, dim2 + 1, m) 
    } // +^:
    ...
    {  
        if (u.dim != dim2) flaw (":+", "vector does not match row dimension")
        RleMatrixD (for (i <- range2) yield 
            RleVectorD (for (j <- 0 until (dim1 + 1)) yield if (j < dim1) v(i)(j) else u(i)))
    } // :+
    ...
    {
        val m = u match {
            case _: RleVectorD => v :+ u.asInstanceOf [RleVectorD]
            case _             => v :+ RleVectorD (for (i <- u.range) yield u(i)) 
        } // match     
        new RleMatrixD (dim1, dim2 + 1, m) 
    } // :^+
    ...
    def ++^ (b: MatriD): RleMatrixD =
    {
        ...
        val m = b match {
        case _: RleMatrixD => v ++ b.asInstanceOf [RleMatrixD]()
        case _             => v ++ RleMatrixD (b.asInstanceOf [MatrixD])()
        } // match
        new RleMatrixD (dim1, dim2 + dim2, m)
    } // ++^
    ...
    def + (b: MatriD): RleMatrixD = 
    {
         val c = new RleMatrixD (dim1, dim2) 
         b match {
         case _: RleMatrixD => for (i <- range2) c.setCol (i, col(i) + b.col(i))                                                             
    ...
    {
        b match {
        case _: RleMatrixD => for (i <- range2) col(i) += b.col(i)
     ...
     */  
    def ~^ (p: Int): RleMatrixD = 
    {
        if (p < 2)        flaw ("~^", "p must be an integer >= 2")
        if (dim1 != dim2) flaw ("~^", "Only defined on square matrices")

        var t    = p
        var res  = new RleMatrixD (dim1, dim2)
        var temp = new RleMatrixD (dim1, dim2)
        for (i <- range1; j <- range2) {
            temp(i, j) = this(i, j)
            res(i, j)  = if (i == j) 1.0 else 0.0
        } // for
        while (t > 0) {
            if (t % 2 == 1) res *= temp
            temp *= temp
     ...
     */ 
    def lowerT: RleMatrixD = 
    {
        var x = new RleMatrixD (dim1, dim2)
     ...
     */
    private def partialPivoting (a: RleMatrixD, i: Int): Int =
    {
     ...
     */ 
    def upperT: RleMatrixD = 
    {
        var x = RleMatrixD (new MatrixD (dim1, dim2))
     ...
     */
    def mul2 (u: RleVectorD): RleVectorD =
    {      
        val c = new RleVectorD (dim1)
        for (i <- range1) {
            var sum = 0.0
    ...
    def * (u: VectoD): VectoD = 
    {
        ...
        u match {
        case _: RleVectorD => mul2 (u.asInstanceOf [RleVectorD])
        case _             => mul (this, u)
        } // match
    } // *
    ...
    {
        b match {
        case _: RleMatrixD => t mdot b.asInstanceOf [RleMatrixD]
        case _             => toDense * b
        } // match
    } // *
    ...
    def *= (b: MatriD): MatriD = 
    {
        ...
        val temp = new RleMatrixD (b.dim1, b.dim2)
    ...
    def ** (u: VectoD): MatriD = 
    {
        var c = new RleMatrixD (dim1,dim2)
     ...
     */
    def clean (thres: Double = TOL, relative: Boolean = true): MatriD = 
    {
    ...
    def diag (p: Int,q: Int = 0): MatriD = 
    {
        ...
        var c = new RleMatrixD (p + dim1 + q, p + dim2 + q)
        var Ip = eye (p)
        var Iq = eye (q)
     ...
     */
    def diag (b: MatriD): RleMatrixD = 
    {
        var c = new RleMatrixD (dim1 + b.dim1, dim2 + b.dim2)
        var x1 = new RleVectorD (b.dim2)
        for (i <- range1) c(i) = this(i) ++ x1
        val x2 = new RleVectorD (dim2)
     ...
     */
    def dot (b: MatriD): RleVectorD = 
    {
        if (dim1 != b.dim1) flaw ("dot", "incompatible")        
        RleVectorD (for (j <- range2) yield  v(j) dot b.col(j))
    } // dot
     ...
     *  @param b  the matrix to multiply by (requires same first dimensions)
     */
    def dot (b: RleMatrixD): RleVectorD = 
    {
        if (dim1 != b.dim1) flaw ("dot", "incompatible")        
     ...
     *  @param b  the matrix to multiply by (requires same first dimensions)
     */
    def mdot (b: MatriD): RleMatrixD = 
    {   
        if (dim1 != b.dim1) flaw ("mdot", "incompatible first dimensions")
        val vv = Array.ofDim [RleVectorD] (d2)   
        for (j <- b.range2) vv(j) = RleVectorD (for (i <- range2) yield  v(i) dot b.col(j))
    ...
    {   
        if (dim1 != b.dim1) flaw ("mdot", "incompatible first dimensions")
        val vv = Array.ofDim [RleVectorD] (d2)   
        for (j <- b.range2) vv(j) = RleVectorD (for (i <- range2) yield  v(i) dot b.v(j))
        new RleMatrixD (dim2, b.dim2, vv)
    } // mdot
     ...
     */
    def getDiag (k: Int = 0): RleVectorD = 
    {
        var x = new RleVectorD (1)
        var i, j = 0
        if (k >= 0) { i = 0;      j = k }
        else        { i = -1 * k; j = 0 }
        x(0) = this(i, j)
    ...
    def inverse: MatriD = 
    {
        var b = new RleMatrixD (dim1, dim2)
        var u = eye (dim1)
     ...
     *  Takes '[a | b]' to '[I | x]'.
     */
    def reduce: RleMatrixD = 
    {
        if (dim2 < dim1) flaw ("reduce", "requires n(columns) >= n(rows)")
            
        val b = new RleMatrixD (dim1, dim2)
        for (i <- range1; j <- range2) b(i, j) = this(i, j)
        for (i <- range1) {
            var x = b(i,i)
     ...
     */
    def solve (l: MatriD,u: MatriD, b: VectoD): RleVectorD = 
    {
        var y = new RleVectorD (l.dim2)
        for (i <- 0 until y.dim) {
            var sum = 0.0
            for (j <- 0 until i) sum += l(i, j) * y(j)
            y(i) = b(i) - sum
        } // for
        u.bsolve (y).asInstanceOf [RleVectorD]
    } // solve
     ...
     *  @param b   the constant vector
     */
    def solve (lu: Tuple2 [MatriD, MatriD], b: VectoD): RleVectorD = solve (lu._1, lu._2, b)
     ...
     *  @param b  the constant vector
     */
    def solve (u: MatriD, b: VectoD): RleVectorD = solve (this, u, b)
    ...
    {
        b.isInstanceOf [RleMatrixD] && (v.deep equals b.asInstanceOf [RleMatrixD].v.deep)
    } // equals
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `RleMatrixD` companion object provides operations for `RleMatrixD` that don't require
...
object RleMatrixD 
{
    ...
    /**
     */
    def apply (x: RleVectorD, xs: RleVectorD*): RleMatrixD = 
    {     
        new RleMatrixD (xs.length + 1, x.dim, (x +: xs).toArray)    
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def apply (x: Seq [RleVectorD]): RleMatrixD = 
    {
        new RleMatrixD (x(0).dim, x.length, x.toArray)
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def apply (u: Vector [VectoD]): RleMatrixD = 
    {
        val u_dim = u(0).dim
        val x = new RleMatrixD (u_dim, u.length)
    ...
    /**
     */
    def apply (x: MatrixD): RleMatrixD = 
    {
        RleMatrixD (for ( j <- 0 until x.dim2) yield RleVectorD (x.col(j)))
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def mul (at: RleMatrixD, u: VectoD): RleVectorD = 
    {
        RleVectorD (for (i <- 0 until at.dim1) yield at(i) dot u) 
    } // mul

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def mul (a: RleMatrixD, u: RleVectorD): RleVectorD = 
    {
        RleVectorD (for (i <- 0 until a.dim1) yield a(i) dot u)    
    } // mul
 ...
 */
object RleMatrixDTest extends App with PackageInfo
{
    for (l <- 1 to 4) {
        println ("\n\tTest RleMatrixD on real matrices of dim " + l)
        val x = new RleMatrixD (l, l)
        val y = new RleMatrixD (l, l)
        x.set (2)
        y.set (3)
        println ("x + y = " + (x + y))
        println ("x - y = " + (x - y))
    ...
    println ("\n\tTest RleMatrixD on additional operations")

    val z   = RleMatrixD (new MatrixD ((2, 2), 1, 2,
                                               3, 2))
    val t   = RleMatrixD (new MatrixD ((3, 3), 1, 2, 3,
                                               4, 3, 2,
                                               1, 3, 1))
    val zz  = RleMatrixD (new MatrixD ((3, 3), 3, 1, 0,
                                               1, 4, 2,
                                               0, 2, 5))
    ...
    val w = RleMatrixD (new MatrixD ((2, 3), 2,  3, 5, 
                                            -4,  2, 3))
    val v = RleMatrixD (new MatrixD ((3, 2), 2, -4, 
                                             3,  2,
                                             5,  3))
    println ("w         = " + w)
    println ("v         = " + v)
    ...
    val aa = RleMatrixD (new MatrixD ((3, 2), 1, 2,
                                              3, 4,
                                              5, 6))
    val bb = RleMatrixD (new MatrixD ((2, 2), 1, 2,
                                              3, 4))
    val cc = RleMatrixD (new MatrixD ((3, 2), 3, 4,
                                              5, 6,
                                              7, 8))
