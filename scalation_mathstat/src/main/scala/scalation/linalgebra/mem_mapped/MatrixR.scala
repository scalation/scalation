...
import scalation.util.{Error, MM_ArrayR, PackageInfo}
...
class MatrixR (val d1: Int,
               val d2: Int,
       private var v:  Array [MM_ArrayR] = null)
      extends MatriR with Error with Serializable
{
    ...
    if (v == null) {
        v = Array.ofDim [MM_ArrayR] (dim1)
        for (i <- 0 until dim1) v(i) = MM_ArrayR.ofDim (dim2)
     ...
     */
    def this (u: Array [MM_ArrayR]) { this (u.length, u(0).length, u) }
