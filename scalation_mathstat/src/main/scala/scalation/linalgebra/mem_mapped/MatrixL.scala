...
import scalation.util.{Error, MM_ArrayL, PackageInfo}
...
class MatrixL (val d1: Int,
               val d2: Int,
       private var v:  Array [MM_ArrayL] = null)
      extends MatriL with Error with Serializable
{
    ...
    if (v == null) {
        v = Array.ofDim [MM_ArrayL] (dim1)
        for (i <- 0 until dim1) v(i) = MM_ArrayL.ofDim (dim2)
     ...
     */
    def this (u: Array [MM_ArrayL]) { this (u.length, u(0).length, u) }
     ...
     */
    def set (i: Int, u: VectorL, j: Int = 0) { for (k <- 0 until u.dim) v(i)(k+j) = u(k) }
     ...
     *  @param b  the constant vector
     */
    def solve (u: MatriL, b: VectorL): VectorL = solve (this, u, b)
...
object MatrixL extends Error
{
     ...
     */
    def apply (u: Array [VectorL], columnwise: Boolean = true): MatrixL =
    {
        var x: MatrixL = null
        val u_dim = u(0).dim
        if (columnwise) {
     ...
     */
    def apply (u: Vector [VectorL]): MatrixL =
    {
        val u_dim = u(0).dim
