...
import scalation.math.Real.{abs => ABS, max => MAX, min => MIN, _}
 ...
 */
 class RleMatrixR (val d1: Int, 
                   val d2: Int, 
                   private var v: Array [RleVectorR] = null,
...
{ 
    ...
    if (v == null) {
        v = Array.ofDim [RleVectorR] (d2)
        for (i <- 0 until v.length) {
            val v1 = new ReArray [TripletR] (1)
            v1(0) = new TripletR (_0, d1, 0)
            v(i) = new RleVectorR (d1, v1)
        } // for   
    } // if
     ...
     */
    def copy (): RleMatrixR = new RleMatrixR (dim1, dim2, v.clone())
     ...
     */
    def zero (mm: Int, nn: Int): RleMatrixR = RleMatrixR (new MatrixR (mm, nn))
     ...
     */
    def apply (i: Int, j: Int): Real = v(j)(i)
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' matrix's vector at the 'i'-th index position ('i'-th row).
     *  @param i  the row index
     */
    def apply (i: Int): RleVectorR = { RleVectorR (for (j <- range2) yield  v(j)(i)) }
     ...
     */
    def apply (ir: Range, jr: Range): MatriR = slice (ir.start, ir.end, jr.start, jr.end)
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get the underlying 1D array for 'this' matrix.
     */
    def apply (): Array [RleVectorR] = v
     ...
     */
    def update (i: Int, j: Int, x: Real) { v(j)(i) = x }
     ...
     */
    def update (i: Int, u: VectoR) { for (j <- range2)  v(j)(i) = u(j) }
    ...
    {
        if (b.isInstanceOf [RleMatrixR]) {
            val bb = b.asInstanceOf [RleMatrixR]
            for (i <- ir; j <- jr) this(i, j) = bb(i, j)
        } else {
            flaw ("update","must convert b to RleMatrixR first")
        } // if
    } // update
     ...
     *  @param x  the scalar value to assign
     */
    def set (x: Real) { for (i <- range1; j <- range2) v(i)(j) = x }
    ...
    { 
        v = (for (j <- range2) yield RleVectorR (for (i <- range1) yield u(j)(i))).toArray
    } // set
    ...
    { 
        RleMatrixR (for (i <- range2) yield RleVectorR (for (j <- from until end) yield v(i)(j)))       
    } // slice
    ...
    {
        if (from >= end) return new RleMatrixR (0, 0)
        RleMatrixR (for (i <- from until end) yield v(i))
    } // sliceCol
    ...
    {       
        RleMatrixR (for (i <- c_from until c_end) yield RleVectorR (for (j <- r_from until r_end) yield v(i)(j)))
    } // slice
     ...
     */
    def sliceExclude (row: Int, col: Int): RleMatrixR =
    {
        RleMatrixR (for (i <- range2 if i != col) yield RleVectorR (for (j <- range1 if j != row) yield v(i)(j))) 
    } // sliceExclude
    ...
    { 
        val c = new RleMatrixR (rowIndex.length, dim2)
     ...
     */
    def col (col: Int): RleVectorR =  v(col)
    ...
    { 
        v (col) = if (u.isInstanceOf [RleVectorR]) u.asInstanceOf [RleVectorR] 
                  else RleVectorR (u) 
    } // setCol
    ...
    {
        val c = new RleMatrixR (dim1, colIndex.length)
     ...
     */
    def t: RleMatrixR = RleMatrixR (for (i <- range1) yield this(i))
    ...
    {  
        ...
        RleMatrixR (for (i <- range2) yield 
            RleVectorR (for (j <- 0 until (dim1 + 1)) yield if (j == 0) u(i) else v(i)(j-1)))
    } // +:
    ...
    {
        val m = u match {
            case _: RleVectorR => u.asInstanceOf [RleVectorR] +: v
            case _             => RleVectorR (for (i <- u.range) yield u(i)) +: v
        } // match     
        new RleMatrixR (dim1, dim2 + 1, m) 
    } // +^:
    ...
    {  
        if (u.dim != dim2) flaw (":+", "vector does not match row dimension")
        RleMatrixR (for (i <- range2) yield 
            RleVectorR (for (j <- 0 until (dim1 + 1)) yield if (j < dim1) v(i)(j) else u(i)))
    } // :+
    ...
    {
        val m = u match {
            case _: RleVectorR => v :+ u.asInstanceOf [RleVectorR]
            case _             => v :+ RleVectorR (for (i <- u.range) yield u(i)) 
        } // match     
        new RleMatrixR (dim1, dim2 + 1, m) 
    } // :^+
    ...
    def ++ (b: MatriR): RleMatrixR =
    {
        ...
        val c = new MatrixR (dim1 + b.dim1, dim2)
    ...
    def ++^ (b: MatriR): RleMatrixR =
    {
        ...
        val m = b match {
        case _: RleMatrixR => v ++ b.asInstanceOf [RleMatrixR]()
        case _             => v ++ RleMatrixR (b.asInstanceOf [MatrixR])()
        } // match
        new RleMatrixR (dim1, dim2 + dim2, m)
    } // ++^
    ...
    def + (b: MatriR): RleMatrixR = 
    {
         val c = new RleMatrixR (dim1, dim2) 
         b match {
         case _: RleMatrixR => for (i <- range2) c.setCol (i, col(i) + b.col(i))                                                             
    ...
    {
        b match {
        case _: RleMatrixR => for (i <- range2) col(i) += b.col(i)
    ...
    def ~^ (p: Int): RleMatrixR = 
    {
        ...
        var t    = p
        var res  = new RleMatrixR (dim1, dim2)
        var temp = new RleMatrixR (dim1, dim2)
    ...
    {
        var x = new RleMatrixR (dim1, dim2)
     ...
     */
    private def partialPivoting (a: RleMatrixR, i: Int): Int =
    {
    ...
    {
        var x = RleMatrixR (new MatrixR (dim1, dim2))
    ...
    def * (u: VectoR): VectoR = 
    {
        ...
        u match {
        case _: RleVectorR => mul2 (u.asInstanceOf [RleVectorR])
        case _             => mul (this, u)
        } // match
    } // *
    ...
    {
        b match {
        case _: RleMatrixR => t mdot b.asInstanceOf [RleMatrixR]
        case _             => toDense * b
        } // match
    } // *
    ...
    def *= (b: MatriR): MatriR = 
    {
        ...
        val temp = new RleMatrixR (b.dim1, b.dim2)
        for (i <- 0 until b.dim1; j <- 0 until b.dim2) temp(i, j) = b(i, j)
        for (i <- range1) {
            val row_i = new VectorR (dim2)
    ...
    def ** (u: VectoR): MatriR = 
    {
        var c = new RleMatrixR (dim1,dim2)
    ...
    def diag (p: Int,q: Int = 0): MatriR = 
    {
        ...
        var c = new RleMatrixR (p + dim1 + q, p + dim2 + q)
        var Ip = eye (p)
        var Iq = eye (q)
    ...
    {
        var c = new RleMatrixR (dim1 + b.dim1, dim2 + b.dim2)
        var x1 = new RleVectorR (b.dim2)
        for (i <- range1) c(i) = this(i) ++ x1
        val x2 = new RleVectorR (dim2)
    ...
    def dot (b: MatriR): RleVectorR = 
    {
        ...
        RleVectorR (for (j <- range2) yield  v(j) dot b.col(j))
    } // dot
    ...
    {
        if (dim1 != b.dim1) flaw ("dot", "incompatible")        
        RleVectorR (for (j <- range2) yield  v(j) dot b.v(j))
    } // dot
    ...
    {   
        ...
        val vv = Array.ofDim [RleVectorR] (d2)   
        for (j <- b.range2) vv(j) = RleVectorR (for (i <- range2) yield  v(i) dot b.col(j))
        new RleMatrixR (dim2, b.dim2, vv)
    } // mdot
    ...
    {   
        if (dim1 != b.dim1) flaw ("mdot", "incompatible first dimensions")
        val vv = Array.ofDim [RleVectorR] (d2)   
        for (j <- b.range2) vv(j) = RleVectorR (for (i <- range2) yield  v(i) dot b.v(j))
        new RleMatrixR (dim2, b.dim2, vv)
    } // mdot
    ...
    def inverse: MatriR = 
    {
        var b = new RleMatrixR (dim1, dim2)
        var u = eye (dim1)
     ...
     */
    def lud: Tuple2 [RleMatrixR, RleMatrixR] = 
    {
        val l = new RleMatrixR (dim1, dim2)
        val u = new RleMatrixR (dim1, dim2)
    ...
    def lud_ip: (RleMatrixR, RleMatrixR) = 
    {
        ...
        var l = new RleMatrixR (dim1, dim2)
    ...
    def reduce: RleMatrixR = 
    {
        ...
        val b = new RleMatrixR (dim1, dim2)
        for (i <- range1; j <- range2) b(i, j) = this(i, j)
        for (i <- range1) {
            var x = b(i,i)
     ...
     */
    def solve (l: MatriR,u: MatriR, b: VectoR): RleVectorR = 
    {
        var y = new RleVectorR (l.dim2)
        for (i <- 0 until y.dim) {
            var sum = _0
            for (j <- 0 until i) sum += l(i, j) * y(j)
            y(i) = b(i) - sum
        } // for
        u.bsolve (y).asInstanceOf [RleVectorR]
    } // solve
     ...
     *  @param b   the constant vector
     */
    def solve (lu: Tuple2 [MatriR, MatriR], b: VectoR): RleVectorR = solve (lu._1, lu._2, b)
     ...
     *  @param b  the constant vector
     */
    def solve (u: MatriR, b: VectoR): RleVectorR = solve (this, u, b)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Solve for 'x' in the equation 'a*x = b' where 'a' is 'this' matrix.
     *  @param b  the constant vector.
     */ 
    def solve (b: VectoR): RleVectorR = solve (lud, b)
    ...
    {
        b.isInstanceOf [RleMatrixR] && (v.deep equals b.asInstanceOf [RleMatrixR].v.deep)
    } // equals
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `RleMatrixR` companion object provides operations for `RleMatrixR` that don't require
...
object RleMatrixR 
{
    ...
    /**
     */
    def apply (x: RleVectorR, xs: RleVectorR*): RleMatrixR = 
    {     
        new RleMatrixR (xs.length + 1, x.dim, (x +: xs).toArray)    
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def apply (x: Seq [RleVectorR]): RleMatrixR = 
    {
        new RleMatrixR (x(0).dim, x.length, x.toArray)
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def apply (u: Vector [VectoR]): RleMatrixR = 
    {
        val u_dim = u(0).dim
        val x = new RleMatrixR (u_dim, u.length)
    ...
    /**
     */
    def apply (x: MatrixR): RleMatrixR = 
    {
        RleMatrixR (for ( j <- 0 until x.dim2) yield RleVectorR (x.col(j)))
    } // apply
  
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def mul (at: RleMatrixR, u: VectoR): RleVectorR = 
    {
        RleVectorR (for (i <- 0 until at.dim1) yield at(i) dot u) 
    } // mul

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /**
     */
    def mul (a: RleMatrixR, u: RleVectorR): RleVectorR = 
    {
        RleVectorR (for (i <- 0 until a.dim1) yield a(i) dot u)    
    } // mul
 ...
 *  > run-main scalation.linalgebra.RleMatrixRTest
 */
object RleMatrixRTest extends App with PackageInfo
{
    for (l <- 1 to 4) {
        println ("\n\tTest RleMatrixR on real matrices of dim " + l)
        val x = new RleMatrixR (l, l)
        val y = new RleMatrixR (l, l)
        x.set (2)
        y.set (3)
        println ("x + y = " + (x + y))
        println ("x - y = " + (x - y))
    ...
    println ("\n\tTest RleMatrixR on additional operations")

    val z   = RleMatrixR (new MatrixR ((2, 2), 1, 2,
                                               3, 2))
    val t   = RleMatrixR (new MatrixR ((3, 3), 1, 2, 3,
                                               4, 3, 2,
                                               1, 3, 1))
    val zz  = RleMatrixR (new MatrixR ((3, 3), 3, 1, 0,
                                               1, 4, 2,
                                               0, 2, 5))
    val bz  = VectorR (5, 3, 6)
    val b   = VectorR (8, 7)
    val lu  = z.lud
    ...
    val w = RleMatrixR (new MatrixR ((2, 3), 2,  3, 5, 
                                            -4,  2, 3))
    val v = RleMatrixR (new MatrixR ((3, 2), 2, -4, 
                                             3,  2,
                                             5,  3))
    println ("w         = " + w)
    println ("v         = " + v)
    ...
    val aa = RleMatrixR (new MatrixR ((3, 2), 1, 2,
                                              3, 4,
                                              5, 6))
    val bb = RleMatrixR (new MatrixR ((2, 2), 1, 2,
                                              3, 4))
    val cc = RleMatrixR (new MatrixR ((3, 2), 3, 4,
                                              5, 6,
                                              7, 8))
    ...
    bb.write (filename)
    println ("bb_csv = " + MatrixR (filename))

} // RleMatrixRTest object
