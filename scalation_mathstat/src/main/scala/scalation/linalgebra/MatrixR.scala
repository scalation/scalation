 ...
 */
class MatrixR (d1: Int,
               d2: Int,
               private [linalgebra] var v: Array [Array [Real]] = null)
      extends MatriR with Error with Serializable
{
     ...
     */
    def this (u: Array [Array [Real]]) { this (u.length, u(0).length, u) }
     ...
     */
    def zero (m: Int = dim1, n: Int = dim2): MatrixR = new MatrixR (m, n)
     ...
     */
    def apply (i: Int, j: Int): Real = v(i)(j)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get 'this' matrix's vector at the 'i'-th index position ('i'-th row).
     *  @param i  the row index
     */
    def apply (i: Int): VectorR = VectorR (v(i))
     ...
     *  @param x  the scalar value to assign
     */
    def update (i: Int, j: Int, x: Real) { v(i)(j) = x }
    ...
    {
        if (b.isInstanceOf [MatrixR]) {
            val bb = b.asInstanceOf [MatrixR]
            for (i <- ir; j <- jr) v(i)(j) = bb.v(i - ir.start)(j - jr.start)
        } else {
            flaw ("update", "must convert b to a MatrixR first")
        } // if
    } // update
     ...
     *  @param u  the 2D array of values to assign
     */
    def set (u: Array [Array [Real]]) { for (i <- range1; j <- range2) v(i)(j) = u(i)(j) }
...
//  } // t
    ...
    def ++^ (b: MatriR): MatrixR =
    {
        ...
        val c = new MatrixR (dim1, dim2 + b.dim2)
     ...
     *  @param b   the constant vector
     */
    def solve (lu: (MatriR, MatriR), b: VectoR): VectorR = solve (lu._1, lu._2, b)
     ...
     *  @param b  the constant vector
     */
    def solve (u: MatriR, b: VectoR): VectorR = solve (this, u, b)
     ...
     */
    def diag (p: Int, q: Int = 0): MatrixR =
    {
     ...
     */
    def setDiag (x: Real) { for (i <- 0 until math.min (dim1, dim2)) v(i)(i) = x }
     ...
     */
    def clean (thres: Double = TOL, relative: Boolean = true): MatrixR =
    {
    ...
    /** Create a normalized version of 'this' matrix.
      */
    def normalizeU: MatrixR = MatrixR (for (j <- range2) yield col(j).normalizeU)
    ...
    {
        if (! b.isInstanceOf [MatriR]) return false
        val bb = b.asInstanceOf [MatriR]
...
object MatrixR extends Error
{
     ...
     */
    def apply (u: Seq [VectoR], columnwise: Boolean = true): MatrixR =
    {
        var x: MatrixR = null
        val u_dim = u(0).dim
        if (columnwise) {
            x = new MatrixR (u_dim, u.length)
            for (j <- 0 until u.length) x.setCol (j, u(j))    // assign column vectors
        } else {
            x = new MatrixR (u.length, u_dim)
    ...
    {
        val u_dim = u(0).dim
        val x = new MatrixR (u_dim, u.length)
        for (j <- 0 until u.length) x.setCol (j, u(j))        // assign column vectors
        x
    } // apply
     ...
     *  @param u    the repeated values
     */
    def apply (dim: (Int, Int), u: Real*): MatrixR =
    {
        val x = new MatrixR (dim._1, dim._2)
        var k = 0
    ...
    def apply (fileName: String): MatrixR =
    {
        ...
        for (i <- x.range1) {
            x(i) = VectorR (lines(i).split (sp))
            if ((i+1) % PROGRESS == 0) println (s"Read 4 rows ...")
        } // for
        x
    } // apply
    ...
    def apply (fileName: String, skip: Int): MatrixR =
    {
        ...
        val x      = new MatrixR (m, n)
        for (i <- x.range1) {
            x(i) = VectorR (lines(i+skip).split (sp))
            if ((i+1) % PROGRESS == 0) println (s"Read 4 rows ...")
        } // for
        x
    } // apply
     ...
     */
    def ++ (u: VectoR, w: VectoR): MatrixR =
    {
        if (u.dim != w.dim) flaw ("++", "vector dimensions do not match")

        val c = new MatrixR (2, u.dim)
        c(0) = u
        c(1) = w
        c
    } // ++
     ...
     */
    def outer (x: VectoR, y: VectoR): MatrixR =
    {
     ...
     */
    def form_rw (x: VectoR, y: VectoR): MatrixR =
    {
     ...
     *  @param y  the second vector -> row 1
     */
    def form_rw (x: Real, y: VectoR): MatrixR =
    {
        val cols = y.dim
        val c = new MatrixR (2, cols)
     ...
     */
    def form_rw (x: VectoR, y: Real): MatrixR =
    {
        val cols = x.dim
        val c = new MatrixR (2, cols)
        c(0) = x
     ...
     */
    def form_cw (x: VectoR, y: VectoR): MatrixR =
    {
     ...
     *  @param y  the second vector -> column 1
     */
    def form_cw (x: Real, y: VectoR): MatrixR =
    {
        val rows = y.dim
        val c = new MatrixR (rows, 2)
     ...
     */
    def form_cw (x: VectoR, y: Real): MatrixR =
    {
        val rows = x.dim
        val c = new MatrixR (rows, 2)
        c.setCol(0, x)
...
object MatrixRTest extends App with PackageInfo
{
    ...
    println ("check left  nullspace = " + MatrixR.times (v.t.nullspace, v))
