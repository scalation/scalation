...
import scalation.linalgebra.{Fac_QR_H3, MatrixD, VectorD}
 ...
 *  > test
 */
class Fac_QR_H3_T { @Test def testAll () { Fac_QR_H3_T } }
 ...
 *  > test:runMain testing.linalgebra.Fac_QR_H3_T
 */
object Fac_QR_H3_T extends Tester with App
{
    ...
    KLASS   = "Fac_QR_H3"                                          // the class under test
    ...
    private val rvg  = RandomVecD (dim1)                           // random matrix generator
    ...
    test ("factor",    { val (q, r) = (new Fac_QR_H3 (x)).factor12 (); q * r },
                       x)
    test ("q.t * q",   { val q = (new Fac_QR_H3 (x)).factor1 (); (q.t * q).asInstanceOf [MatrixD].clean (TOL) },
                       eye (dim2))
    test ("solve",     { val qr = new Fac_QR_H3 (x); qr.factor (); qr.solve (y) },
                       { val (q, r) = (new Fac_QR_H3 (x)).factor12 (); r.bsolve (q.t * y) })
