...
import collection.mutable.{PriorityQueue, SynchronizedQueue}
...
import scalation.util.{Error, Identity, PQItem}
...
class Transition (val x: Double, val y: Double, firingDist: Variate, colors: Array [Color])
      extends PQItem with Ordered [Transition] with PetriNetRules with Identity
{
     ...
     */
    var cqueue: SynchronizedQueue [AnimateCommand] = null
        ...
        for (iD <- inD) {                                          // for each incoming continuous arc
            ...
            for (i <- 0 until _fluids.dim) {
                ...
                if (amount > 0) {
                    ...
                    cqueue += AnimateCommand (ScaleTokensAt, -1, null, null, false,
      ...
      extends PetriNetRules with Error
{
                       ...
                       timeDilationFactor: Double = 1000.)
    {
        ...
        for (pI <- placeI) {
            ...
            for (i <- 0 until tokens.dim) {          // number of tokens by color at this place
                for (j <- 0 until tokens(i)) {
                    ...
                    cqueue += AnimateCommand (CreateToken, tk_id, Ellipse (),
                           "tk" + tk_id, false, colors(i), null, 0, pI.id)
                } // for
            } // for
        } // for
        ...
        for (pD <- placeD) {
            cqueue += AnimateCommand (CreateNode, pD.id, Ellipse (),
            ...
            for (i <- 0 until fluids.dim) {          // amount of fluids by color at this place
                ...
                if (amount > 0) {
                    ...
                    cqueue += AnimateCommand (CreateToken, fl_id, Ellipse (),
                           "fl" + fl_id, false, colors(i), Array [Double] (amount, amount), 0, pD.id)
                } // if
            } // for
        } // for
        ...
        for (tr <- transition) {
            cqueue += AnimateCommand (CreateNode, tr.id, Rectangle (),
                   "tr" + tr.id, true, gColors(2), Array [Double] (tr.x, tr.y, 30, 60), 0)
            for (aI <- tr.inI) {
                cqueue += AnimateCommand (CreateEdge, aI.id, QCurve (),
                       "aI" + aI.id, true, gColors(3), null, 0, aI.place.id, aI.transition.id)
            } // for
            for (aD <- tr.inD) {
                cqueue += AnimateCommand (CreateEdge, aD.id, QCurve (),
                       "aD" + aD.id, true, gColors(3), null, 0, aD.place.id, aD.transition.id)
            } // for
            for (aI <- tr.outI) {
                cqueue += AnimateCommand (CreateEdge, aI.id, QCurve (),
                       "aI" + aI.id, true, gColors(3), null, 0, aI.transition.id, aI.place.id)
            } // for
            for (aD <- tr.outD) {
                cqueue += AnimateCommand (CreateEdge, aD.id, QCurve (),
                       "aD" + aD.id, true, gColors(3), null, 0, aD.transition.id, aD.place.id)
            } // for
        } // for
