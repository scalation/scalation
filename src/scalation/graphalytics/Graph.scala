...
// *  @param labelMap  the map from label to the set of vertices with the label
 *  @param inverse   whether to store inverse adjacency sets (parents)
 */
class Graph (val adj:      Array [ISet],                    // child vertices
                 ...
                 inverse:  Boolean = false)                 // parent vertices flag
{
     ...
     */
    var par: Array [ISet] = null     // by default, don't use 'par'
     ...
     */
    def buildLabelMapFromLabels (label: Array [Int]): Map [TLabel, ISet] =
    {
...
object Graph
{
     ...
     *  @param inverse   whether to store inverse adjacency sets (parents)
     */
    def apply (gFile: String, inverse: Boolean = false): Graph =
    {
