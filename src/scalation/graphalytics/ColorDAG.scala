 ...
 *  The ColorDAG consists of source nodes (in-degree is 0), sink nodes (out-degree is 0) and
 *  internal nodes.  The edges connecting nodes must have color compatibility.
    ...
    def animateDAG
    {
        ...
        for (i <- 0 until stages) {                               // display nodes
            ...
            for (n <- nodes(i)) {
                ...
                if (i == 0) {
                    cq += AnimateCommand (CreateNode, n.id, Ellipse (), "", false, n.outColors(0).colr,
                                          Array (x, y, nSize, nSize), 0)
                } else if (i == stages-1) {
                    cq += AnimateCommand (CreateNode, n.id, Ellipse (), "", false, n.inColors(0).colr,
                                          Array (x, y, nSize, nSize), 0)
                } else {
                    cq += AnimateCommand (CreateNode, n.id, Rectangle (), "", false, n.inColors(0).colr,
                                          Array (x, y, nSize, nSize), 0)
                    cq += AnimateCommand (CreateNode, 1000 + n.id, Rectangle (), "", false, n.outColors(0).colr,
