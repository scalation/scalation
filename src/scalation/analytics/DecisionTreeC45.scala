...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** This class implements a Decision Tree classifier using the C4.5 algorithm.
 ...
 *  @param featr   the string array holding the feature names
 ...
 */
class DecisionTreeC45 (val x: MatrixI, val y: Array [Int], featr: Array [String],
                       isCont: Array [Boolean], vc: Array [Int], k: Int = 2)
      extends Classifier with Error
{
    ...
    var root: DecisionTreeC45#Node = null        // decision tree, store according to layers of tree
    var threshold = new Array [Double] (n)       // threshold for continuous features (below <=, above >)
                ...
                val leaf: Boolean = false, val decision: Int = -1)
    {
        ...
        {
            if (leaf) {
                "Node (LeafOf: " + featr(f) + "\t" + "BranchValue: "  + value + "\tclass: " + decision + ")"
            } else if (isCont(f)) {
                if (value == -1) {
                    "Node (feature: " + featr(f) + "\t BranchValue: ROOT" + "\tThreshold: " + threshold + ")"
                } else {
                    "Node (feature: " + featr(f) + "\t BranchValue: " + value + "\tThreshold: " + threshold + ")"
                } // if
            } else {
                if (value == -1) {
                    "Node (feature: " + featr(f) + "\t BranchValue: ROOT" + ")"
                } else {
                    "Node (feature: " + featr(f) + "\t BranchValue: " + value + ")"
                } // if
            } // if
        } // toString
    } // Node class
     ...
     */
    def nextXY (f: Int, value: Int): Tuple2 [MatrixI, Array [Int]] =
    {
        var count = 0
        ...
        val ny = new Array [Int] (count)       // new y array
    ...
    def buildTree (opt: Tuple2 [Int, Double])
    {
        ...
        for (i <- 0 until vc(opt._1)) {
            ...
            var flag = true
            for (j <- 0 until next._2.length-1 if next._2(j) != next._2(j+1)) flag = false
            if (flag) {
                root.next += new Node (opt._1, root.next.length, -1, true, next._2(1))
                if (DEBUG) {
                    println (" --> Leaf = " + root.next)
                    println ("\t x      = " + next._1)
                    println ("\t y      = " + next._2.deep)
                    println ("\t vc     = " + vc.deep)
                    println ("\t isCont = " + isCont.deep)
                } // if
            } else {
                var subtree = new DecisionTreeC45 (next._1, next._2, featr, isCont, vc, k)
                subtree.train ()
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** This object is used to test the DecisionTreeC45 class.  Ex: Classify (No/Yes)
...
object DecisionTreeC45Test extends App
{
    ...
    val vc = Array (3, 3, 2, 2)
    val feature = Array ("Outlook", "Temp", "Humidity", "Wind") // distinct values for each feature
    val flag = Array (false, false, false, false)
...
//  val flag = Array (false, true, true, false)
    ...
    val cl = new DecisionTreeC45 (x, y, feature, flag, vc)       // create the classifier            
    cl.train ()
    cl.printTree
