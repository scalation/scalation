...
case class Polygon (vertex: Array [R2])
     extends java.awt.geom.Path2D.Double with Error
{
    {
        val n = vertex.length
        if (n < 3) flaw ("constructor", "need at least 3 vertices to make a polygon")
     ...
     extends java.awt.geom.Path2D.Double with Error
{
    {
        val n = vertex.length
        if (n != 3) flaw ("constructor", "need exactly 3 vertices to make a triangle")
        moveTo (vertex(0).x, vertex(0).y)
        for (i <- 1 until n) lineTo (vertex(i).x, vertex(i).y)
        lineTo (vertex(0).x, vertex(0).y)
    } // primary constructor
     ...
     *  @param side2    the heigth of the triangle (change in y)
     */
     ...
     extends java.awt.geom.Path2D.Double with Error
{
    {
        val n = vertex.length
        if (n != 4) flaw ("constructor", "need exactly 4 vertices to make a quad")
        moveTo (vertex(0).x, vertex(0).y)
        for (i <- 1 until n) lineTo (vertex(i).x, vertex(i).y)
        lineTo (vertex(0).x, vertex(0).y)
    } // primary constructor
     ...
     extends java.awt.geom.Path2D.Double with Error
{
    {
        val n = vertex.length
        if (n != 5) flaw ("constructor", "need exactly 5 vertices to make a pentagon")
        moveTo (vertex(0).x, vertex(0).y)
        for (i <- 1 until n) lineTo (vertex(i).x, vertex(i).y)
        lineTo (vertex(0).x, vertex(0).y)
    } // primary constructor
     ...
     extends java.awt.geom.Path2D.Double with Error
{
    {
        val n = vertex.length
        if (n != 6) flaw ("constructor", "need exactly 6 vertices to make a hexagon")
        moveTo (vertex(0).x, vertex(0).y)
        for (i <- 1 until n) lineTo (vertex(i).x, vertex(i).y)
        lineTo (vertex(0).x, vertex(0).y)
    } // primary constructor
     ...
     extends java.awt.geom.Path2D.Double with Error
{
    {
        val n = vertex.length
        if (n != 8) flaw ("constructor", "need exactly 8 vertices to make an octagon")
        moveTo (vertex(0).x, vertex(0).y)
        for (i <- 1 until n) lineTo (vertex(i).x, vertex(i).y)
        lineTo (vertex(0).x, vertex(0).y)
    } // primary constructor
