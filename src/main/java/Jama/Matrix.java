 ...
 * ------------------------ */
   ...
   */
    ...
    *  @exception  ArrayIndexOutOfBoundsException
    */
   public VectorD get (int i)
   {
      VectorD v = new VectorD (n);
      double [] c = v.getArray ();
      for (int j = 0; j < n; j++) c[j] = A[i][j];
      return v;
   } // get
   ...
   */
   ...
   public Matrix times (Matrix B) {
      ...
      for (int j = 0; j < B.n; j++) {
         ...
         for (int i = 0; i < m; i++) {
            double[] Arowi = A[i];
            double s = 0;
            for (int k = 0; k < n; k++) s += Arowi[k]*Bcolj[k];
            C[i][j] = s;
         } // for
      } // for
      return X;
   ...
   /** Linear algebraic matrix-vector multiplication, A * v
   @param v    the vector
   @return     Matrix-vector product, A * v
   @exception  IllegalArgumentException Matrix and vector dimensions must agree.
   */
   ...
   public double [] times (double [] v) {
      ...
      for (int i = 0; i < m; i++) {
         double [] Arowi = A[i];
         double s = 0;
         for (int j = 0; j < n; j++) s+= Arowi[j] * v[j];
         u[i] = s;
      } // for
      return u;
   } // times
