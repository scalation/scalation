      ...
      extends Traversable [Double] with PartiallyOrdered [VectorD] with Error with Serializable
{
    ...
    def toInt: VectorI =
    {
        ...
        for (i <- range) c(i) = (round (v(i))).toInt
        c
    } // toInt
     ...
     */
    def mean: Double = v.sum / v.length.toDouble
...
object VectorD
{
    ...
    def apply (x: String, xs: String*): VectorD =
    {
        val c = new VectorD (1 + xs.length)
        c(0)  = x.toDouble
        for (i <- 1 until c.dim) c.v(i) = xs(i-1).toDouble
        c
    } // apply
     ...
     */
    def apply (xa: Array [String]): VectorD = 
    {
        val c = new VectorD (xa.length)
        for (i <- c.range) c.v(i) = xa(i).toDouble
        c
    } // apply
