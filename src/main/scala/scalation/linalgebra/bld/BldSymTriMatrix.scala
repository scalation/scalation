 ...
 *  > run-main scalation.linalgebra.bld.BldSymTriMatrix
 */
      ...
      extends $MATRI with Error with Serializable
{
    ...
    {
        throw new NoSuchMethodException ("SymTri$MATRIX must be symmetric")
    } // slice
     ...
     */
    def sliceExclude (row: Int, col: Int): SymTri$MATRIX =
    {
        throw new NoSuchMethodException ("SymTri$MATRIX does not support sliceExclude")
    } // sliceExclude
    ...
    {
        throw new NoSuchMethodException ("SymTri$MATRIX does not support selectRows")
    } // selectRows
    ...
    {
        throw new NoSuchMethodException ("SymTri$MATRIX does not support selectCols")
    } // selectCols
     ...
     */
    def ++ (u: $VECTOR): SymTri$MATRIX =
    {
        throw new NoSuchMethodException ("SymTri$MATRIX does not support ++")
    } // ++
    ...
    def + (u: $VECTOR): $MATRIX =
    {
        throw new NoSuchMethodException ("SymTri$MATRIX does not support + for $VECTOR")
    } // +
    ...
    def * (b: $MATRI): SymTri$MATRIX = 
    {
        throw new NoSuchMethodException ("SymTri$MATRIX does not support * with general matrices")
    } // *
     ...
     */
    def dot (u: $VECTOR): $VECTOR = this * u
    ...
    def solve (l: $MATRI, u: $MATRI, b: $VECTOR): $VECTOR = 
    {
        ...
        val ll = l.asInstanceOf [$MATRIX]
        val uu = u.asInstanceOf [$MATRIX]
        ll.solve (uu, b)
    } // solve
    ...
    def solve (b: $VECTOR): $VECTOR =
    {
        ...
        val d = _dg                           // diagonal
        val e = new $VECTOR (_sd  ++ $ZERO)     // augmented superdiagonal
     ...
     */
    def sumAbs: $BASE = _dg.sumAbs + _sd.sumAbs + _sd.sumAbs
...
object SymTri$MATRIX extends Error
{
     ...
     */
    def apply (u: Array [$VECTOR], columnwise: Boolean = true): SymTri$MATRIX =
    {
     ...
     */
    def apply (u: Vector [$VECTOR]): SymTri$MATRIX =
    {
        val u_dim = u(0).dim
 ...
 *  > run-main scalation.linalgebra.SymTri${MATRIX}Test
 */
