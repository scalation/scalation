...
object SparseMatrixR
{
     ...
     */
    def apply (u: Array [VectorR], columnwise: Boolean = true): SparseMatrixR =
    {
     ...
     */
    def apply (u: Vector [VectorR]): SparseMatrixR =
    {
        val u_dim = u(0).dim
      ...
      extends MatriR with Error with Serializable
{
     ...
     */
    def setCol (col: Int, u: VectorR) { for (i <- range1) this(i, col) = u(i) }
     ...
     *  @param b  the constant vector.
     */
    def solve (b: VectorR): VectorR = solve (lud, b)
...
object SparseMatrixRTest extends App
{
    ...
    for (l <- 1 to 4) {
        println ("\n\tTest SparseMatrixR on real matrices of dim " + l)
        val x = new SparseMatrixR (l, l)
        val y = new SparseMatrixR (l, l)
        x.set (2)
        y.set (3)
        println ("x + y = " + (x + y))
        println ("x - y = " + (x - y))
    ...
    println ("\n\tTest SparseMatrixR on additional operations")
