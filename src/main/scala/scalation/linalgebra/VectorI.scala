...
//    extends Traversable [Int] with PartiallyOrdered [VectorI] with Vec with Error with Serializable
{
     ...
     */
    def this (iv: Tuple2 [Int, Int], dm: Int) { this (dm); v(iv._1) = iv._2 }
     ...
     *  @param p  the predicate (Boolean function) to apply
     */
    def filterPos (p: Int => Boolean): Seq [Int] = for (i <- range if p (v(i))) yield i
     ...
     */
    def sum: Int = v.foldLeft (0)((s, x) => s + x)
