...
/** @author  John Miller
 *  @version 1.1
 *  @date    Sat Jul 20 22:34:05 EDT 201
...
import scalation.math.Rational.{abs, fromDouble, _0}
      ...
      extends Matrir with Error with Serializable
{
     ...
     */
    def this (dim1: Int, x: Rational, y: Rational)
    {
    ...
    {
        this (dim._1, dim._2)                      // invoke primary constructor
        for (i <- range1; j <- range2) v(i)(j) = if (j == 0 && j == 0) fromDouble (u00)
                                               else fromDouble (u(i * dim2 + j - 1))
    } // constructor
   ...
   //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    {
        val c = new MatrixR (dim1 - 1, dim2 - 1)
    ...
    def *= (b: MatrixR): MatrixR =
    {
        val c = if (b == this) new MatrixR (this) else b
        for (i <- range1) {
            val row_i = new VectorR (row(i))          // save so not overwritten
            for (j <- range1) {
                var sum = _0
    ...
    private def partialPivoting (a: MatrixR, i: Int): Int =
    {
        ...
        for (k <- i + 1 until a.dim1 if Rational.abs (a(k, i)) > max) {
            max  = Rational.abs (a(k, i))
            kMax = k
        } // for
     ...
     */
    private def swap (a: MatrixR, i: Int, k: Int, col: Int)
    {
    ...
    def diag (p: Int, q: Int): MatrixR =
    {
        ...
        for (i <- 0 until n; j <- 0 until n) {
            c.v(i)(j) = if (i < p || i > p + dim1) if (i == j) Rational._1 else _0
                        else                       v(i-p)(j-p)
        } // for
        c
    } // diag
    ...
    def inverse_npp: MatrixR =
    {
        ...
        val c = new MatrixR (dim1, Rational._1, _0)           // let c represent the augmentation
     ...
     *  @param thres     the cutoff threshold (a small value as a rational number)
     *  @param relative  whether to use relative or absolute cutoff
     */
    def clean (thres: Rational, relative: Boolean = true): MatrixR =
    {
        val s = if (relative) mag else Rational._1             // use matrix magnitude or 1
    ...
    def nullspace: VectorR =
    {
        ...
        reduce.col(dim2 - 1) * Rational._1n  ++ Rational._1
    } // nullspace
    ...
    def nullspace_ip: VectorR =
    {
        if (dim2 != dim1 + 1) flaw ("nullspace", "requires n (columns) = m (rows) + 1")
        reduce_ip
        col(dim2 - 1) * Rational._1n  ++ Rational._1
    } // nullspace_ip
...
object MatrixR extends Error
{
     ...
     *  @param y  the second vector -> row 1
     */
    def form_rw (s: Rational, y: VectorR): MatrixR =
    {
        val cols = y.dim
        val c = new MatrixR (2, cols)
     ...
     */
    def form_rw (x: VectorR, s: Rational): MatrixR =
    {
        val cols = x.dim
        val c = new MatrixR (2, cols)
        c(0) = x
     ...
     *  @param y  the second vector -> column 1
     */
    def form_cw (s: Rational, y: VectorR): MatrixR =
    {
        val rows = y.dim
        val c = new MatrixR (rows, 2)
     ...
     */
    def form_cw (x: VectorR, s: Rational): MatrixR =
    {
        val rows = x.dim
        val c = new MatrixR (rows, 2)
        c.setCol(0, x)
