...
class MatrixR (val d1: Int,
               val d2: Int,
       private var v:  Array [Array [Real]] = null)
      extends MatriR with Error with Serializable
{
     ...
     */
    def set (i: Int, u: VectorR, j: Int = 0) { for (k <- 0 until u.dim) v(i)(k+j) = u(k) }
     ...
     *  @param b  the constant vector
     */
    def solve (u: MatriR, b: VectorR): VectorR = solve (this, u, b)
...
object MatrixR extends Error
{
     ...
     */
    def apply (u: Array [VectorR], columnwise: Boolean = true): MatrixR =
    {
        var x: MatrixR = null
        val u_dim = u(0).dim
        if (columnwise) {
     ...
     */
    def apply (u: Vector [VectorR]): MatrixR =
    {
        val u_dim = u(0).dim
     ...
     *  @param y  the second vector -> row 1
     */
    def form_rw (x: Real, y: VectorR): MatrixR =
    {
        val cols = y.dim
        val c = new MatrixR (2, cols)
     ...
     */
    def form_rw (x: VectorR, y: Real): MatrixR =
    {
        val cols = x.dim
        val c = new MatrixR (2, cols)
        c(0) = x
     ...
     *  @param y  the second vector -> column 1
     */
    def form_cw (x: Real, y: VectorR): MatrixR =
    {
        val rows = y.dim
        val c = new MatrixR (rows, 2)
     ...
     */
    def form_cw (x: VectorR, y: Real): MatrixR =
    {
        val rows = x.dim
        val c = new MatrixR (rows, 2)
        c.setCol(0, x)
