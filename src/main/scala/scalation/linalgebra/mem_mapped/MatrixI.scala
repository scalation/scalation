...
import scalation.util.{Error, MM_ArrayI, PackageInfo}
...
class MatrixI (val d1: Int,
               val d2: Int,
       private var v:  Array [MM_ArrayI] = null)
      extends MatriI with Error with Serializable
{
    ...
    if (v == null) {
        v = Array.ofDim [MM_ArrayI] (dim1)
        for (i <- 0 until dim1) v(i) = MM_ArrayI.ofDim (dim2)
     ...
     */
    def this (u: Array [MM_ArrayI]) { this (u.length, u(0).length, u) }
     ...
     */
    def set (i: Int, u: VectorI, j: Int = 0) { for (k <- 0 until u.dim) v(i)(k+j) = u(k) }
     ...
     *  @param b  the constant vector
     */
    def solve (u: MatriI, b: VectorI): VectorI = solve (this, u, b)
...
object MatrixI extends Error
{
     ...
     */
    def apply (u: Array [VectorI], columnwise: Boolean = true): MatrixI =
    {
        var x: MatrixI = null
        val u_dim = u(0).dim
        if (columnwise) {
     ...
     */
    def apply (u: Vector [VectorI]): MatrixI =
    {
        val u_dim = u(0).dim
