...
import collection._
import collection.mutable.{ListBuffer, Map, HashMap, MutableList, Set => SET, Stack}
import math.pow
      ...
      extends DigMatcher (g, q)
{
    ...
    private val qmet = new DigMetrics (q.clone, false)                        // creating graph metrics object of query graph

    private val dataSize  = g.size                                            // size of the data graph
    private val querySize = q.size                                            // size of the query graph

    private val phi0 = new MDigDualSim (g, q).mappings ()
    ...
    def strictSim (phi: Array [SET [Int]]): HashMap [Int, Array [SET [Int]]] =
    {
        ...
        val balls      = HashMap [Int, DigBall] ()                      // map of balls: center -> ball               
	...
	val bCenters   = SET [Int] ()                                   // set of centers for all balls
        ...
        for (center <- gCenters) {                                      // for each mapped data graph center
            val ball = new DigBall (newGraph, center, qDiameter)        // create a new ball for that center vertex
     ...
     */ 
    def filterGraph (phi: Array [SET [Int]]): MDigraph = 
    {
        ...
        new MDigraph (newCh, g.label, g.elabel, g.inverse, g.name + "2")    // create a new data graph
    } // filterGraph
 
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Perform dual simulation onto the ball.
     *  @param phi  mappings from a query vertex u_q to { graph vertices v_g }
     *  @param ball the Ball B(Digraph, Center, Radius)
     */ 
    def dualFilter (phi: Array [SET [Int]], ball: DigBall): Array [SET [Int]] = 
    {
     ...
     *  @param g               the data graph  G(V, E, l)
     *  @param balls           mappings from a center vertex to the Ball B(Digraph, Center, Radius)
     *  @param matchCenters    set of all vertices which are considered as center
     */
    def calculateTotalEdges (g: MDigraph, balls: HashMap [Int, DigBall], matchCenters: SET [Int]): Int = 
    {
     ...
     *  @param balls  mappings from a center vertex to the Ball B(Digraph, Center, Radius)
     */
    def calculateBallDiameterMetrics (balls: HashMap [Int, DigBall]): Statistic =
    {
     ...
     */
    def selectivityCriteria (qmet: DigMetrics): Int =
    {
        var index = 0
        var max   = 0.0
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: ::::::::::::
/** The `MDigStrictSimTest` object is used to test the `MDigStrictSim` class.
 *  > run-main scalation.graphalytics.MDigStrictSimTest
 */
object MDigStrictSimTest extends App 
{
    val g = MDigraph.g1p
    val q = MDigraph.q1p
    ...
    (new MDigStrictSim (g, q)).test ("MDigStrictSim")   // Strict Graph Simulation Pattern Matcher

} // MDigStrictSimTest object


//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: ::::::::::::
/** The `MDigStrictSimTest2` object is used to test the `MDigStrictSim` class.
 *  > run-main scalation.graphalytics.MDigStrictSimTest2
 */
object MDigStrictSimTest2 extends App 
{
    val g = MDigraph.g2p
    val q = MDigraph.q2p
...
} // MDigStrictSimTest2 object


//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `MDigStrictSimTest3` object test the `MDigStrictSim` class by passing data graph
 *  and query graph relative file paths.
 *  > run-main scalation.graphalytics.MDigStrictSimTest3
 */
object MDigStrictSimTest3 extends App 
{
    val g = MDigIO ("gfile")
    val q = MDigIO ("qfile")
