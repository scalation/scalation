...
class TightSimulation ( printSetting: String) 
{
     ...
     */
    def tightSim (q: Graph, g: Graph, queryName: String, dataName: String): Array [ISet] = 
    {
     ...
     */ 
    def filterGraph (g: Graph, q: Graph,  sim: Array [ISet]): Graph = 
    {
        ...
        for (i <- 0 until newAdjSet.size) {
            newAdjSet(i)  = immutable.Set [Int] ()
            newParList(i) = immutable.Set [Int] ()
        } // for
     ...
     */ 
    def dualFilter (query: Graph, sim: Array [ISet], ball: Ball): Array [ISet] = 
    {
        ...
        for ( v <- sim.indices if sim(v).isEmpty) return Array [ISet] ()
        sim
    } //dualFilter
    ...
    def performPostProcessing (g: Graph, balls: HashMap [Int, Ball], matchCenters: Set [Int]) 
    {
        ...
        for (vert_id <- 0 until g.adj.length) {                            // check for every vertex of a data graph
            ...
            if (balls.keySet contains vert_id) {                           // if its a center vertex, then need to perform post processing
                ...
                val str = immutable.Set (ballstring.replaceAll ("[\\[\\]\\-\\>,]*", " ").replaceAll ("  ", ",").replaceAll (" ","").split (","):_*)
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `TightSimulationTest` object test the `TightSimulation` class by passing
...
object TightSimulationTest extends App 
{
    if (args.length != 3) {
        ...
        time { new TightSimulation (printSetting).tightSim (q, g, qfile, gfile) }
    } // if
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: ::::::::::::
/** The `TightSimulationTest2` object test the `TightSimulation` class by feeding
 *  data graph, query graph absolute file path, print match/ball (0/1).
 */
