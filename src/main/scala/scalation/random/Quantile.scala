       ...
       extends Error
{
     ...
     */
    def studentTInv2 (p: Double = .95, _df: Array [Int] = Array (10)): Double =
    {
        val extreme = check (p)                        // handle extreme cases
        if (extreme._1) return extreme._2
...
trait QuantileTest
{
    ...
    //:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Test the given iCDF 'fi' over a range of 'p' values for the given degrees
     *  of freedom (df).
     *  @param fi     the iCDF Finv(.)
     *  @param name   the name of iCDF Finv(.)
     *  @param df     the array giving the degrees of freedom
     */
    def test_df (fi: Distribution, name: String, df: Array [Int] = Array ())
    {
        println ("-----------------------------------------------------------")
        ...
        for (i <- 0 to n) {
            val p = i / n.toDouble
            println (s"$name ($p, ${df.deep})\t = ${fi(p, df)}")
        } // for
    } // test_df
    ...
    def test (icdf: String)
    {
        icdf match {
        case "normalInv" =>
            test_df (normalInv, "normalInv")
        case "studentTInv" =>
            for (df <- 1 to 30)
                test_df (studentTInv, "studentTInv", Array (df))
//              test_df (studentTInv2, "studentTInv", Array (df))
        case "chiSquareInv" =>
            for (df <- 1 to 30)
                test_df (chiSquareInv, "chiSquareInv", Array (df))
        case _ =>
            for (df1 <- 1 to 10; df2 <- 1 to 10)
                test_df (fisherInv, "fisherInv", Array (df1, df2))
        } // match
    } // test
